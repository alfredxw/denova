...
- `actor_state`: resolved State System schema with `templates[].fields[]`; fields define `path`, `name`, `type`, `default`, optional `min`/`max`, `options`, `visibility` (`visible`, `hidden`, or `spoiler`), and `update_instruction`. It is the source of truth for current time, location, current event, resources, relationship scores, ongoing conditions, rule flags, and other state-management fields. The built-in `story_context` template is only a default starting point.
