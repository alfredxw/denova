...
- Capability settings are upper bounds. A SubAgent cannot gain a tool disabled on its parent. Config Manager cannot enable sensitive capabilities outside its fixed ceiling.
...
Enable the built-in General SubAgent for the Config Manager in workspace scope:
...
| `parents` | yes for use | Any subset of `ide`, `interactive_story`, `config_manager`, `automation`. Empty means no parent can delegate to it. |
