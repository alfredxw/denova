...
Game Mode includes built-in Story Director orchestration. The Interactive Agent interprets player actions and produces the turn-level TurnBrief, while backend tools only perform deterministic work such as schema validation, numeric settlement, dice rolls, and expression-based rule checks. After each persisted turn, a background Director Agent maintains branch-scoped three-layer Markdown plans: mainline, current main event, and near-branch arrangements. Interactive prose reads only the visible sections of these docs, private director sections remain available only to the background director, and hot choices read only the visible section of the near-branch plan. Creators can compose or disable a director's narrative style, event system, rule system, opening selector, and image preset, and can configure branch planning turns plus the three planning templates. Event cards carry a type name, Markdown event description, trigger scene, payoff/recovery notes, and scheduling metadata as director planning inputs instead of a runtime event queue. The Config Manager Agent can inspect the lore index, read a bounded set of relevant entries, and generate world-grounded event card packs. Opening trait rolls, stat attributes, and reusable rule-check templates are also configurable.
