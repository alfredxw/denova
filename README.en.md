...
Game Mode includes built-in Story Director orchestration. The Interactive Agent interprets player actions and produces the turn-level TurnBrief, while backend tools only perform deterministic work such as schema validation, state settlement, and fixed-d20 rule checks. After each persisted turn, a background Director Agent maintains one branch-scoped `director.md` document. Its Prose-agent visible and Director private sections jointly cover stage hooks, lore anchors, important characters and factions, the current scene, clue density, checks and costs, crisis reversals, continuity, and near-branch arrangements. Interactive prose reads only bounded visible content from `director.md`, while private director sections remain available only to the background director. Each non-terminal TurnResult also persists 2–4 action suggestions for the player stage, without a separate Hot Choices Agent or follow-up model request. Creators can compose or disable a director's narrative style, event packages, TRPG Checks, State System, Story Memory Structure, and image preset, and can configure branch planning turns, the single planning template, and `off / sparse / balanced / frequent` event-opportunity cadence. Event packages are explicitly selected narrative libraries rather than random queues: the backend deterministically creates evaluation opportunities from committed turns, and the Director may decline or audit an event being seeded, advanced, paid off, resolved, or abandoned. Active events live in branch-scoped Director metadata and remain safe across retries, branches, and rewinds. When a new opportunity is due, the Director receives only a compact card index and reads card bodies on demand. Director planning prioritizes important characters, factions, rules, and locations from the lore library and only invents temporary additions when lore is insufficient. Every playable turn should advance meaningful information, relationship change, pressure, reward/cost, or a new hook so the interaction keeps a high-density web-novel-like pace. The State System defines Actor field schemas, defaults, bounds, visibility, initial Actors, reusable trait pools, and template draw rules. By default, a creation-time initialization Director adapts preset templates, fields, and initial Actors from bounded story setup, then the backend validates TRPG references and freezes the result as the story-specific schema; this step can be disabled in Story Director settings to use the preset as-is. Protagonists, important characters, enemies, and monsters receive persisted trait-definition snapshots through the same creation flow. TRPG Checks provide reusable fixed-d20 rule templates and can use State Binding to read state fields for modifiers, while Story Memory Structure defines long-term narrative schemas. The prose Agent receives only a sourced, visibility-filtered current Actor state summary capped at 64KB; the complete trait library is not injected every turn.
