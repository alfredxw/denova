...
func (s *ChatService) RuntimeStatusProjection(ctx context.Context, options RunOptions) (agentruntime.StatusSnapshot, error) {
	...
	if err != nil {
		return agentruntime.StatusSnapshot{}, fmt.Errorf("derive agent runtime projection binding: %w", err)
	}
	status, err := s.harness.runtime.Project(ctx, binding)
	if err != nil {
		return agentruntime.StatusSnapshot{}, fmt.Errorf("project agent runtime status: %w", err)
	}
	return status, nil
}
...
func (s *ChatService) CloseRuntimeBindings(ctx context.Context, selector agentruntime.BindingSelector) error {
	if s == nil || s.harness == nil || s.harness.runtime == nil {
