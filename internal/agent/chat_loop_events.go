...
func (l *chatAgentLoop) handleToolOutput(messageOutput *adk.MessageVariant, eventMeta agentEventMetadata) chatLoopResult {
	run := l.run
	...
	if drainErr != nil {
		if errors.Is(drainErr, adk.ErrStreamCanceled) && run.control.hasTriggeredControl() {
			return chatLoopResult{action: chatLoopContinue}
		}
...
func (l *chatAgentLoop) handleAssistantOutput(messageOutput *adk.MessageVariant, eventMeta agentEventMetadata) chatLoopResult {
	...
	if messageOutput.IsStreaming && messageOutput.MessageStream != nil {
		...
		if streamErr != nil {
			...
			if errors.Is(streamErr, adk.ErrStreamCanceled) && run.control.hasTriggeredControl() {
				return chatLoopResult{action: chatLoopContinue}
			}
