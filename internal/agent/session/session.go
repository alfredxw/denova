...
// Append 追加消息并持久化到磁盘。
func (s *Session) Append(msg *adk.Message) error {
	return s.AppendWithMetadata(msg, MessageMetadata{})
}

func (s *Session) AppendWithMetadata(msg *adk.Message, metadata MessageMetadata) error {
...
func (s *Session) appendMessageLocked(msg *adk.Message, metadata MessageMetadata, kind string) error {
	if msg == nil {
	...
	if s.title == defaultSessionTitle && msg.Role == adk.User && strings.TrimSpace(msg.Content) != "" {
...
func (s *Session) AppendContextMessage(msg *adk.Message) error {
...
func (s *Session) GetMessages() []*adk.Message {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Session) GetEffectiveMessages() []*adk.Message {
	s.mu.Lock()
	defer s.mu.Unlock()

	result := make([]*adk.Message, len(s.messages)-s.clearAfterIndex)
