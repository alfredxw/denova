...
func TestADKMessageWireRoundTripsAllStableFields(t *testing.T) {
	dir := t.TempDir()
	...
	want := &adk.Message{
		...
		Name:                     "writer",
		ToolCalls:                []adk.ToolCall{{Index: &callIndex, ID: "call-1", Type: "function", Function: adk.FunctionCall{Name: "read_file", Arguments: `{"path":"chapter.md"}`}, Extra: map[string]any{"provider": "test"}}},
		...
		ResponseMeta: &adk.ResponseMeta{
			...
			Usage:        &adk.TokenUsage{PromptTokens: 10, PromptTokenDetails: adk.PromptTokenDetails{CachedTokens: 4}, CompletionTokens: 5, TotalTokens: 15, CompletionTokensDetails: adk.CompletionTokensDetails{ReasoningTokens: 2}},
			LogProbs:     &adk.LogProbs{Content: []adk.LogProb{{Token: "x", LogProb: -0.1, Bytes: []int64{120}, TopLogProbs: []adk.TopLogProb{{Token: "y", LogProb: -0.2}}}}},
		},
	}
	...
	if len(got) != 1 || !reflect.DeepEqual(got[0], want) {
		t.Fatalf("ADK message wire changed across reload:\n got=%#v\nwant=%#v", got, want)
	}
}
...
func TestClearMarkerKeepsHistoryAndLimitsEffectiveContext(t *testing.T) {
	...
	if err := sess.Append(adk.UserMessage("清理前用户")); err != nil {
		t.Fatal(err)
	}
	if err := sess.Append(adk.AssistantMessage("清理前助手", nil)); err != nil {
		t.Fatal(err)
	}
	if err := sess.Clear(); err != nil {
		t.Fatal(err)
	}
	if err := sess.Append(adk.UserMessage("清理后用户")); err != nil {
		t.Fatal(err)
	}
...
func TestAssistantMessageMetadataPersistsRunID(t *testing.T) {
	...
	if err := sess.Append(adk.UserMessage("写一段")); err != nil {
		t.Fatal(err)
	}
	if err := sess.AppendWithMetadata(adk.AssistantMessage("已完成", nil), MessageMetadata{
		RunID:         "run-1",
		AgentKind:     "ide",
...
func TestUserMessageReferencesPersistAcrossSessionReload(t *testing.T) {
	...
	if err := sess.AppendWithMetadata(adk.UserMessage("请修改"), MessageMetadata{UserReferences: []UserMessageReference{
...
func TestDisplayEventsPersistOutsideEffectiveContext(t *testing.T) {
	...
	if err := sess.Append(adk.UserMessage("帮我规划下一章")); err != nil {
		t.Fatal(err)
	}
	...
	if err := sess.Append(adk.AssistantMessage("规划完成", nil)); err != nil {
		t.Fatal(err)
	}
...
func TestContextMessagesPersistInEffectiveContextButNotHistory(t *testing.T) {
	...
	if err := sess.Append(adk.UserMessage("读取第一章")); err != nil {
		t.Fatal(err)
	}
	...
	if err := sess.AppendContextMessage(adk.ToolMessage("第一章内容", "call-read", adk.WithToolName("read_file"))); err != nil {
		t.Fatal(err)
	}
	if err := sess.Append(adk.AssistantMessage("已读取", nil)); err != nil {
		t.Fatal(err)
	}
	...
	if effective[1].Role != adk.Assistant || len(effective[1].ToolCalls) != 1 || effective[2].Role != adk.Tool || effective[2].Content != "第一章内容" {
...
func TestSubAgentAssistantDisplayChunksPersistOutsideEffectiveContext(t *testing.T) {
	...
	if err := sess.Append(adk.UserMessage("委派调研")); err != nil {
		t.Fatal(err)
	}
	if err := sess.AppendDisplayEvent(DisplayEvent{
...
func TestTokenUsageDisplayEventPersistsOutsideEffectiveContext(t *testing.T) {
	...
	if err := sess.Append(adk.UserMessage("统计一下")); err != nil {
		t.Fatal(err)
	}
	if err := sess.AppendDisplayEvent(DisplayEvent{
		ID:                   "run-1",
	...
	if err := sess.Append(adk.AssistantMessage("统计完成", nil)); err != nil {
		t.Fatal(err)
	}
...
func TestContextCompactionPersistsOutsideVisibleHistory(t *testing.T) {
	...
	if err := sess.Append(adk.UserMessage("第一轮")); err != nil {
		t.Fatal(err)
	}
	if err := sess.Append(adk.AssistantMessage("第一答", nil)); err != nil {
		t.Fatal(err)
	}
...
func TestMultipleSessionsAreIsolatedAndActiveSessionPersists(t *testing.T) {
	...
	if err := first.Append(adk.UserMessage("会话 A")); err != nil {
		t.Fatal(err)
	}
	...
	if err := second.Append(adk.UserMessage("会话 B")); err != nil {
		t.Fatal(err)
	}
...
func TestListAndDeleteByPrefixForInteractiveSessions(t *testing.T) {
	...
	if err := matching.Append(adk.UserMessage("互动故事")); err != nil {
		t.Fatal(err)
	}
