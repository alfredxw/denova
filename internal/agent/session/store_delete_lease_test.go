...
func TestDeletedSessionHandleCannotAppendIntoRecreatedJournal(t *testing.T) {
	...
	if err := stale.Append(adk.UserMessage("old journal")); err != nil {
		t.Fatal(err)
	}
	...
	if err := recreated.Append(adk.UserMessage("new journal")); err != nil {
		t.Fatal(err)
	}

	appendErr := stale.Append(adk.UserMessage("must not cross incarnation"))
