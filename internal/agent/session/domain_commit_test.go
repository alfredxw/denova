...
func TestCommitDomainMessageIsIdempotentAndPersistsCoordinatorIdentity(t *testing.T) {
	...
	intent, err := NewDomainCommitIntent(
		...
		adk.AssistantMessage("canonical answer", nil),
...
func TestCommitDomainMessageRejectsIdentityReuseWithDifferentPayload(t *testing.T) {
	...
	first, _ := NewDomainCommitIntent(identity, adk.AssistantMessage("first", nil), MessageMetadata{})
	second, _ := NewDomainCommitIntent(identity, adk.AssistantMessage("different", nil), MessageMetadata{})
...
func TestCommitDomainMessageRejectsSameContentWithDifferentSemanticMetadata(t *testing.T) {
	...
	identity := DomainCommitIdentity{CommandID: "command-1", OperationID: "operation-1", Cycle: 1}
	first, err := NewDomainCommitIntent(identity, adk.UserMessage("same content"), MessageMetadata{UserReferences: []UserMessageReference{{Kind: "file", Label: "a.md"}}})
	if err != nil {
		t.Fatal(err)
	}
	second, err := NewDomainCommitIntent(identity, adk.UserMessage("same content"), MessageMetadata{UserReferences: []UserMessageReference{{Kind: "file", Label: "b.md"}}})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := sess.CommitDomainMessage(first); err != nil {
...
func TestContextCursorRejectsStaleStructuralMutation(t *testing.T) {
	...
	if err := sess.Append(adk.UserMessage("new turn")); err != nil {
		t.Fatal(err)
	}
