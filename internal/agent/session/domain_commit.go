...
func NewDomainCommitIntent(identity DomainCommitIdentity, message *adk.Message, metadata MessageMetadata) (DomainCommitIntent, error) {
...
func (s *Session) FindDomainCommit(identity DomainCommitIdentity, role adk.RoleType, hash string) (_ DomainCommitReceipt, _ bool, resultErr error) {
	if s == nil {
	...
	if role != adk.User && role != adk.Assistant {
...
func (s *Session) GetEffectiveMessagesWithCursor() ([]*adk.Message, ContextCursor) {
	if s == nil {
		return nil, ContextCursor{}
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	messages := make([]*adk.Message, len(s.messages)-s.clearAfterIndex)
	copy(messages, s.messages[s.clearAfterIndex:])
	return messages, s.contextCursorLocked()
}
...
func (s *Session) snapshotContextLocked(agentKind string) ContextSnapshot {
	messages := make([]*adk.Message, len(s.messages)-s.clearAfterIndex)
	copy(messages, s.messages[s.clearAfterIndex:])
	snapshot := ContextSnapshot{EffectiveMessages: messages, Cursor: s.contextCursorLocked()}
...
func (s *Session) AppendContextMessageAt(expected ContextCursor, msg *adk.Message) error {
...
func domainMessageHash(message adk.Message, metadata MessageMetadata) (string, error) {
	metadata = sanitizeMessageMetadata(metadata)
	payload := struct {
		Message  adk.Message `json:"message"`
		Metadata struct {
...
func deterministicDomainMessageID(identity DomainCommitIdentity, role adk.RoleType) string {
...
func (s *Session) findDomainCommitLocked(identity DomainCommitIdentity, role adk.RoleType, hash string) (DomainCommitReceipt, bool, error) {
