...
func TestLegacyJournalAppendPreservesExistingBytesAndReloads(t *testing.T) {
	...
	if err := sess.Append(adk.AssistantMessage("新回答", nil)); err != nil {
		t.Fatal(err)
	}
...
func TestLoaderIgnoresOnlyIncompleteTailAndNextAppendRepairsIt(t *testing.T) {
	...
	if err := sess.Append(adk.UserMessage("完整消息")); err != nil {
		t.Fatal(err)
	}
	path := filepath.Join(dir, "default.jsonl")
	...
	if err := recovered.Append(adk.AssistantMessage("恢复后消息", nil)); err != nil {
		t.Fatal(err)
	}
...
func TestLoaderAcceptsCompleteFinalRecordWithoutNewline(t *testing.T) {
	...
	if err := sess.Append(adk.AssistantMessage("下一条", nil)); err != nil {
		t.Fatal(err)
	}
...
func TestFailedAppendDoesNotMutateSessionMemory(t *testing.T) {
	...
	if err := sess.Append(adk.UserMessage("不能进入内存")); err == nil {
		restore()
