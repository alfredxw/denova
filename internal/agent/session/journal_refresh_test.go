...
func TestRefreshCanonicalReplaysOnlyAppendedJournalTail(t *testing.T) {
	...
	if err := writer.Append(adk.UserMessage("first")); err != nil {
		t.Fatal(err)
	}
	...
	if err := writer.Append(adk.AssistantMessage("second", nil)); err != nil {
		t.Fatal(err)
	}
	if err := writer.Append(adk.UserMessage("third")); err != nil {
		t.Fatal(err)
	}
