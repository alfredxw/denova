...
func (s *runControlState) request(control RunControl, cancel adk.AgentCancelFunc) {
	if s == nil || cancel == nil {
		return
	}
	...
	var options []adk.AgentCancelOption
	switch control.Kind {
	case RunControlPreempt:
		options = append(options, adk.WithAgentCancelMode(adk.CancelAfterChatModel|adk.CancelAfterToolCalls))
...
func (s *runControlState) wrapProtocolCancel(cancel adk.AgentCancelFunc) adk.AgentCancelFunc {
	if s == nil || cancel == nil {
		return cancel
	}
	return func(options ...adk.AgentCancelOption) (*adk.CancelHandle, bool) {
		s.mu.Lock()
		defer s.mu.Unlock()
...
func (s *runControlState) suppressesStreamCanceledError(event Event) bool {
	if s == nil || event.Type != "error" || eventDataString(event.Data, "message") != adk.ErrStreamCanceled.Error() {
		return false
	}
	s.mu.Lock()
	defer s.mu.Unlock()
...
func startRunControlWatcher(ctx context.Context, controls <-chan RunControl, cancel adk.AgentCancelFunc, state *runControlState) <-chan struct{} {
