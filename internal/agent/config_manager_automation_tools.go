...
func newListAutomationsTool(novaDir, workspace string, workspaces []string) (adk.BaseTool, error) {
	return adk.InferTool("list_automations", "列出用户的全局自动化任务索引，按显式执行目标返回 catalog_id、名称、启用状态、模板、触发器和写入策略；需要完整配置时再调用 read_automations。", func(ctx context.Context, input struct{}) (string, error) {
		_ = ctx
		_ = input
...
func newReadAutomationsTool(novaDir, workspace string, workspaces []string) (adk.BaseTool, error) {
	return adk.InferTool("read_automations", "按自动化任务 catalog_id 批量读取完整任务配置。", func(ctx context.Context, input idListInput) (string, error) {
		_ = ctx
...
func newWriteAutomationsTool(novaDir, workspace string, workspaces []string) (adk.BaseTool, error) {
	return adk.InferTool("write_automations", "批量创建、更新或删除用户自动化任务；create 必须显式指定 target，update 必须携带 read_automations 返回的 revision，update/delete 使用 catalog_id。删除必须来自用户明确指令。", func(ctx context.Context, input automationWriteInput) (string, error) {
		_ = ctx
		store := configManagerAutomationStore(novaDir, workspace, workspaces)
