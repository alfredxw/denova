...
func TestSingleInstructionConversationPrependsBoundedStableContext(t *testing.T) {
	...
	result, err := conversation.AssembleModelContext(context.Background(), "", ModelContextInput{
...
func TestSingleInstructionConversationRejectsOversizedStableContext(t *testing.T) {
	...
	if _, err := conversation.AssembleModelContext(context.Background(), "", ModelContextInput{UserMessage: "dynamic", Budget: conversation.ModelContextBudget()}); err == nil || !strings.Contains(err.Error(), "超过上限") {
...
func TestSingleInstructionConversationCapsFinalStableMessageAndTitle(t *testing.T) {
	...
	if _, err := conversation.AssembleModelContext(context.Background(), "", ModelContextInput{UserMessage: "dynamic", Budget: conversation.ModelContextBudget()}); err == nil || !strings.Contains(err.Error(), "最终消息超过上限") {
	...
	if _, err := conversation.AssembleModelContext(context.Background(), "", ModelContextInput{UserMessage: "dynamic", Budget: conversation.ModelContextBudget()}); err == nil || !strings.Contains(err.Error(), "标题超过上限") {
