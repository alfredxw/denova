...
// catalog to the ADK. The catalog summary is part of the stable tool
// description; full instructions are loaded only after an explicit call.
func newSkillTool(ctx context.Context, backend *novaskills.Backend, maxBytes int) (adk.BaseTool, error) {
	if backend == nil {
		return nil, nil
	}
	...
	return adk.InferTool("skill", strings.TrimSpace(description.String()), func(callCtx context.Context, input skillToolInput) (string, error) {
...
func newWriteTodosTool() (adk.BaseTool, error) {
	return adk.InferTool("write_todos", "Replace the current run's complete todo list. Send every item on each update; status must be pending, in_progress, or completed.", func(_ context.Context, input writeTodosInput) (string, error) {
...
func newTaskTool(ctx context.Context, subAgents []adk.Runnable) (adk.BaseTool, error) {
	byName := make(map[string]adk.Runnable, len(subAgents))
	var description strings.Builder
	...
	return adk.InferTool("task", strings.TrimSpace(description.String()), func(callCtx context.Context, input taskToolInput) (string, error) {
...
func runSubAgent(ctx context.Context, child adk.Runnable, request string) (string, error) {
	events := child.Run(ctx, &adk.AgentInput{
		Messages:        []*adk.Message{adk.UserMessage(request)},
		EnableStreaming: true,
	})
	var final *adk.Message
	for {
		...
		forwarded := *event
		forwarded.RunPath = append([]adk.RunStep(nil), event.RunPath...)
		...
		adk.EmitEvent(ctx, &forwarded)
		if message != nil && message.Role == adk.Assistant {
			final = message
		}
	}
	if final == nil {
