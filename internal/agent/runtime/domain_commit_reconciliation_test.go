...
func TestRecoveryReconcilesCanonicalDomainCommitBeforeSettling(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			binding := runstate.WritingBinding{Workspace: "/book", SessionID: "reconcile-" + string(test.stage)}
...
func TestRecoveryInterruptsWhenCanonicalCommitIsNotFound(t *testing.T) {
	...
	store := runstate.NewMemoryJournalStore()
	binding := runstate.WritingBinding{Workspace: "/book", SessionID: "reconcile-not-found"}
...
func TestRecoveryKeepsInputReceiptButPreservesAcceptedAbort(t *testing.T) {
	...
	store := runstate.NewMemoryJournalStore()
	binding := runstate.WritingBinding{Workspace: "/book", SessionID: "reconcile-input-abort"}
...
func TestRecoveryQueryErrorLeavesOperationRetryable(t *testing.T) {
	...
	store := runstate.NewMemoryJournalStore()
	binding := runstate.WritingBinding{Workspace: "/book", SessionID: "reconcile-error"}
...
func TestFileJournalPersistsReconciledReceiptAcrossReopen(t *testing.T) {
	...
	binding := runstate.WritingBinding{Workspace: "/book", SessionID: "file-reconcile"}
...
func TestFileJournalPersistsAuthoritativeNotFoundAbandonmentAcrossAbortAndReopen(t *testing.T) {
	...
	binding := runstate.WritingBinding{Workspace: "/book", SessionID: "file-not-found-abandonment"}
...
func appendAbortToPendingDomainCommit(t *testing.T, store runstate.JournalStore, binding runstate.Binding, reason string) {
	t.Helper()
	ref, err := runstate.BindingReference(binding)
	if err != nil {
		t.Fatal(err)
