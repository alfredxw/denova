...
func TestAcceptedAbortDominatesLateEngineCompletion(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), runstate.WritingBinding{Workspace: "/book", SessionID: "abort-dominates"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCloseBindingDominatesLateEngineCompletion(t *testing.T) {
	...
	binding := runstate.WritingBinding{Workspace: "/book", SessionID: "close-dominates"}
