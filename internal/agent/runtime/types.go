...
// BindingKind is deliberately closed so runtime identity cannot silently drift
// when more product modes are added.
type BindingKind string

const (
	BindingWriting    BindingKind = "writing"
	BindingGame       BindingKind = "game"
	BindingAutomation BindingKind = "automation"
)

// AgentProfile selects the binding-specific model, prompt, context, and tool
// policy behind an EngineFactory. BindingKind owns product lifecycle semantics;
// AgentProfile owns the implementation policy used inside that lifecycle.
type AgentProfile string

const (
	ProfileWriting       AgentProfile = "writing"
	ProfileGame          AgentProfile = "game"
	ProfileAutomation    AgentProfile = "automation"
	ProfileConfigManager AgentProfile = "config_manager"
	ProfileImage         AgentProfile = "image"
	ProfileDirector      AgentProfile = "director"
)

// BindingRef is the stable, engine-facing identity of a harness.
...
func (selector BindingSelector) matches(ref BindingRef) bool {
	return (selector.Kind == "" || selector.Kind == ref.Kind) &&
		(selector.Profile == "" || selector.Profile == ref.Profile) &&
		(selector.Workspace == "" || selector.Workspace == ref.Workspace) &&
		(selector.SessionID == "" || selector.SessionID == ref.SessionID) &&
		(selector.StoryID == "" || selector.StoryID == ref.StoryID) &&
		(selector.BranchID == "" || selector.BranchID == ref.BranchID) &&
		(selector.TaskID == "" || selector.TaskID == ref.TaskID)
}
...
func (WritingBinding) binding() {}

func (b WritingBinding) bindingRef() BindingRef {
	return BindingRef{
		Kind: BindingWriting, Profile: defaultProfile(b.Profile, ProfileWriting), Workspace: b.Workspace, SessionID: b.SessionID,
		StoryID: b.StoryID, BranchID: b.BranchID,
	}
}
...
func (GameBinding) binding() {}

func (b GameBinding) bindingRef() BindingRef {
	return BindingRef{
		Kind: BindingGame, Profile: defaultProfile(b.Profile, ProfileGame), Workspace: b.Workspace, SessionID: b.SessionID,
		StoryID: b.StoryID, BranchID: b.BranchID,
	}
}
...
func (AutomationBinding) binding() {}

func (b AutomationBinding) bindingRef() BindingRef {
	return BindingRef{
		Kind: BindingAutomation, Profile: defaultProfile(b.Profile, ProfileAutomation), Workspace: b.Workspace, SessionID: b.SessionID,
		TaskID: b.TaskID,
	}
}

func defaultProfile(profile, fallback AgentProfile) AgentProfile {
	if profile == "" {
		return fallback
	}
	return profile
...
// DomainCommitState is the bounded recovery receipt for the canonical Session
// or Story write performed outside the control journal. An empty Revision means
