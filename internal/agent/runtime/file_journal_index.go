...
const fileJournalCommandIndexVersion = 1

type fileJournalCommandIndexEntry struct {
...
// fileJournalCommandIndexBody is an append-only index checkpoint. A compact
...
type fileJournalCommandIndexBody struct {
	Version               int                            `json:"version"`
	Snapshot              bool                           `json:"snapshot,omitempty"`
	PreviousJournalCursor Cursor                         `json:"previous_journal_cursor"`
	JournalBytes          int64                          `json:"journal_bytes"`
	JournalModifiedNS     int64                          `json:"journal_modified_ns"`
	JournalCursor         Cursor                         `json:"journal_cursor"`
	TailRecordSHA256      string                         `json:"tail_record_sha256,omitempty"`
	Commands              []fileJournalCommandIndexEntry `json:"commands,omitempty"`
}

type fileJournalCommandIndex struct {
	fileJournalCommandIndexBody
	Checksum string `json:"checksum"`
}

func (j *fileJournal) commandIndexPath() string {
	return j.path + ".commands.jsonl"
}

func (j *fileJournal) indexCommittedCommands(events []Event) {
	...
	for _, event := range events {
		...
		j.commandIndex[accepted.CommandID] = CommandRecord{
			Receipt: Receipt{
...
func (j *fileJournal) rewriteCommandIndexLocked() error {
	...
	body := fileJournalCommandIndexBody{
		Version: fileJournalCommandIndexVersion, Snapshot: true,
...
func (j *fileJournal) appendCommandIndexLocked(previous Cursor, events []Event) error {
	info, err := os.Stat(j.tailPath)
	if err != nil {
		return fmt.Errorf("stat command index journal: %w", err)
	}
	...
	body := fileJournalCommandIndexBody{
		Version:               fileJournalCommandIndexVersion,
...
func commandIndexEntries(commands map[CommandID]CommandRecord) []fileJournalCommandIndexEntry {
	entries := make([]fileJournalCommandIndexEntry, 0, len(commands))
	for commandID, record := range commands {
		entries = append(entries, fileJournalCommandIndexEntry{
...
func commandIndexEntriesFromEvents(events []Event) []fileJournalCommandIndexEntry {
	entries := make([]fileJournalCommandIndexEntry, 0, 1)
	for _, event := range events {
		accepted, ok := event.Payload.(CommandAcceptedEvent)
		if !ok {
			continue
...
func encodeCommandIndexRecord(body fileJournalCommandIndexBody) ([]byte, error) {
	bodyJSON, err := json.Marshal(body)
	if err != nil {
		return nil, fmt.Errorf("encode command index checksum body: %w", err)
	}
	digest := sha256.Sum256(bodyJSON)
	encoded, err := json.Marshal(fileJournalCommandIndex{
		fileJournalCommandIndexBody: body,
...
func (j *fileJournal) loadPersistedCommandIndexLocked() (bool, error) {
	...
	var last fileJournalCommandIndexBody
	lineNumber := 0
	for {
		...
		var record fileJournalCommandIndex
		if err := json.Unmarshal(line, &record); err != nil {
			_ = file.Close()
			return false, fmt.Errorf("decode command index line %d: %w", lineNumber, err)
		}
		bodyJSON, err := json.Marshal(record.fileJournalCommandIndexBody)
		if err != nil {
			_ = file.Close()
			return false, err
		}
		digest := sha256.Sum256(bodyJSON)
		if record.Version != fileJournalCommandIndexVersion || record.Checksum != hex.EncodeToString(digest[:]) {
			_ = file.Close()
		...
		for _, entry := range record.Commands {
			...
			commands[entry.CommandID] = CommandRecord{
				Receipt:     Receipt{CommandID: entry.CommandID, OperationID: entry.OperationID, Cursor: entry.Cursor},
				Fingerprint: entry.Fingerprint,
			}
		}
		last = record.fileJournalCommandIndexBody
