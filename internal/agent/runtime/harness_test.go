...
func TestStartTurnIsDurableBeforeReceiptAndReplaysAfterOpen(t *testing.T) {
	...
	binding := runstate.WritingBinding{Workspace: "/book", SessionID: "session-1"}
...
func TestSubmitReturnsOriginalReceiptForIdempotentRetry(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), runstate.WritingBinding{Workspace: "/book", SessionID: "retry"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestBindingProfileIsExplicitAndValidated(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), runstate.WritingBinding{
		Workspace: "/book", SessionID: "config", Profile: runstate.ProfileConfigManager,
	})
	if err != nil {
	...
	if observation.Snapshot.Binding.Profile != runstate.ProfileConfigManager {
		t.Fatalf("profile = %q, want %q", observation.Snapshot.Binding.Profile, runstate.ProfileConfigManager)
	}
	if _, err := runtime.Open(context.Background(), runstate.WritingBinding{
		Workspace: "/book", SessionID: "bad", Profile: runstate.ProfileDirector,
	}); !errors.Is(err, runstate.ErrInvalidBinding) {
		t.Fatalf("open invalid profile error = %v, want ErrInvalidBinding", err)
	}
	if _, err := runtime.Open(context.Background(), runstate.GameBinding{
	...
	if _, err := runtime.Open(context.Background(), runstate.AutomationBinding{
...
func TestCommandsRejectStaleOperationTargets(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), runstate.WritingBinding{Workspace: "/book", SessionID: "stale-target"})
	if err != nil {
		t.Fatalf("open harness: %v", err)
	}
...
func TestCommandsEnforceConfiguredDurableInputEnvelope(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), runstate.WritingBinding{Workspace: "/book", SessionID: "bounded-input"})
	if err != nil {
		t.Fatalf("open harness: %v", err)
	}
...
func TestActiveOutputIsDisplayOnlyReconnectState(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), runstate.WritingBinding{Workspace: "/book", SessionID: "active-output"})
	if err != nil {
		t.Fatalf("open harness: %v", err)
	}
...
func TestRuntimeCloseDurablyAbortsAndRejectsNewOpen(t *testing.T) {
	...
	binding := runstate.WritingBinding{Workspace: "/book", SessionID: "runtime-close"}
...
func TestRuntimeOpenDoesNotSerializeDifferentBindings(t *testing.T) {
	...
	runExternalErrorTestGoroutine(slowDone, "slow independent binding open", func() error {
		_, err := runtime.Open(context.Background(), runstate.WritingBinding{Workspace: "/book", SessionID: "slow"})
		return err
	})
	<-factory.blocked
	if _, err := runtime.Open(context.Background(), runstate.WritingBinding{Workspace: "/book", SessionID: "fast"}); err != nil {
...
func TestObserveFromNowDoesNotReplayDisplayHistory(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), runstate.WritingBinding{Workspace: "/book", SessionID: "from-now"})
	if err != nil {
		t.Fatalf("open harness: %v", err)
	}
...
func TestSteerPreemptsIntoNextCycleOfTheSameOperation(t *testing.T) {
	...
	harness, err := runtime.Open(ctx, runstate.WritingBinding{Workspace: "/book", SessionID: "steer"})
	if err != nil {
		t.Fatalf("open harness: %v", err)
	}
...
func TestFollowUpWaitsForTheCurrentCycleToComplete(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), runstate.WritingBinding{Workspace: "/book", SessionID: "follow-up"})
	if err != nil {
		t.Fatalf("open harness: %v", err)
	}
...
func TestNextTurnRunsAsASeparateOperationAfterTheActiveOne(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), runstate.GameBinding{
...
func TestFileJournalReplaysAcrossStoreInstances(t *testing.T) {
	...
	binding := runstate.WritingBinding{Workspace: "/book", SessionID: "file-journal"}
	harness, err := runtime.Open(context.Background(), binding)
	if err != nil {
		t.Fatalf("open harness: %v", err)
	}
...
func TestRecoveryNeverRetriesAnUnfinishedToolEffect(t *testing.T) {
	...
	journals := runstate.NewMemoryJournalStore()
	binding := runstate.WritingBinding{Workspace: "/book", SessionID: "recover"}
