...
func newListSkillsTool(cfg *config.Config) (adk.BaseTool, error) {
	return adk.InferTool("list_skills", "列出 Skills 索引，返回名称、scope、agent、描述、是否可编辑和是否生效；需要完整 SKILL.md 时再调用 read_skills。", func(ctx context.Context, input struct{}) (string, error) {
		_ = input
...
func newReadSkillsTool(cfg *config.Config) (adk.BaseTool, error) {
	return adk.InferTool("read_skills", "按 scope/name 批量读取完整 SKILL.md。", func(ctx context.Context, input readSkillsInput) (string, error) {
...
func newWriteSkillsTool(cfg *config.Config) (adk.BaseTool, error) {
	return adk.InferTool("write_skills", "批量创建、更新或删除 Skills。scope 必须是 user 或 workspace；修改内置/预制 Skill 时使用 workspace 同名覆盖，禁止写 builtin；删除必须来自用户明确指令。", func(ctx context.Context, input skillsWriteInput) (string, error) {
