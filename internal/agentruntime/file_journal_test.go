...
func TestFileJournalAppendUsesLoadedCursorWithoutRescanning(t *testing.T) {
	...
	for cursor := Cursor(0); cursor < 8; cursor++ {
		appendTestJournalEvent(t, journal, cursor, CommandID(string(rune('a'+cursor))))
	}
	if reads != 1 {
...
func TestFileJournalReconcilesCommittedAppendAfterAmbiguousCloseError(t *testing.T) {
	...
	committed, err := journal.Append(context.Background(), 0, []EventPayload{CommandAcceptedEvent{
...
func TestFileJournalDoesNotMaskFileSyncFailureWithReadBack(t *testing.T) {
	...
	if _, err := journal.Append(context.Background(), 0, []EventPayload{CommandAcceptedEvent{
...
func newTestFileJournal(t *testing.T) *fileJournal {
	t.Helper()
	store, err := NewFileJournalStore(t.TempDir())
	if err != nil {
	...
	journal, ok := opened.(*fileJournal)
	if !ok {
		t.Fatalf("journal type = %T, want *fileJournal", opened)
	}
	t.Cleanup(func() { _ = journal.Close() })
	return journal
}

func appendTestJournalEvent(t *testing.T, journal *fileJournal, expected Cursor, id CommandID) {
	t.Helper()
	_, err := journal.Append(context.Background(), expected, []EventPayload{CommandAcceptedEvent{
		CommandID: id, CommandKind: "test", OperationID: OperationID(id), Fingerprint: string(id),
	}})
	if err != nil {
