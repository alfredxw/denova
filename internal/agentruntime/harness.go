...
func newHarness(
	...
	h := &Harness{
		binding: binding, engine: engine, journal: j,
...
func (h *Harness) closeOnLifecycleEnd() {
	defer func() {
		if recovered := recover(); recovered != nil {
			log.Printf("agentruntime: binding=%+v lifecycle watcher panic: %v", h.binding, recovered)
		}
	}()
	select {
...
func (h *Harness) run(state harnessState) {
	defer func() {
		...
		if recovered := recover(); recovered != nil {
			...
			log.Printf("agentruntime: binding=%+v cursor=%d actor failed: %v", h.binding, state.cursor, terminal)
