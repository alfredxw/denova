...
func TestFileJournalStreamingReplayReportsIOAndLetsReducerStayBounded(t *testing.T) {
	...
	for index := 0; index < records; index++ {
		...
		line := encodeTestJournalTransaction(t, cursor, CommandID("command-"+testDecimal(index)))
	...
	retained := make([]Event, 0, 3)
	maxRetained := 0
	stats, err := journal.Replay(context.Background(), func(event Event) error {
...
func TestFileJournalColdCommandLookupUsesValidatedPersistentIndex(t *testing.T) {
	...
	for cursor := Cursor(0); cursor < 12; cursor++ {
		appendTestJournalEvent(t, journal, cursor, CommandID("indexed-"+testDecimal(int(cursor))))
	}
	...
	reopened := reopenedRaw.(*fileJournal)
...
func TestFileJournalCommandIndexUsesAppendOnlyBoundedDeltas(t *testing.T) {
	...
	journal := newTestFileJournal(t)
	for cursor := Cursor(0); cursor < 8; cursor++ {
		appendTestJournalEvent(t, journal, cursor, CommandID("delta-"+testDecimal(int(cursor))))
	}
...
func encodeTestJournalTransaction(t *testing.T, cursor Cursor, commandID CommandID) []byte {
	t.Helper()
	event, err := encodeDurableEvent(Event{
		Cursor: cursor, Durability: EventDurable,
		Payload: CommandAcceptedEvent{
			CommandID: commandID, CommandKind: "test", OperationID: OperationID(commandID), Fingerprint: string(commandID),
		},
	})
	if err != nil {
		t.Fatal(err)
	}
	body := journalTransactionBody{Version: fileJournalVersion, Start: cursor, End: cursor, Events: []encodedEvent{event}}
