...
func TestRuntimeOpenHoldsMemoryBindingLeaseUntilClose(t *testing.T) {
	...
	binding := agentruntime.WritingBinding{Workspace: "/book", SessionID: "memory-lease"}
...
func TestRuntimeOpenHoldsFileBindingLeaseAcrossStores(t *testing.T) {
	...
	root := t.TempDir()
	firstStore, err := agentruntime.NewFileJournalStore(filepath.Join(root, "journals"))
	if err != nil {
		t.Fatal(err)
	}
	secondStore, err := agentruntime.NewFileJournalStore(filepath.Join(root, "journals"))
	if err != nil {
		t.Fatal(err)
	}
	first := newTestRuntime(t, firstStore)
	second := newTestRuntime(t, secondStore)
	binding := agentruntime.WritingBinding{Workspace: "/book", SessionID: "file-lease"}
...
func TestCloseBindingWaitsForPendingOpenAndPreventsOverlappingHarness(t *testing.T) {
	...
	binding := agentruntime.WritingBinding{Workspace: "/book", SessionID: "pending-close"}
...
func assertOpenWaitsForLease(t *testing.T, runtime *agentruntime.Runtime, binding agentruntime.Binding) {
	t.Helper()
...
func newTestRuntime(t *testing.T, store agentruntime.JournalStore) *agentruntime.Runtime {
	t.Helper()
	runtime, err := agentruntime.NewRuntime(agentruntime.NewScriptedEngine(), store, agentruntime.RuntimeConfig{})
	if err != nil {
		t.Fatal(err)
	}
	return runtime
