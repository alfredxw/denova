...
func (h *Harness) startStructuralEngine(state *harnessState, snapshot StructuralOperationSnapshot) {
	...
	request := StructuralEngineRequest{Binding: h.binding, Snapshot: snapshot, Controls: forwarded}
	go func() {
		returned := false
		defer func() {
			cancel()
			...
			if !returned {
				log.Printf("agentruntime: binding=%+v operation=%s structural engine exited without a result", h.binding, snapshot.OperationID)
			}
		}()
...
func forwardStructuralControls(
	...
	defer func() {
		if recovered := recover(); recovered != nil {
			log.Printf("agentruntime: binding=%+v operation=%s structural control bridge panic: %v", binding, operationID, recovered)
			cancel()
		}
	}()
	for {
