...
func (h *Harness) startEngine(state *harnessState, snapshot TurnSnapshot) {
	...
	request := EngineRequest{Binding: h.binding, Snapshot: snapshot, Controls: controls}
	go func() {
		returned := false
		defer func() {
			...
			if !returned {
				...
				log.Printf("agentruntime: binding=%+v operation=%s cycle=%d engine exited without a result", h.binding, snapshot.OperationID, snapshot.Cycle)
			}
		}()
...
func (h *Harness) startQueuedCycle(state *harnessState, item QueuedInput) {
	...
	if err := h.resumePendingInputMaterialization(h.lifecycle, state); err != nil {
		...
		log.Printf(
			"agentruntime: binding=%+v command=%s operation=%s cycle=%d accepted input remains pending: %v",
			h.binding,
			item.CommandID,
