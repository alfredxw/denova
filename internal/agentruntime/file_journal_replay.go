...
const fileJournalReadBufferBytes = 64 * 1024
...
// append).
func (j *fileJournal) replayLocked(
	ctx context.Context,
	repairTornTail bool,
	reduce func(Event) error,
) (JournalReplayStats, error) {
	...
	if errors.Is(err, os.ErrNotExist) {
		j.rememberReplay(0, false, "", map[CommandID]CommandRecord{}, JournalReplayStats{})
		return JournalReplayStats{}, nil
	}
	if err != nil {
		return JournalReplayStats{}, fmt.Errorf("open file journal for replay: %w", err)
	}
	...
	for {
		...
		for _, event := range transactionEvents {
			if accepted, ok := event.Payload.(CommandAcceptedEvent); ok {
				commands[accepted.CommandID] = CommandRecord{
					Receipt: Receipt{
...
func (j *fileJournal) rememberReplay(
	cursor Cursor,
	needsNewline bool,
	tailHash string,
...
func (j *fileJournal) backupAndRepairTornTail(validBytes int64) error {
