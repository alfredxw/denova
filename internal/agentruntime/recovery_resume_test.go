...
func TestAcceptedTransientInputWaitsForExactReplayAfterCrash(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			binding := WritingBinding{Workspace: "/book", SessionID: "recover-" + test.name}
...
func TestRecoveredTransientQueuePausesBeforeEachUnavailableInput(t *testing.T) {
	...
	store := NewMemoryJournalStore()
	binding := WritingBinding{Workspace: "/book", SessionID: "recover-multiple-transient-inputs"}
	ref, err := BindingReference(binding)
	if err != nil {
		t.Fatal(err)
	}
...
func TestStructuralRecoveryResumesOnlyOnExactCommandReplay(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			store := NewMemoryJournalStore()
			binding := WritingBinding{Workspace: "/book", SessionID: "structural-" + test.name}
			ref, err := BindingReference(binding)
			if err != nil {
				t.Fatal(err)
			}
...
func TestStructuralRecoverySettlesCanonicalCommitBeforeReplay(t *testing.T) {
	...
	store := NewMemoryJournalStore()
	binding := WritingBinding{Workspace: "/book", SessionID: "structural-canonical"}
	ref, err := BindingReference(binding)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCloseRecoveryPausedStructuralOperationSettlesWithoutEngineCallback(t *testing.T) {
	...
	store := NewMemoryJournalStore()
	binding := WritingBinding{Workspace: "/book", SessionID: "close-paused-structural"}
	ref, err := BindingReference(binding)
	if err != nil {
		t.Fatal(err)
	}
...
func TestStructuralRecoveryResumesIntentProvenAbsentFromCanonicalStore(t *testing.T) {
	...
	store := NewMemoryJournalStore()
	binding := WritingBinding{Workspace: "/book", SessionID: "structural-not-committed"}
	ref, err := BindingReference(binding)
	if err != nil {
		t.Fatal(err)
	}
