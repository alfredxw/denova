...
- write_lore_items：批量创建或更新资料库条目；只有作者明确要求删除时才传 delete_ids。写入时每个条目都要给出完整字段、brief_description 简介和正文，避免丢失已有设定；简介用于判断何时加载完整资料正文，应一两句概括适用场景、别名和关键设定
...
6. 只有写入 chapters/ 或作者明确确认定稿后，才更新 progress.md 和资料库；未定稿草稿不写入全书事实状态
7. 不更改 outline.md，大纲只作为写作方向参考
8. 只更新 progress.md，并使用 write_lore_items 批量同步资料库中有明确变化的条目；不过多更新，记录本章完成后的角色状态、关系、位置、能力、伤势、物品和世界事实变化
