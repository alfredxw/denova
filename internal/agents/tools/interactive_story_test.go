...
func TestInteractiveTurnToolsExposeOneStructuredSubmissionTool(t *testing.T) {
	...
	tools, err := newInteractiveTurnTools(InteractiveContext{
	...
	if info.Name != submitInteractiveTurnToolName || !strings.Contains(schemaText, `"state_changes"`) || !strings.Contains(schemaText, `"choices"`) {
