...
func isContextRewindMessage(msg *agent.Message) bool {
	return msg != nil && strings.HasPrefix(strings.TrimSpace(msg.Content), contextRewindSummaryPrefix)
}
...
func compactMessagesForModelThroughSource(
	...
	protectedPrefix, _ := splitProtectedModelPrefix(messages)
	...
	for index := prefixEnd; index < len(messages); index++ {
		...
		if message == nil || isContextCompactionMessage(message) {
			continue
		}
	...
	summaryMessage := NewContextCompactionSummaryMessage(epoch, payload)
	...
	for index, message := range messages {
		...
		candidates = append(candidates, candidate{
			...
			receipt: receipt{
				...
				SanitizedArguments: boundedCompactionReceiptField(sanitizedArguments, protectedReceiptArgumentsBytes),
				OutcomeReceipt:     boundedCompactionReceiptField(outcomeReceipt, protectedReceiptOutcomeBytes),
...
// splitProtectedModelPrefix keeps the native system/developer prefix and every
// contiguous assembler-owned leading message byte-for-byte and in order.
func splitProtectedModelPrefix(messages []*agent.Message) ([]*agent.Message, []*agent.Message) {
	boundary := 0
	for boundary < len(messages) {
		...
		placement, _ := message.Extra[agentcontext.MessageExtraPlacement].(string)
		if placement == string(agentcontext.PlacementLeadingMessage) {
			boundary++
			continue
		}
		break
...
// PreserveModelContextLeadingMessage restores one deterministic leading
// fragment after system/developer messages without reordering an existing
// protected prefix or duplicating the fragment.
func PreserveModelContextLeadingMessage(messages []*agent.Message, content string) []*agent.Message {
...
// compactedMessagesAfterRewindSource applies a newer checkpoint over the
...
func compactedMessagesAfterRewindSource(
	messages []*agent.Message,
	...
	rewoundSource := applyContextWindowProjection(messages[:sourceEndOffset], effectiveStart, projection)
...
// BuildCompactedModelMessagesThroughSource applies the durable compaction
...
func BuildCompactedModelMessagesThroughSource(
	messages []*agent.Message,
	summary, existingCheckpoint string,
	epoch, retainedTurns, sourceEnd int,
) ([]*agent.Message, string) {
