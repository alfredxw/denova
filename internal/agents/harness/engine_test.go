...
func TestHarnessEngineRunCompletesAndConsumesTurnSpec(t *testing.T) {
	...
	err := registerAcceptedHarnessTurn(engine, "turn-complete", TurnSpec{
...
func TestHarnessEngineFactoryRejectsMismatchedBindingBeforeConsumingTurnSpec(t *testing.T) {
	owner := newHarnessEngine(newTestExecutor(agentrun.DefaultLoopPolicy()))
	if err := registerAcceptedHarnessTurn(owner, "turn-binding-mismatch", TurnSpec{
...
func TestHarnessEngineRejectsTurnSpecWhoseProfileDoesNotMatchBinding(t *testing.T) {
	...
	if err := registerAcceptedHarnessTurn(owner, "turn-profile-mismatch", TurnSpec{
	...
	if _, err := bound.Run(context.Background(), harnessEngineRequest("turn-profile-mismatch", nil), func(runstate.EngineEvent) error { return nil }); !errors.Is(err, ErrBindingMismatch) {
...
func TestHarnessEngineRejectsIncompleteExecutableBeforeModelEffects(t *testing.T) {
	owner := newHarnessEngine(newTestExecutor(agentrun.DefaultLoopPolicy()))
	if err := registerAcceptedHarnessTurn(owner, "turn-incomplete", TurnSpec{
		Request: agentchat.ChatRequest{Message: "write"},
		Options: testHarnessRunOptions(),
	}); err != nil {
		t.Fatal(err)
...
func TestHarnessEngineReportsLegacyOutcome(t *testing.T) {
	...
	if err := registerAcceptedHarnessTurn(engine, "turn-outcome", TurnSpec{
...
func TestHarnessEngineMaterializesQueuedTurnAtExecutionTime(t *testing.T) {
	engine := newHarnessEngine(newTestExecutor(agentrun.DefaultLoopPolicy()))
	prepared := false
	if err := registerAcceptedHarnessTurn(engine, "turn-lazy", TurnSpec{
		Request: agentchat.ChatRequest{Message: "queued request"},
		Options: agentrun.Options{AgentKind: agentrun.AgentKindIDE, RootAgentName: "run-control-test"},
		Prepare: func(context.Context) (TurnExecution, error) {
			prepared = true
...
func TestHarnessEngineReleasesCancelledPendingTurnSpec(t *testing.T) {
	engine := newHarnessEngine(newTestExecutor(agentrun.DefaultLoopPolicy()))
	if err := registerAcceptedHarnessTurn(engine, "queued-turn", TurnSpec{}); err != nil {
		t.Fatal(err)
	}
...
func TestHarnessEngineBindsDurableCycleIdentityBeforeExecution(t *testing.T) {
	...
	if err := registerAcceptedHarnessTurn(engine, "turn-identity", TurnSpec{
...
func TestHarnessEnginePreparesCycleAfterIdentityAndBeforeRuntime(t *testing.T) {
	...
	if err := registerAcceptedHarnessTurn(engine, "turn-prepare-order", TurnSpec{
...
func TestHarnessEnginePreparationFailureStopsCycleBeforeRuntime(t *testing.T) {
	...
	outcomes := make(chan agentrun.Outcome, 1)
	if err := registerAcceptedHarnessTurn(engine, "turn-prepare-error", TurnSpec{
...
func TestHarnessEngineControlInterruptsBlockingPreparation(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err := registerAcceptedHarnessTurn(engine, ref, TurnSpec{
...
func TestHarnessEngineBridgesPreemptControl(t *testing.T) {
	...
	engine := newHarnessEngine(newTestExecutor(agentrun.DefaultLoopPolicy()))
	if err := registerAcceptedHarnessTurn(engine, "turn-preempt", TurnSpec{
...
func TestHarnessEngineRejectsMissingOrConflictingTurnSpec(t *testing.T) {
	engine := newHarnessEngine(newTestExecutor(agentrun.DefaultLoopPolicy()))
	if err := registerAcceptedHarnessTurn(engine, "", TurnSpec{}); !errors.Is(err, ErrHarnessTurnSpecRefRequired) {
	...
	firstLease, err := engine.register("duplicate", first, TurnSpec{Request: agentchat.ChatRequest{Message: "same"}})
	if err != nil {
		t.Fatalf("first registration: %v", err)
	}
	defer firstLease.release()
	retryLease, err := engine.register("duplicate", first, TurnSpec{Request: agentchat.ChatRequest{Message: "same"}})
	if err != nil {
	...
	if _, err := engine.register("duplicate", conflict, TurnSpec{}); !errors.Is(err, ErrHarnessTurnSpecConflict) || !errors.Is(err, runstate.ErrInvalidCommand) {
...
func TestHarnessEngineReturnsSinkError(t *testing.T) {
	engine := newHarnessEngine(newTestExecutor(agentrun.DefaultLoopPolicy()))
	if err := registerAcceptedHarnessTurn(engine, "turn-sink-error", TurnSpec{
...
func TestHarnessEngineMapsDescriptorRecoveryToRetrySafety(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := harnessToolRetrySafety(agenttool.ExecutionRecord{ToolName: tt.name, Descriptor: agent.ToolDescriptor{Recovery: tt.recovery}}); got != tt.want {
...
func TestHarnessEngineConcurrentRegistrationLeasesPreserveAcceptedSpec(t *testing.T) {
	...
	for index := 0; index < contenders; index++ {
		runErrorTestGoroutine(errs, "concurrent turn registration", func() error {
			lease, err := engine.register("shared", command, TurnSpec{Request: agentchat.ChatRequest{Message: "same input"}})
			if err == nil {
				leases <- lease
			}
			return err
...
func TestHarnessEngineRegistrationRejectsDifferentCommandSemantics(t *testing.T) {
	...
	lease, err := engine.register("shared", first, TurnSpec{Request: agentchat.ChatRequest{Message: "first payload"}})
	if err != nil {
		t.Fatal(err)
	}
	defer lease.release()
	...
	if _, err := engine.register("shared", conflict, TurnSpec{Request: agentchat.ChatRequest{Message: "different payload"}}); !errors.Is(err, runstate.ErrInvalidCommand) || !errors.Is(err, ErrHarnessTurnSpecConflict) {
...
func testHarnessRunOptions() agentrun.Options {
...
func registerAcceptedHarnessTurn(engine *harnessEngine, ref string, spec TurnSpec) error {
