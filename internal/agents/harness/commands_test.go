...
func TestChatServiceRejectsOversizedCommandBeforeOpeningBinding(t *testing.T) {
	...
	_, err = service.SubmitCommand(context.Background(), CommandSpec{
		Kind: CommandStartTurn, CommandID: strings.Repeat("x", limits.MaxCommandIDBytes+1),
...
func TestChatServiceStartRejectsOversizedCommandBeforeOpeningBinding(t *testing.T) {
	...
	limits := runstate.DefaultInputLimits()
	_, err = service.StartWithOptions(context.Background(), nil, nil, nil, agentchat.ChatRequest{
...
func TestChatServiceSubmitSteerTargetsActiveOperation(t *testing.T) {
	...
	receipt, err := service.SubmitCommand(context.Background(), CommandSpec{
		...
		Conversation: &runControlConversation{}, Request: agentchat.ChatRequest{Message: "change direction"},
...
func TestChatServiceAcceptedRunFollowsNextTurnAndRemainsControllable(t *testing.T) {
	...
	nextReceipt, err := service.SubmitCommand(context.Background(), CommandSpec{
	...
	followUpReceipt, err := service.SubmitCommand(context.Background(), CommandSpec{
		...
		Conversation: &runControlConversation{}, Request: agentchat.ChatRequest{Message: "continue successor"}, Emit: emit,
		Options: agentrun.Options{AgentKind: agentrun.AgentKindIDE, Workspace: "/book", SessionID: "next-turn-control"},
	})
	if err != nil {
		t.Fatal(err)
...
func TestChatServiceCommitsInitialAndFollowUpCyclesExactlyOnce(t *testing.T) {
	...
	followUpSpec := CommandSpec{
		...
		Conversation: followUp, Request: agentchat.ChatRequest{Message: "continue"},
	...
	queuedRetry.Runner = newRunControlTestRunner(t, &runControlFixedModel{message: agent.AssistantMessage("must not run", nil)}, true)
	...
	if _, err := service.coordinator.engine.take(turnRef); !errors.Is(err, ErrHarnessTurnSpecNotFound) {
...
func TestChatServiceSteerPreemptsBlockingPreparationBeforeModelEffects(t *testing.T) {
	...
	startReceipt, err := service.SubmitCommand(context.Background(), CommandSpec{
		Kind: CommandStartTurn, CommandID: "prepare-start",
		Runner:       newRunControlTestRunner(t, &runControlFixedModel{message: agent.AssistantMessage("must not run", nil)}, true),
		Conversation: initial, Request: agentchat.ChatRequest{Message: "initial"}, Options: options,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	steerReceipt, err := service.SubmitCommand(context.Background(), CommandSpec{
		Kind: CommandSteer, CommandID: "prepare-steer", OperationID: startReceipt.OperationID,
		Runner:       newRunControlTestRunner(t, &runControlFixedModel{message: agent.AssistantMessage("steered answer", nil)}, true),
		Conversation: steered, Request: agentchat.ChatRequest{Message: "redirect"}, Options: options,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestChatHarnessCloseClearsDurablyAcceptedTurnSpec(t *testing.T) {
	...
	lease, err := service.coordinator.engine.register(
		...
		command,
		TurnSpec{Request: agentchat.ChatRequest{Message: "queued"}},
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := service.coordinator.engine.take("accepted-before-close"); !errors.Is(err, ErrHarnessTurnSpecNotFound) {
...
func TestHarnessTurnSpecSemanticFingerprintUsesCallerPayloadOnly(t *testing.T) {
	first := CommandSpec{Request: agentchat.ChatRequest{
...
func (c *harnessCommitConversation) CommitAgentCycle(_ context.Context, outcome agentrun.Outcome) error {
