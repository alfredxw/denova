...
func (e *harnessEngine) take(ref string) (TurnSpec, error) {
	ref = strings.TrimSpace(ref)
	if ref == "" {
		return TurnSpec{}, ErrHarnessTurnSpecRefRequired
	}
	e.mu.Lock()
	defer e.mu.Unlock()
	entry, exists := e.specs[ref]
	if !exists {
		return TurnSpec{}, fmt.Errorf("%w: %q", ErrHarnessTurnSpecNotFound, ref)
	}
	delete(e.specs, ref)
...
func (e *harnessEngine) restorePendingInput(
	...
	if e == nil {
		return fmt.Errorf("%w: engine is nil", ErrTurnRestoreUnavailable)
	}
	...
	e.mu.Unlock()
	if restorer == nil {
		return ErrTurnRestoreUnavailable
	}
	...
	if err != nil {
		return fmt.Errorf("%w: %v", ErrTurnRestoreUnavailable, err)
	}
...
func restoredQueuedCommand(request TurnRestoreRequest, input runstate.UserInput) (runstate.Command, error) {
	switch request.Kind {
	case CommandSteer:
	...
	default:
		return nil, fmt.Errorf("%w: queued command kind %q is not restorable", ErrTurnRestoreUnavailable, request.Kind)
	}
}
