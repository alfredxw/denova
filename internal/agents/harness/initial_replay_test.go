...
func TestInitialStartColdReplayReturnsDurableOutcomeWithoutEngine(t *testing.T) {
	...
	firstService, err := newService(context.Background(), agentrun.DefaultLoopPolicy(), firstStore)
	if err != nil {
		t.Fatal(err)
	}
	...
	secondService, err := newService(context.Background(), agentrun.DefaultLoopPolicy(), secondStore)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOlderSettledInitialStartColdReplayDoesNotWaitForFutureEvents(t *testing.T) {
	...
	service, err := newService(context.Background(), agentrun.DefaultLoopPolicy(), store)
	if err != nil {
		t.Fatal(err)
	}
	options := agentrun.Options{
	...
	reopened, err := newService(context.Background(), agentrun.DefaultLoopPolicy(), reopenedStore)
	if err != nil {
		t.Fatal(err)
	}
