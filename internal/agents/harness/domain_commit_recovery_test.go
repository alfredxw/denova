...
func TestBindingHarnessEngineDelegatesExactDomainCommitQuery(t *testing.T) {
	...
	owner.domainCommitReconciler = func(_ context.Context, got agentrun.DomainCommitReconcileRequest) (agentrun.DomainCommitReconcileResult, error) {
...
func TestWithHarnessDomainCommitReconcilerRejectsNil(t *testing.T) {
	t.Parallel()
	if _, err := NewDurableService(context.Background(), t.TempDir(), WithDomainCommitReconciler(nil)); err == nil {
