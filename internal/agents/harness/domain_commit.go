...
func coordinateHarnessDomainCommit(
	...
	if err := emit(runstate.EngineDomainCommitIntent{Identity: identity, Hash: intent.Hash}); err != nil {
		if stage == agentrun.DomainCommitOutput {
			_ = commitHarnessCycleStage(ctx, participant, stage, agentrun.Outcome{Status: agentrun.OutcomeAborted, Error: err, Reason: err.Error(), Content: outcome.Content, Thinking: outcome.Thinking})
		}
...
func commitHarnessCycleStage(ctx context.Context, participant agentrun.DomainCommitParticipant, stage agentrun.DomainCommitStage, outcome agentrun.Outcome) (err error) {
	defer func() {
...
func reportHarnessOutcome(ctx context.Context, spec TurnSpec, outcome agentrun.Outcome) {
	if spec.Outcome == nil {
		return
	}
	select {
...
func commitHarnessCycle(ctx context.Context, commit func(context.Context, agentrun.Outcome) error, outcome agentrun.Outcome) (err error) {
	if commit == nil {
		return nil
	}
	defer func() {
