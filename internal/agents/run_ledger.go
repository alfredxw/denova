...
func (l *RunLedger) RecordToolDecision(decision ToolDecision) error {
	...
	if reasonClass := safeErrorClass(decision.Reason); reasonClass != "" {
...
func (l *RunLedger) RecordToolExecution(result ToolExecutionRecord) error {
	...
	if errorClass := safeErrorClass(result.Error); errorClass != "" {
...
func (l *RunLedger) RecordFinish(status, reason string, generatedBytes int) error {
	...
	if reasonClass := safeErrorClass(reason); reasonClass != "" {
...
func (l *RunLedger) writeRecord(recordType string, data map[string]any) error {
	if l == nil || l.file == nil {
		return nil
	}
	l.mu.Lock()
...
func (l *RunLedger) traceSpanData(span TraceSpanRecord) map[string]any {
	...
	if span.Error != "" {
		data["error_class"] = safeErrorClass(span.Error)
	}
...
func appendLedgerErrorClass(out, data map[string]any, keys ...string) {
	for _, key := range keys {
		...
		if class := safeErrorClass(value); class != "" {
