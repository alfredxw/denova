...
func newWorkspaceEditFileTool(changes workspaceChangeService) (agent.BaseTool, error) {
	if changes == nil {
...
func newWorkspaceWriteFileTool(changes workspaceChangeService) (agent.BaseTool, error) {
	if changes == nil {
		return nil, fmt.Errorf("workspace change service is nil")
	}
	workspace, err := canonicalChangeWorkspace(changes)
