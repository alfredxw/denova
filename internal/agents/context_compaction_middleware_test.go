...
type maintenancePlanningConversation struct {
	...
	compactionIn     ContextCompactionInput
	sawSnapshot      bool
	compacted        []*agent.Message
	compactionErr    error
	stageErr         error
	compactionResult ContextCompactionResult
}

func (conversation *maintenancePlanningConversation) ContextPressurePolicy([]*agent.Message) ContextPressurePolicy {
	return conversation.policy
}

func (conversation *maintenancePlanningConversation) StageToolResultCleanup(_ context.Context, _ []*agent.Message, plan ToolResultCleanupPlan) error {
...
func (*recordingNativeCleanupExecutor) ExecutionMode() ToolResultCleanupExecutionMode {
	return ToolResultCleanupNativeCacheEdit
}

func (executor *recordingNativeCleanupExecutor) Execute(_ context.Context, snapshot *agent.ModelRequestSnapshot, plan ToolResultCleanupPlan) error {
	executor.calls++
...
func TestNativeAndLocalCleanupExecutorsProduceIdenticalModelProjection(t *testing.T) {
	...
	if err != nil || localResult.Action != ContextMaintenanceCleanup {
	...
	if err != nil || nativeResult.Action != ContextMaintenanceCleanup {
		t.Fatalf("native cleanup result=%#v err=%v", nativeResult, err)
	}
	if nativeExecutor.calls != 1 || nativeExecutor.snapshot == nil || nativeResult.Cleanup.CleanupExecutionMode != ToolResultCleanupNativeCacheEdit {
...
func TestNativeCleanupPreparationFailureDiscardsStagedRecord(t *testing.T) {
	...
	if err == nil || !result.Attempted || result.Action != ContextMaintenanceNone || next != call {
...
func (conversation *maintenancePlanningConversation) CompactContextIfNeeded(ctx context.Context, input ContextCompactionInput) ([]*agent.Message, ContextCompactionResult, error) {
	conversation.compactionRuns++
	conversation.compactionIn = input
	conversation.sawSnapshot = compactionRequestSnapshotFromContext(ctx) != nil
	if conversation.compactionErr != nil {
		return input.Messages, ContextCompactionResult{}, conversation.compactionErr
	}
	...
	return input.Messages, ContextCompactionResult{}, nil
}

func TestCleanupStageFailureFallsBackOnlyAtCompactionPressure(t *testing.T) {
	messages := pressureHistory(10, 9000, agent.ToolResultDeferred, agent.ToolResultContextDiscardable)
	messages = append(messages, agent.UserMessage(strings.Repeat("cold-suffix ", 12_000)))
	findPolicy := func(hard bool) ContextPressurePolicy {
		for window := 120_000; window <= 400_000; window += 2_000 {
			...
			if decision.Action == ContextMaintenanceCleanup && atHard == hard {
				return policy
			}
		}
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			conversation := &maintenancePlanningConversation{
				...
				compacted: []*agent.Message{NewContextCompactionSummaryMessage(1, "checkpoint"), agent.UserMessage("current")},
			}
			call := &agent.ModelCall{Model: &compactionForkCaptureModel{}, Messages: messages, Options: []agent.ModelOption{agent.WithTools(nil)}}
			...
			if test.hard && result.Action != ContextMaintenanceCompaction {
				t.Fatalf("hard fallback action=%q", result.Action)
			}
			if !test.hard && result.Action != ContextMaintenanceNone {
...
func TestDegradedCompactionDefersPrimaryProviderCall(t *testing.T) {
	...
	conversation := &maintenancePlanningConversation{
		...
		compacted:        []*agent.Message{NewContextCompactionSummaryMessage(1, "checkpoint")},
		compactionResult: ContextCompactionResult{Degraded: true},
	}
	...
	built, err := agent.NewAgent(context.Background(), agent.AgentConfig{
		...
		Middlewares: []agent.Middleware{&contextCompactionMiddleware{BaseMiddleware: &agent.BaseMiddleware{}, agentKind: "ide"}},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestContextMaintenanceRunsAtEveryModelSeamButMutatesOnlyOncePerRun(t *testing.T) {
	...
	conversation := &maintenancePlanningConversation{policy: policy}
	model := &maintenanceSequenceModel{}
	middleware := &contextCompactionMiddleware{BaseMiddleware: &agent.BaseMiddleware{}, agentKind: "ide"}
	built, err := agent.NewAgent(context.Background(), agent.AgentConfig{
...
func TestContextMaintenancePlannerRoutesHardPressureThroughSnapshotFork(t *testing.T) {
	...
	conversation := &maintenancePlanningConversation{
		policy: policy,
		compacted: []*agent.Message{
			NewContextCompactionSummaryMessage(1, "checkpoint"),
			agent.UserMessage("current"),
		},
	}
	...
	next, result, err := prepareContextMaintenance(
		context.Background(),
		&contextCompactionController{conversation: conversation, emit: func(event Event) { events = append(events, event) }},
		call,
		&agent.ModelContext{},
	)
	if err != nil {
	...
	if !result.Triggered || result.Action != ContextMaintenanceCompaction || conversation.compactionRuns != 1 {
		t.Fatalf("maintenance result = %#v compaction_calls=%d", result, conversation.compactionRuns)
	}
	if conversation.compactionIn.Force || !conversation.compactionIn.Planned || conversation.compactionIn.Phase != contextCompactionPhaseModelStep || !conversation.sawSnapshot {
	...
	if len(next.Messages) != 2 || !isContextCompactionMessage(next.Messages[0]) || next.Messages[1].Content != "current" {
	...
	for _, event := range events {
		...
		if !ok || data["status"] != "skipped" || data["action"] != ContextMaintenanceCompaction {
...
func TestContextMaintenanceAdvancesCompactionBeforeForkCapacityIsExhausted(t *testing.T) {
	...
	if !result.Triggered || result.Action != ContextMaintenanceCompaction || result.Cleanup.Reason != "compaction_capacity_reserve" {
