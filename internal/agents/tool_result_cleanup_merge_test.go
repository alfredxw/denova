...
func TestMergeToolResultCleanupIsOrderedIdempotentAndStorageNeutral(t *testing.T) {
	merged, err := MergeToolResultCleanup(
		[]PersistedToolResultReplacement{{MessageIndex: 4, ToolCallID: "old", Placeholder: "old receipt"}},
		[]ToolResultCleanupReplacement{
...
func TestMergeToolResultCleanupRejectsOutOfBoundsProjection(t *testing.T) {
	_, err := MergeToolResultCleanup(nil, []ToolResultCleanupReplacement{{
