...
func (l *chatAgentLoop) handleToolOutput(messageOutput *agent.MessageVariant, eventMeta agentEventMetadata) chatLoopResult {
	...
	if illustrationResult, parseErr := producttools.ParseChapterIllustrationResult(message.ToolName, fullToolContent); parseErr != nil {
		run.logger.WarnContext(run.ctx, "parse_chapter_illustration_result_failed", slog.String("tool", message.ToolName), slog.String("error_class", agentrun.ErrorClass(parseErr.Error())))
		...
		run.logger.WarnContext(run.ctx, "parse_interactive_image_result_failed", slog.String("tool", message.ToolName), slog.String("error_class", agentrun.ErrorClass(parseErr.Error())))
	...
	if receipt, ok := workspacechange.ParseToolReceipt(message.ToolName, fullToolContent); ok {
	...
	run.emit(agentrun.Event{Type: "tool_result", Data: data})
	return chatLoopResult{action: chatLoopContinue}
}
...
func (l *chatAgentLoop) handleToolExecution(event *agent.AgentEvent) chatLoopResult {
	...
	case agent.ToolExecutionStarted:
		run.emit(agentrun.Event{Type: "tool_started", Data: data})
	case agent.ToolExecutionProgress:
		data["delta"] = execution.Delta
		run.emit(agentrun.Event{Type: "tool_progress", Data: data})
	case agent.ToolExecutionFinished:
		if execution.Result == nil || agentplan.IsToolName(execution.ToolName) {
			return chatLoopResult{action: chatLoopContinue}
		}
		...
		data["tool_result_original_tokens"] = toolresult.EstimatedTokens(int64(result.Metadata.OriginalModelBytes))
		data["tool_result_inline_tokens"] = toolresult.EstimatedTokens(int64(result.Metadata.ReturnedModelBytes))
...
func (l *chatAgentLoop) toolDrainFailed(drainErr error) chatLoopResult {
	...
	l.cancel()
	run.logger.ErrorContext(run.ctx, "run_interrupted", slog.String("reason", "tool_result_idle_timeout"), slog.String("error_class", agentrun.ErrorClass(drainErr.Error())), slog.Int("generated_bytes", len(terminalContent)))
	...
	run.emit(agentrun.Event{Type: "error", Data: map[string]string{"message": drainErr.Error()}})
	return chatLoopResult{action: chatLoopTerminal, outcome: agentrun.NewOutcome(agentrun.OutcomeFailed, drainErr, drainErr.Error(), terminalContent, terminalThinking)}
}
...
func (l *chatAgentLoop) handleAssistantOutput(messageOutput *agent.MessageVariant, eventMeta agentEventMetadata) chatLoopResult {
	...
	if messageOutput.IsStreaming && messageOutput.MessageStream != nil {
		...
		if streamErr != nil {
			...
			if reason, retrying := agentinteractive.CompletionRetryFromError(streamErr); retrying {
...
func (l *chatAgentLoop) assistantStreamFailed(streamErr error) chatLoopResult {
	...
	return chatLoopResult{action: chatLoopTerminal, outcome: agentrun.NewOutcome(agentrun.OutcomeFailed, streamErr, streamErr.Error(), terminalContent, terminalThinking)}
}
