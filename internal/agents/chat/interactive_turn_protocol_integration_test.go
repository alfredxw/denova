...
func TestInteractiveProtocolRetryRevalidatesCompleteProviderInput(t *testing.T) {
	...
	nativeAgent, err := agent.NewAgent(ctx, agent.AgentConfig{
		...
		Middlewares: []agent.Middleware{
			...
			agentrun.NewModelInputLoggingMiddleware(
				agentrun.AgentKindInteractiveStory, providers.ModelConfig{}, 0, providerInputMaxBytes,
			),
		},
		Retry: &agent.RetryConfig{
			MaxRetries:  1,
			ShouldRetry: agentinteractive.NewCompletionGuard(ready.Load),
	...
	runner := agent.NewRunner(agent.RunnerConfig{Agent: nativeAgent, EnableStreaming: true})
	iterator := runner.Run(ctx, []*agent.Message{agent.UserMessage(strings.Repeat("初始事实。", 400))})
	var limitErr *modelio.ProviderInputLimitError
	for {
...
func TestInteractiveTurnProtocolRecoversMissingSubmissionInsideAgentLoop(t *testing.T) {
	ctx := context.Background()
	var ready atomic.Bool
	tools, err := producttools.NewInteractiveTurn(agenttoolruntime.ProjectInteractiveContext(agentinteractive.InteractiveStoryToolContext{
	...
	runner := agent.NewRunner(agent.RunnerConfig{Agent: nativeAgent, EnableStreaming: true})
	iterator := runner.Run(ctx, []*agent.Message{agent.UserMessage("推开石门")})
	final := ""
	for {
		event, ok := iterator.Next()
		if !ok {
		...
		if event.Err != nil {
			if _, retrying := agentinteractive.CompletionRetryFromError(event.Err); retrying {
				continue
			}
		...
		if streamErr != nil {
			if _, retrying := agentinteractive.CompletionRetryFromError(streamErr); retrying {
				continue
			}
	...
	if len(toolChoices) != 3 || toolChoices[2] != string(agent.ToolChoiceForbidden) {
...
func TestInteractiveTurnProtocolAccountsRejectedModelCallUsage(t *testing.T) {
	...
	outcome := NewExecutor(agentrun.DefaultLoopPolicy()).Run(ctx, runner, conversation, nil, ChatRequest{Message: "推开石门"}, agentrun.Options{
...
func TestInteractiveTurnProtocolRetriesRejectedModulesBeforeReusingCandidate(t *testing.T) {
	...
	chatModel := &interactiveTurnProtocolChatModel{responses: []*agent.Message{
		agent.AssistantMessage("门后传来锁链拖地的声音。", nil),
		agent.AssistantMessage("", []agent.ToolCall{{
			ID: "call-submit-1", Function: agent.FunctionCall{Name: agentinteractive.TurnSubmissionToolName, Arguments: `{"state_changes":[{"op":"replace","actor_id":"story","field_id":"当前事件","value":1}],"choices":["进入房间","观察门后","检查锁链","询问同伴","退后戒备"]}`},
		}}),
		agent.AssistantMessage("", []agent.ToolCall{{
			ID: "call-submit-2",
			Function: agent.FunctionCall{
				Name:      agentinteractive.TurnSubmissionToolName,
	...
	conversation := &interactiveProtocolConversation{ready: &ready}
	var events []agentrun.Event
	NewExecutor(agentrun.DefaultLoopPolicy()).Run(ctx, runner, conversation, nil, ChatRequest{Message: "推开石门"}, agentrun.Options{
		AgentKind:     agentrun.AgentKindInteractiveStory,
...
func TestInteractiveTurnProtocolLocksFirstCandidateAcrossRejectedModuleAndLaterProse(t *testing.T) {
	...
	chatModel := &interactiveTurnProtocolChatModel{responses: []*agent.Message{
		...
		agent.AssistantMessage("", []agent.ToolCall{{
			ID: "rejected-state-good-choices", Function: agent.FunctionCall{Name: agentinteractive.TurnSubmissionToolName, Arguments: `{"state_changes":[{"op":"replace","actor_id":"story","field_id":"当前事件","value":"首次领域校验拒绝"}],"choices":["继续观察","绕到侧面","悄然后退","制造声响","询问同伴"]}`},
		}}),
	...
	nativeAgent, err := agent.NewAgent(ctx, agent.AgentConfig{
		...
		Middlewares: []agent.Middleware{agentinteractive.NewTurnProtocolMiddleware(ready.Load)},
		Tools:       tools,
		Retry:       &agent.RetryConfig{MaxRetries: 1, ShouldRetry: agentinteractive.NewCompletionGuard(ready.Load), BackoffFunc: func(context.Context, int) time.Duration { return time.Nanosecond }},
	})
	if err != nil {
		t.Fatal(err)
	}
	runner := agent.NewRunner(agent.RunnerConfig{Agent: nativeAgent, EnableStreaming: true})
	conversation := &interactiveProtocolConversation{ready: &ready}
	NewExecutor(agentrun.DefaultLoopPolicy()).Run(ctx, runner, conversation, nil, ChatRequest{Message: "观察敌情"}, agentrun.Options{
		AgentKind: agentrun.AgentKindInteractiveStory, RootAgentName: "interactive-protocol-first-candidate-test",
	}, func(agentrun.Event) {})
...
func eventTypeIndex(events []agentrun.Event, eventType string) int {
...
func (m *interactiveTurnProtocolChatModel) Generate(_ context.Context, messages []*agent.Message, opts ...agent.ModelOption) (*agent.Message, error) {
	return m.nextMessage(messages, opts...)
}

func (m *interactiveTurnProtocolChatModel) Stream(_ context.Context, messages []*agent.Message, opts ...agent.ModelOption) (*agent.StreamReader[*agent.Message], error) {
...
func (m *interactiveTurnProtocolChatModel) nextMessage(messages []*agent.Message, opts ...agent.ModelOption) (*agent.Message, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.calls++
	...
	if m.calls <= len(m.responses) {
		...
		switch m.calls {
		case 1:
			message = agent.AssistantMessage("门后传来锁链拖地的声音。", nil)
		case 2:
			message = agent.AssistantMessage("", []agent.ToolCall{{
				ID: "call-submit", Function: agent.FunctionCall{Name: agentinteractive.TurnSubmissionToolName, Arguments: `{"state_changes":[],"choices":["进入房间","观察门后","检查锁链","询问同伴","退后戒备"]}`},
			}})
		default:
			message = agent.AssistantMessage("石门缓缓开启。", nil)
		}
	}
	message.ResponseMeta = &agent.ResponseMeta{Usage: &agent.TokenUsage{
...
func readInteractiveProtocolMessage(variant *agent.MessageVariant) (*agent.Message, error) {
	if variant == nil {
		return nil, nil
	}
