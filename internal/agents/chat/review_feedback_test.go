...
func TestTurnInputProjectionInjectsOnlyResolvedReviewFeedbackWithSource(t *testing.T) {
	req := ChatRequest{
		...
		ResolvedReviewFeedback: agentreview.Contexts{{
			...
			Source:         agentreview.SourceDocument,
			...
			Comments: []agentreview.Comment{{
				...
				Target: &agentreview.Target{
					...
					Snapshot: &agentreview.Snapshot{Revision: "lore-r2", Content: "Canonical lore body."},
				},
...
func TestReviewFeedbackContextEnforcesWholeBlockByteLimit(t *testing.T) {
	feedback := agentreview.Contexts{{
		ReviewThreadID: "thread-1",
		Comments: []agentreview.Comment{{
			ID:      "comment-1",
			GroupID: "group-1",
			Body:    strings.Repeat("界", agentreview.MaxContextBytes),
		}},
	}}
	if got := feedback.EncodedSize(); got <= agentreview.MaxContextBytes {
	...
	block, err := agentreview.RenderModelContextBlock(feedback)
	if err != nil {
		t.Fatal(err)
	}
	if len(block) == 0 || len(block) > agentreview.MaxContextBytes {
		t.Fatalf("review feedback block bytes=%d", len(block))
	}
}

func TestEmptyReviewFeedbackDoesNotProduceModelContext(t *testing.T) {
	if block, ok := (agentreview.Contexts)(nil).ModelContextBlock(); ok || block != "" {
