...
func TestPlanProtocolParserCarriesRunMetadata(t *testing.T) {
	...
	for _, ev := range events {
		...
		if eventDataString(ev.Data, "agent_kind") != agentrun.AgentKindIDE {
			t.Fatalf("agent_kind = %q, want %q in event %#v", eventDataString(ev.Data, "agent_kind"), agentrun.AgentKindIDE, ev)
		}
...
func TestPlanProtocolToolCallEmitsPlanCardInsteadOfToolCall(t *testing.T) {
	...
	msg := agent.AssistantMessage("", []agent.ToolCall{{
		ID: "call-plan",
	...
	processNonStreamingEvent(&agent.MessageVariant{Message: msg}, &content, &thinking, 0, agentEventMetadata{
		AgentKind: agentrun.AgentKindIDE,
		RunID:     "run-plan-tool",
	}, parser, func(ev agentrun.Event) {
	...
	if eventDataString(events[0].Data, "id") != agentplan.ToolEventID {
		t.Fatalf("id = %q, want %q", eventDataString(events[0].Data, "id"), agentplan.ToolEventID)
	}
