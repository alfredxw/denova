...
func emitContextCleanupEvent(emit func(agentrun.Event), status string, decision agentcontext.ContextPressureDecision, actualReclaimed int, err error) {
	if emit == nil {
		return
	}
	eagerApplied := 0
	if status == "completed" && decision.Action == agentcontext.ContextMaintenanceCleanup {
		eagerApplied = decision.Cleanup.EagerGroupCount
	}
	data := map[string]any{
		"phase":                               agentcompaction.PhaseModelStep,
		"status":                              status,
	...
	if err != nil {
		// The live UI receives the actionable diagnostic. agentrun.Ledger converts it
		// to error_class and never persists this raw text.
		data["error"] = err.Error()
	}
	emit(agentrun.Event{Type: "context_cleanup", Data: data})
}
