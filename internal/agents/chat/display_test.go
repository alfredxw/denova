...
func (c *failingAssistantConversation) AssembleModelContext(ctx context.Context, _ string, input agentcontext.ModelContextInput) (agentcontext.ModelContextResult, error) {
...
func TestDisplayRecorderPersistsReclassifiedInteractiveContentAsThinking(t *testing.T) {
	...
	recorder.Record(agentrun.Event{Type: "interactive_content_reclassified", Data: map[string]interface{}{
		"agent_kind": agentrun.AgentKindInteractiveStory,
...
func TestDisplayRecorderMarksLastAssistantSegmentPartialAfterAbort(t *testing.T) {
	...
	recorder.Record(agentrun.Event{Type: "chunk", Data: map[string]interface{}{"run_id": "run-aborted", "content": "先完成了一部分。"}})
	recorder.Record(agentrun.Event{Type: "aborted", Data: map[string]string{"reason": "stopped"}})
...
func TestDisplayRecorderPreservesSubAgentSegmentsAroundThinking(t *testing.T) {
	...
	event := func(eventType, content string) agentrun.Event {
		data := make(map[string]interface{}, len(meta)+1)
		for key, value := range meta {
			data[key] = value
		}
		data["content"] = content
		return agentrun.Event{Type: eventType, Data: data}
	}

	for _, item := range []agentrun.Event{
		event("chunk", "调研结论一。"),
...
func TestDisplayRecorderCanSuppressOnlyRootAssistantSegments(t *testing.T) {
	...
	recorder.Record(agentrun.Event{Type: "chunk", Data: rootChunk})
	recorder.Record(agentrun.Event{Type: "chunk", Data: subAgentChunk})
	recorder.Record(agentrun.Event{Type: "done", Data: map[string]string{}})
...
func TestDisplayRecorderAnnotatesStreamingDeltasWithTheirSegmentID(t *testing.T) {
	...
	recorder.Record(agentrun.Event{Type: "chunk", Data: first})
	recorder.Record(agentrun.Event{Type: "chunk", Data: second})
	recorder.Record(agentrun.Event{Type: "thinking", Data: reasoning})
...
func TestDisplayRecorderAppendsStreamingWriteFileContent(t *testing.T) {
	...
	recorder.Record(agentrun.Event{Type: "tool_args_delta", Data: map[string]interface{}{
		"agent_kind": agentrun.AgentKindIDE,
		"id":         "call-1",
		"name":       "write",
