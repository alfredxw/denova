...
// Domain names the canonical store that owns a structural
...
// RestorePlan is the bounded deterministic description of an
...
func describeResult(result Result) contextStructuralResultDescriptor {
	compaction := result.Compaction
	return contextStructuralResultDescriptor{
		Compaction: contextCompactionResultDescriptor{
			CompactionCheckpoint: agentcompaction.NewCheckpoint("", compaction),
...
func (descriptor contextStructuralResultDescriptor) result() Result {
	compaction := descriptor.Compaction
	result := agentcompaction.ResultFromCheckpoint(compaction.CompactionCheckpoint)
...
// IntentHash binds authorization to one action, exact runtime
...
// EncodeRestorePlan validates and encodes a plan under the
...
func EncodeRestorePlan(
	...
	return encodeRestorePlan(plan, ref, expectedRevision, runstate.DefaultInputLimits().MaxRestoreDescriptorBytes)
}

// EncodeRuntimeRestorePlan is the harness-facing variant for an already
// validated runtime binding and an optional stricter descriptor limit.
func EncodeRuntimeRestorePlan(
	plan RestorePlan,
	binding runstate.BindingRef,
	expectedRevision string,
	maxBytes int,
) (json.RawMessage, error) {
	return encodeRestorePlan(plan, binding, expectedRevision, maxBytes)
}
...
func encodeRestorePlan(
	...
	normalized, err := validateRestorePlan(plan, binding, expectedRevision)
	if err != nil {
		return nil, err
	}
	wire := contextStructuralRestorePlanWire{
		...
		Result: describeResult(normalized.Result), Mutation: normalized.Mutation,
	}
...
func contextStructuralRestorePlanDeclaredBytes(plan RestorePlan) int64 {
...
// DecodeRestorePlan rejects unknown fields and validates the
// descriptor against the immutable binding/revision carried by its snapshot.
func DecodeRestorePlan(
	...
	return decodeRestorePlan(descriptor, ref, expectedRevision)
}

// DecodeRuntimeRestorePlan validates a descriptor against the immutable
// binding stored by the durable runtime.
func DecodeRuntimeRestorePlan(
	descriptor json.RawMessage,
	binding runstate.BindingRef,
	expectedRevision string,
) (RestorePlan, error) {
	return decodeRestorePlan(descriptor, binding, expectedRevision)
}
...
func decodeRestorePlan(
	...
	if len(descriptor) == 0 {
		return RestorePlan{}, fmt.Errorf("structural context restore descriptor is absent")
	}
	...
	if err := decodeStrictContextStructuralJSON(descriptor, &wire); err != nil {
		return RestorePlan{}, fmt.Errorf("decode structural context restore plan: %w", err)
	}
	if err := validateRestorePlanRequiredFields(descriptor); err != nil {
		return RestorePlan{}, fmt.Errorf("decode structural context restore plan: %w", err)
	}
	plan := RestorePlan{
	...
	return validateRestorePlan(plan, binding, expectedRevision)
}

func validateRestorePlanRequiredFields(descriptor json.RawMessage) error {
...
func validateRestorePlan(
	...
	if plan.Version != RestorePlanVersion {
		return RestorePlan{}, fmt.Errorf("unsupported structural context restore plan version %d", plan.Version)
	}
	if err := validateContextStructuralBindingRef(binding); err != nil {
		return RestorePlan{}, err
	}
	...
	switch plan.Domain {
	case DomainSession:
		if productBinding.AgentKind != agentrun.AgentKindGeneral && productBinding.AgentKind != agentrun.AgentKindIDE && productBinding.AgentKind != agentrun.AgentKindConfigManager && productBinding.AgentKind != agentrun.AgentKindImage {
			return RestorePlan{}, fmt.Errorf("session structural plan does not match %q binding", binding.Kind)
		}
	case DomainStory:
		if productBinding.AgentKind != agentrun.AgentKindInteractiveStory && productBinding.AgentKind != config.AgentKindInteractiveDirector {
			return RestorePlan{}, fmt.Errorf("story structural plan does not match %q binding", binding.Kind)
		}
	default:
		return RestorePlan{}, fmt.Errorf("unsupported structural context domain %q", plan.Domain)
	}
	if RuntimeKind(plan.Action) == "" {
		return RestorePlan{}, fmt.Errorf("unsupported structural context action %q", plan.Action)
	}
	switch plan.Action {
	case Compact:
		if plan.Result.Removed || plan.Result.Compaction.Triggered != plan.Commit {
			return RestorePlan{}, fmt.Errorf("compact structural plan commit/result mismatch")
		}
	case Remove:
		if plan.Result.Removed != plan.Commit || plan.Result.Compaction != (agentcompaction.Result{}) {
			return RestorePlan{}, fmt.Errorf("remove structural plan commit/result mismatch")
		}
	}
	...
	if plan.IntentHash == "" || plan.IntentHash != wantHash {
		return RestorePlan{}, fmt.Errorf("structural context restore plan intent hash mismatch")
	}
	return plan, nil
}
...
func validateContextStructuralBindingRef(binding runstate.BindingRef) error {
	...
	if productBinding.AgentKind == agentrun.AgentKindAutomation {
