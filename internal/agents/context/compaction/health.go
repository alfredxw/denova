...
// FailureState is the small provider-neutral state persisted
...
const (
	...
	HealthManualRetry HealthOutcome = "manual_retry"
)

// Publication records whether a prepared checkpoint reached its durable
// conversation store. Health transitions use it to distinguish successful
// summary generation from successful checkpoint publication.
...
func (state FailureState) Blocks(structureFingerprint string, maximum int, automatic bool) bool {
...
func (state FailureState) NextFailure(structureFingerprint string) FailureState {
	...
	return FailureState{StructureFingerprint: structureFingerprint, ConsecutiveFailures: next}
}

// StructureFingerprint hashes only deterministic structure
...
func StructureFingerprint(messages []*agent.Message, tools []*agent.ToolInfo, anchors ...string) string {
	payload := struct {
...
func NoProgressLatched(
	...
	hardPublishBand := PublishLimit(contextWindow, threshold)
	return previousTokens > recoveryTarget && previousTokens < hardPublishBand
}

// CandidateIdentity hashes bounded tool-result locators and
...
func CandidateIdentity(messages []*agent.Message, _ int) (string, uint64) {
...
// FailureReason returns the bounded semantic class recorded by durable health
// state; provider error strings are intentionally not persisted verbatim.
