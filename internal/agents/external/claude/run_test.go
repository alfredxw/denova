...
func testInstalledClaudeToolLoop(t *testing.T, source string) {
	...
	for _, tool := range modelTools {
		if !strings.HasPrefix(tool, "mcp__denova__") {
	...
	if _, err := c.Run(stopCtx, input, cancelledHost); !errors.Is(err, context.Canceled) {
		t.Fatalf("cancelled CLI run: %v", err)
	}
	c.mu.Lock()
	active := len(c.active)
	c.mu.Unlock()
	if active != 0 || len(cancelledHost.calls) != 1 {
		t.Fatal("cancel left an active process or ran another tool")
	}
