...
func TestCompactionSummarizerLayersOversizedSourceWithoutDroppingBytes(t *testing.T) {
	...
	calls := 0
	observedBytes := 0
	summarizeContextForCompaction = func(_ context.Context, _ *config.Config, _ string, checkpoint string, source []*agent.Message, _ string, _ int, _ contextCompactionPolicy, _ func(int, string)) (string, int, error) {
		calls++
		batchBytes := 0
