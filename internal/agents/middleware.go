...
func (m *interactiveDirectorPlanFileMiddleware) WrapInvokableToolCall(
	_ context.Context,
	endpoint agent.InvokableToolCallEndpoint,
	toolCtx *agent.ToolContext,
) (agent.InvokableToolCallEndpoint, error) {
	return func(ctx context.Context, args string, opts ...agent.ToolOption) (string, error) {
...
func (m *interactiveDirectorPlanFileMiddleware) WrapStreamableToolCall(
	_ context.Context,
	endpoint agent.StreamableToolCallEndpoint,
	toolCtx *agent.ToolContext,
) (agent.StreamableToolCallEndpoint, error) {
	return func(ctx context.Context, args string, opts ...agent.ToolOption) (*agent.StreamReader[string], error) {
		if msg := m.blockedDirectorToolMessage(toolName(toolCtx), args); msg != "" {
...
type ToolDecision struct {
	...
	Descriptor        agenttools.Descriptor `json:"descriptor"`
}
...
func (m *toolOrchestratorMiddleware) WrapInvokableToolCall(
	...
	return func(ctx context.Context, args string, opts ...agent.ToolOption) (string, error) {
		...
		if err != nil {
			...
			if agent.IsInterruptError(err) {
				return "", err
			}
			return msg, nil
		}
		filtered := filterToolResultForModelWithDescriptor(toolName(toolCtx), decision.Descriptor, args, result, m.toolResultLimitBytes())
...
func (m *toolOrchestratorMiddleware) acquireToolExecution(ctx context.Context, decision ToolDecision) (func(), error) {
	...
	manifest := manifestForDefinition(decision.ToolName, decision.Descriptor)
	return m.executionGate.acquire(ctx, executionModeForTool(manifest))
}

func singleChunkReader(msg string) *agent.StreamReader[string] {
	r, w := agent.Pipe[string](1)
	_ = w.Send(msg, nil)
	w.Close()
	return r
}

func filterToolResultReader(ctx context.Context, sr *agent.StreamReader[string], decision ToolDecision, args string, maxBytes int, releases ...func()) *agent.StreamReader[string] {
	r, w := agent.Pipe[string](1)
	go func() {
		defer w.Close()
		defer func() {
...
func (m *toolOrchestratorMiddleware) buildToolDecision(toolCtx *agent.ToolContext, args string) ToolDecision {
	...
	descriptor, declared := agenttools.DescriptorFromInfo(toolCtx.Info)
	if declared {
		manifest = manifestForDefinition(name, descriptor)
	}
