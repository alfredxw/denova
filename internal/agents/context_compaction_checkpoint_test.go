...
func TestContextCompactionCheckpointRoundTripsEveryDurableField(t *testing.T) {
	checkpoint := NewContextCompactionCheckpoint(AgentKindIDE, ContextCompactionResult{
	...
	restored := ContextCompactionResultFromCheckpoint(checkpoint)
	if got := NewContextCompactionCheckpoint(AgentKindIDE, restored); got != checkpoint {
		t.Fatalf("checkpoint round trip changed durable fields:\ngot  %#v\nwant %#v", got, checkpoint)
	}
	applyContextCompactionRecovery(&restored)
