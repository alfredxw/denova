...
func TestSessionToolResultCleanupPersistsProjectionWithoutRewritingHistory(t *testing.T) {
	...
	plan := ToolResultCleanupPlan{
		Replacements: []ToolResultCleanupReplacement{{
			...
			OriginalTokens: estimateMessageTokens(visible[2]), PlaceholderTokens: estimateStringTokens(placeholder),
		}},
		ReclaimedTokens: 1000, EarliestChanged: 1, RendererVersion: toolResultPlaceholderRendererVersion,
	}
	...
	projected := NewSessionConversationForAgent(reloaded, &config.Config{}, AgentKindIDE).modelHistory(reloadedSnapshot)
...
func TestSessionCleanupDefersAcrossRewindWithReusedProviderCallID(t *testing.T) {
	...
	plan := ToolResultCleanupPlan{Replacements: []ToolResultCleanupReplacement{{
		MessageIndex: resultIndex, ToolCallID: "provider-local-call", Placeholder: "rewind-unsafe placeholder",
		OriginalTokens: estimateMessageTokens(visible[resultIndex]), PlaceholderTokens: 4,
	}}}
	...
	decision := PlanContextPressure(visible, nil, policy)
	...
	reloadedVisible := NewSessionConversationForAgent(reloaded, &config.Config{}, AgentKindIDE).modelHistory(reloadedSnapshot)
	if !contextMessagesEqual(visible, reloadedVisible) {
...
func TestSessionCleanupDefersSameTurnStagedRewind(t *testing.T) {
	...
	if err := conversation.StageContextOperation(session.ContextOperation{
		Kind: session.ContextOperationRewind, AgentKind: AgentKindIDE, CheckpointID: "pending-rewind",
	}); err != nil {
		t.Fatal(err)
	}
	...
	err = conversation.StageToolResultCleanup(context.Background(), visible, ToolResultCleanupPlan{
		Replacements: []ToolResultCleanupReplacement{{MessageIndex: 0, ToolCallID: "call", Placeholder: "receipt"}},
	})
