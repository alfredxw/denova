...
func TestProcessToolResultMaterializesOversizedText(t *testing.T) {
	...
	processed, err := processToolResult(ctx, decision, `{"path":"chapter.md"}`, agent.TextToolResult(content), processorTestProcessingPolicy(256))
	if err != nil {
		t.Fatal(err)
	}
...
func TestProcessToolResultReusesOnlyExplicitCompleteModelOutput(t *testing.T) {
	...
	result.Artifacts = []agent.ToolArtifactRef{{
		...
		EstimatedBytes: int64(len(content)), EstimatedTokens: estimatedToolResultTokens(int64(len(content))), Complete: true,
	}}
...
func TestEagerRetentionNoticeUsesThePlannerThreshold(t *testing.T) {
	...
	lowThreshold := toolResultProcessingPolicy{MaxBytes: len(content) + 1024, EagerMinTokens: 10, ContextWindowTokens: 100}
	processed, err := processToolResult(
		context.Background(), processorTestDecision(agent.ToolResultEagerCandidate),
...
func TestProcessToolResultFailsBoundedWithoutPublishingFakeArtifact(t *testing.T) {
	...
	processed, err := processToolResult(ctx, processorTestDecision(agent.ToolResultDeferred), `{"path":"chapter.md"}`, agent.TextToolResult(content), processorTestProcessingPolicy(192))
	if err != nil {
		t.Fatal(err)
	}
...
func TestProcessToolResultRejectsLossyProtectedSpill(t *testing.T) {
	...
	processed, err := processToolResult(ctx, decision, `{"path":"evidence.log"}`, agent.TextToolResult(content), processorTestProcessingPolicy(192))
...
func processorTestDecision(retention agent.ToolResultRetentionMode) ToolDecision {
	return ToolDecision{
...
func processorShellTestDecision() ToolDecision {
	return ToolDecision{
...
func processorTestProcessingPolicy(maxBytes int) toolResultProcessingPolicy {
	return toolResultProcessingPolicy{MaxBytes: maxBytes, EagerMinTokens: 32_000, ContextWindowTokens: 160_000}
}
