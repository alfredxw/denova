...
type PostSettlementToolResultCleanupProvider interface {
	CommitPostSettlementToolResultCleanup(context.Context, OperationID) error
}
...
func (c *SessionConversation) ContextPressurePolicy(messages []*agent.Message) ContextPressurePolicy {
	if c == nil {
...
func (c *SessionConversation) StageToolResultCleanup(ctx context.Context, visible []*agent.Message, plan ToolResultCleanupPlan) error {
	...
	resolved, err := ResolveToolResultCleanupTargets(visible, canonical, plan)
	if err != nil {
		return err
	}

	var existing []PersistedToolResultReplacement
	previousReclaimed := 0
	if existingRecord := snapshot.ToolResultCleanup; existingRecord != nil {
		for _, replacement := range existingRecord.Replacements {
			existing = append(existing, PersistedToolResultReplacement{
	...
	merged, err := MergeToolResultCleanup(existing, resolved, start, int64(total), previousReclaimed, plan.ReclaimedTokens)
	if err != nil {
		return err
	}
	...
	record := session.ToolResultCleanupRecord{
		...
		TriggeredAtUsage: EstimateContextTokens(visible, nil), EarliestChanged: merged.SourceStart,
...
func (c *SessionConversation) CommitPostSettlementToolResultCleanup(ctx context.Context, settledOperationID OperationID) error {
