...
func TestNarrativeStyleConcurrentUpdateAndDeleteHaveOneWinner(t *testing.T) {
	...
	created := applyConfigMutationForTest(t, adapter, Mutation{
		Operation: ApplyCreate,
	...
	go func() {
		ready.Done()
		<-start
		_, err := adapter.Apply(ctx, Mutation{
			Operation: ApplyUpdate, Resource: "narrative_style", ID: created.ID, Revision: created.Revision,
			Value: map[string]any{
	...
	go func() {
		...
		<-start
		_, err := adapter.Apply(ctx, Mutation{
			Operation: ApplyDelete, Resource: "narrative_style", ID: created.ID, Revision: created.Revision,
		})
		errorsByOperation <- err
	}()
	ready.Wait()
...
func TestImagePresetConcurrentCASUpdatesHaveOneWinner(t *testing.T) {
	...
	created := applyConfigMutationForTest(t, adapter, Mutation{
		Operation: ApplyCreate, Resource: "image_preset",
	...
	for _, prompt := range []string{"warm light", "cold light"} {
		...
		go func() {
			...
			<-start
			_, err := adapter.Apply(ctx, Mutation{
				Operation: ApplyUpdate, Resource: "image_preset", ID: created.ID, Revision: created.Revision,
...
func applyConfigMutationForTest(t *testing.T, adapter Adapter, mutation Mutation) configMutationReceipt {
	t.Helper()
