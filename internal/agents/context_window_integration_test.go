...
func TestContextRewindKeepsExplorationDisplayButCommitsOnlyFinalAnswer(t *testing.T) {
	...
	outcome := service.RunWithOptions(
		...
		ChatRequest{CommandID: "context-rewind-output", Message: "research then answer"},
		RunOptions{
			AgentKind: AgentKindIDE, RootAgentName: "context-rewind-integration",
...
func TestCorruptLatestRewindStopsBeforeModelExecution(t *testing.T) {
	...
	outcome := service.RunWithOptions(
		context.Background(), runner, NewSessionConversationForAgent(sess, nil, AgentKindIDE), nil,
		ChatRequest{CommandID: "context-corrupt-rewind", Message: "continue"},
		RunOptions{AgentKind: AgentKindIDE, RootAgentName: "context-corrupt-rewind", Workspace: "context-corrupt-rewind", SessionID: sess.ID},
		nil,
	)
...
func (m *contextRewindSequenceModel) Generate(_ context.Context, input []*agent.Message, _ ...agent.ModelOption) (*agent.Message, error) {
	m.mu.Lock()
	m.inputs = append(m.inputs, cloneContextMessages(input))
	m.calls++
	call := m.calls
	m.mu.Unlock()
	switch call {
...
func (m *contextRewindSequenceModel) input(index int) []*agent.Message {
	...
	return cloneContextMessages(m.inputs[index])
}
