...
func TestContextNormalizerRunsImmediatelyBeforeContextMaintenance(t *testing.T) {
	assembly, err := buildChatModelAgentAssembly(context.Background(), &config.Config{}, chatModelAgentAssemblySpec{
		Kind: config.AgentKindIDE, ToolSettings: config.ResolvedAgentToolSettings{}, IncludeCompaction: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	normalizerIndex := -1
	maintenanceIndex := -1
	for index, middleware := range assembly.Middlewares {
		switch middleware.(type) {
		case *contextNormalizerMiddleware:
			normalizerIndex = index
		case *contextCompactionMiddleware:
			maintenanceIndex = index
		}
	}
	if normalizerIndex < 0 || maintenanceIndex != normalizerIndex+1 {
		t.Fatalf("middleware order normalizer=%d maintenance=%d: %#v", normalizerIndex, maintenanceIndex, assembly.Middlewares)
	}
}

func TestContextNormalizerEmitsBoundedRepairMetric(t *testing.T) {
	missing := contextNormalizerTestCall("missing", "read", `{}`)
