...
func TestInteractiveTurnProtocolRecoversMissingSubmissionInsideAgentLoop(t *testing.T) {
	...
	nativeAgent, err := agent.NewAgent(ctx, agent.AgentConfig{
		...
		Model:         chatModel,
		MaxIterations: 4,
		Middlewares:   []agent.Middleware{newInteractiveTurnProtocolMiddleware(ready.Load)},
		Tools:         tools,
		Retry: &agent.RetryConfig{
			MaxRetries:  1,
			ShouldRetry: newInteractiveCompletionGuard(ready.Load),
...
func TestInteractiveTurnProtocolRetriesRejectedModulesBeforeReusingCandidate(t *testing.T) {
	...
	chatModel := &interactiveTurnProtocolChatModel{responses: []*agent.Message{
		...
		agent.AssistantMessage("", []agent.ToolCall{{
			ID: "call-submit-1", Function: agent.FunctionCall{Name: interactiveTurnSubmissionToolName, Arguments: `{"state_changes":[{"op":"replace","actor_id":"story","field_id":"当前事件","value":1}],"choices":["进入房间","观察门后","检查锁链","询问同伴","退后戒备"]}`},
		}}),
		agent.AssistantMessage("", []agent.ToolCall{{
			ID: "call-submit-2",
...
func TestInteractiveTurnProtocolLocksFirstCandidateAcrossMalformedModuleAndLaterProse(t *testing.T) {
	...
	chatModel := &interactiveTurnProtocolChatModel{responses: []*agent.Message{
		...
		agent.AssistantMessage("", []agent.ToolCall{{
			ID: "good-state", Function: agent.FunctionCall{Name: interactiveTurnSubmissionToolName, Arguments: `{"state_changes":[{"op":"replace","actor_id":"story","field_id":"当前事件","value":"主角在乱石坡观察敌情"}]}`},
		}}),
	}}
	nativeAgent, err := agent.NewAgent(ctx, agent.AgentConfig{
		...
		Middlewares: []agent.Middleware{newInteractiveTurnProtocolMiddleware(ready.Load)},
		Tools:       tools,
		Retry:       &agent.RetryConfig{MaxRetries: 1, ShouldRetry: newInteractiveCompletionGuard(ready.Load), BackoffFunc: func(context.Context, int) time.Duration { return time.Nanosecond }},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if m.calls <= len(m.responses) {
		...
		case 2:
			message = agent.AssistantMessage("", []agent.ToolCall{{
				ID: "call-submit", Function: agent.FunctionCall{Name: interactiveTurnSubmissionToolName, Arguments: `{"state_changes":[],"choices":["进入房间","观察门后","检查锁链","询问同伴","退后戒备"]}`},
			}})
		default:
