...
// ProcessingPolicy controls inline result budgets and eager retention without
// coupling normalization to the complete Agent runtime configuration.
...
// Call is the minimal execution identity needed to persist and recover a tool
// result. Approval and orchestration state deliberately stay in the parent
// agents package.
type Call struct {
	ToolName       string
...
// Process is the single post-execution seam for rich result
...
func Process(
	...
	limit := NormalizeLimitBytes(firstPositive(policy.MaxBytes, decision.Descriptor.MaxResultBytes))
...
func materializeToolResult(ctx context.Context, decision Call, content string) (*agent.ToolArtifactRef, string) {
...
func normalizeProcessedToolResult(decision Call, args string, result agent.ToolResult, limit int) (agent.ToolResult, error) {
	...
	return ProjectReceipt(manifest, args, normalized), nil
}
...
func recoverableToolResultArtifact(artifacts []agent.ToolArtifactRef) *agent.ToolArtifactRef {
	for index := range artifacts {
		...
		if RecoverableArtifactPurpose(artifact.Purpose) {
			return &artifact
		}
	}
	return nil
...
func normalizeUpstreamToolArtifacts(
	...
	for index, candidate := range artifacts {
		candidate = CanonicalArtifact(candidate)
		if RecoverableArtifactPurpose(candidate.Purpose) {
...
func appendToolResultArtifact(artifacts []agent.ToolArtifactRef, artifact agent.ToolArtifactRef) []agent.ToolArtifactRef {
	for index := range artifacts {
		if artifacts[index].ID == artifact.ID ||
			CanonicalArtifact(artifacts[index]).ReadablePath == artifact.ReadablePath {
...
func applyReplayRecoveryHint(result *agent.ToolResult, decision Call, args string, originalBytes int) {
