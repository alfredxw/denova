...
func NewContextWindowController(conversation any, agentKind string) agent.ContextWindowController {
...
func (controller *runContextWindowController) BeforeModel(_ context.Context, messages []*agent.Message) ([]*agent.Message, error) {
	...
	if controller.pending == nil {
		controller.baseline = agentcontext.CloneMessages(messages)
	...
	rewritten := agentcontext.CloneMessages(checkpointBoundary.EffectivePrefix)
	rewritten = append(rewritten, newContextRewindSummaryMessage(operation))
	rewrittenCanonical := agentcontext.CloneMessages(checkpointBoundary.CanonicalPrefix)
	...
	controller.baseline = agentcontext.CloneMessages(rewritten)
...
func (controller *runContextWindowController) freezeBoundary(messages []*agent.Message) (*session.ContextBoundarySnapshot, error) {
	if controller.boundary != nil && agentcontext.MessagesHavePrefix(messages, controller.boundary.EffectivePrefix) {
		suffix := agentcontext.CloneMessages(messages[len(controller.boundary.EffectivePrefix):])
		canonical := append(agentcontext.CloneMessages(controller.boundary.CanonicalPrefix), suffix...)
...
func (controller *runContextWindowController) BeforeComplete(_ context.Context, messages []*agent.Message) ([]*agent.Message, bool, error) {
	...
	return append(agentcontext.CloneMessages(messages), reminder), true, nil
}
