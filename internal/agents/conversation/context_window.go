...
func (c *SessionConversation) rememberContextWindowModelBase(canonical, effective []*agent.Message) {
	...
	c.contextWindowBase = &contextWindowModelBase{
		cursor: c.cycleCursor, canonical: agentcontext.CloneMessages(canonical), effective: agentcontext.CloneMessages(effective),
	}
	c.cycleMu.Unlock()
}
...
func (c *SessionConversation) providerVisibleCompactionSource(
	...
	if offset < 0 || offset >= len(source) || !reflect.DeepEqual(source[offset], canonicalUser) {
		...
		if len(frozenSource) == 0 || len(source) < len(frozenSource) ||
			!agentcontext.MessagesEqual(source[:len(frozenSource)], frozenSource) ||
...
func (c *SessionConversation) stagedRewindCompactionSource(
	...
	effectivePrefix := append(agentcontext.CloneMessages(boundary.EffectivePrefix), rewindSummary.Clone())
	if !agentcontext.MessagesHavePrefix(modelMessages, effectivePrefix) {
	...
	canonicalBody := agentcontext.CloneMessages(boundary.CanonicalPrefix)
	protectedPrefix, _ := agentcompaction.SplitProtectedPrefix(boundary.EffectivePrefix)
	...
	providerBody := agentcontext.CloneMessages(boundary.EffectivePrefix[injectedPrefixCount:])
	suffix := agentcontext.CloneMessages(modelMessages[len(effectivePrefix):])
	canonicalProjection := append(canonicalBody, rewindSummary.Clone())
	canonicalProjection = append(canonicalProjection, suffix...)
	providerProjection := append(agentcontext.CloneMessages(providerBody), rewindSummary.Clone())
	providerProjection = append(providerProjection, agentcontext.CloneMessages(suffix)...)
...
func (c *SessionConversation) FreezeContextWindowBoundary(messages []*agent.Message) (*session.ContextBoundarySnapshot, error) {
	...
	if !agentcontext.MessagesEqual(messages[baseIndex:], base.effective) {
	...
	canonical := append(agentcontext.CloneMessages(base.canonical), agentcontext.CloneMessages(messages[suffixStart:])...)
