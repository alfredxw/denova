...
func TestPreemptedCheckpointPersistsMutationReceiptsForRecovery(t *testing.T) {
	...
	conversation := NewSessionConversationForAgent(sess, nil, agentrun.AgentKindIDE)
	conversation.BindAgentCycleIdentity(agentrun.CycleIdentity{CommandID: "command-1", OperationID: "operation-1", Cycle: 1})
	assembled, err := conversation.AssembleModelContext(context.Background(), "inspect", agentcontext.ModelContextInput{
	...
	controller := NewContextWindowController(conversation, agentrun.AgentKindIDE).(*runContextWindowController)
	...
	if err := conversation.AppendAssistantWithMetadata("partial", "", session.MessageMetadata{AgentKind: agentrun.AgentKindIDE}); err != nil {
		t.Fatal(err)
	}
	if err := conversation.CommitAgentCycleStage(context.Background(), agentrun.DomainCommitOutput, agentrun.Outcome{Status: agentrun.OutcomePreempted}); err != nil {
		t.Fatal(err)
	}
	active, err := sess.ActiveContextCheckpoints(agentrun.AgentKindIDE)
	if err != nil {
		t.Fatal(err)
	}
	...
	conversation.BindAgentCycleIdentity(agentrun.CycleIdentity{CommandID: "command-2", OperationID: "operation-2", Cycle: 1})
	assembled, err = conversation.AssembleModelContext(context.Background(), "continue", agentcontext.ModelContextInput{
	...
	recovered := NewContextWindowController(conversation, agentrun.AgentKindIDE).(*runContextWindowController)
...
func TestRunContextWindowControllerRewindsToPrefixAndRetainsMutationReceipt(t *testing.T) {
	...
	controller := &runContextWindowController{conversation: backend, agentKind: agentrun.AgentKindIDE}
	...
	exploration := append(agentcontext.CloneMessages(initial),
...
func TestSessionConversationFreezeBoundaryRequiresExactCommittedSuffix(t *testing.T) {
	...
	conversation := &SessionConversation{
		session: sess, agentKind: agentrun.AgentKindIDE,
...
func TestCorruptDurableCheckpointBlocksRecoveredModelRun(t *testing.T) {
	...
	if err := sess.AppendWithMetadata(agent.AssistantMessage("checkpoint", nil), session.MessageMetadata{
		ContextOperations: []session.ContextOperation{{
			Kind: session.ContextOperationCheckpoint, AgentKind: agentrun.AgentKindIDE,
	...
	if _, err := reloaded.ActiveContextCheckpoints(agentrun.AgentKindIDE); err == nil || !strings.Contains(err.Error(), "invalid durable boundary") {
		t.Fatalf("corrupt active checkpoint error = %v", err)
	}
	conversation := NewSessionConversationForAgent(reloaded, nil, agentrun.AgentKindIDE)
	controller := NewContextWindowController(conversation, agentrun.AgentKindIDE)
	if controller == nil {
...
func TestGeneralAgentInstallsContextWindowController(t *testing.T) {
	...
	conversation := NewSessionConversationForAgent(sess, nil, agentrun.AgentKindGeneral)
	if controller := NewContextWindowController(conversation, agentrun.AgentKindGeneral); controller == nil {
...
func TestSessionConversationStagesContextOperationsOnlyWithAuthorizedOutput(t *testing.T) {
	...
	if err := conversation.AppendAssistantWithMetadata("done", "", session.MessageMetadata{AgentKind: agentrun.AgentKindIDE}); err != nil {
		t.Fatal(err)
	}
	if err := conversation.CommitAgentCycleStage(context.Background(), agentrun.DomainCommitOutput, agentrun.Outcome{Status: agentrun.OutcomeFailed}); err != nil {
		t.Fatal(err)
	}
	if got, err := sess.ActiveContextCheckpoints(agentrun.AgentKindIDE); err != nil || len(got) != 0 {
	...
	if got, err := sess.ActiveContextCheckpoints(agentrun.AgentKindIDE); err != nil || len(got) != 1 || got[0].CheckpointID != "cp-1" {
...
func TestModelHistoryAppliesNewestDurableRewindWithoutDeletingTranscript(t *testing.T) {
	...
	if err := sess.AppendWithMetadata(agent.AssistantMessage("checkpoint output", nil), session.MessageMetadata{ContextOperations: []session.ContextOperation{{
		Kind: session.ContextOperationCheckpoint, AgentKind: agentrun.AgentKindIDE, CheckpointID: "cp-1", MessageCount: 1,
	...
	if err := sess.AppendWithMetadata(agent.AssistantMessage("answer after rewind", nil), session.MessageMetadata{ContextOperations: []session.ContextOperation{{
		Kind: session.ContextOperationRewind, AgentKind: agentrun.AgentKindIDE, CheckpointID: "cp-1", MessageCount: 1,
...
func TestModelHistoryChoosesNewestStructuralContextProjection(t *testing.T) {
	...
	compaction := session.ContextCompaction{
		CompactionCheckpoint: agentcompaction.NewCheckpoint(agentrun.AgentKindIDE, agentcompaction.Result{
	...
	conversation := &SessionConversation{agentKind: agentrun.AgentKindIDE}
...
func TestRewindGrowCompactReloadNeverRestoresDiscardedBranch(t *testing.T) {
	...
	summarize := func(
		_ context.Context,
		_ *config.Config,
		request agentcompaction.SummaryRequest,
		_ func(int, string),
	) (string, error) {
		joined := joinedContextMessageContent(request.Messages)
		...
		return "## Current state\nThe rewind finding and grown safe turn are preserved.", nil
	}
	...
	transient, result, err := conversation.CompactContextIfNeeded(context.Background(), coldCompactionTestInput(agentcompaction.Input{
	...
	if _, err := sess.AppendContextCompaction(session.ContextCompaction{
		CompactionCheckpoint: agentcompaction.NewCheckpoint(agentrun.AgentKindIDE, result),
...
func TestDurableCheckpointRecoveryUsesFrozenCompactedProjection(t *testing.T) {
	...
	first := NewSessionConversationForAgent(sess, nil, agentrun.AgentKindIDE)
	first.BindAgentCycleIdentity(agentrun.CycleIdentity{CommandID: "checkpoint-command", OperationID: "checkpoint-operation", Cycle: 1})
	assembled, err := first.AssembleModelContext(context.Background(), "inspect", agentcontext.ModelContextInput{
	...
	controller := NewContextWindowController(first, agentrun.AgentKindIDE).(*runContextWindowController)
	if _, err := controller.BeforeModel(context.Background(), assembled.Messages); err != nil {
		t.Fatal(err)
	}
	...
	if err := first.AppendAssistantWithMetadata("partial before restart", "", session.MessageMetadata{AgentKind: agentrun.AgentKindIDE}); err != nil {
		t.Fatal(err)
	}
	if err := first.CommitAgentCycleStage(context.Background(), agentrun.DomainCommitOutput, agentrun.Outcome{Status: agentrun.OutcomePreempted}); err != nil {
		t.Fatal(err)
	}
	...
	second := NewSessionConversationForAgent(reloaded, nil, agentrun.AgentKindIDE)
	second.BindAgentCycleIdentity(agentrun.CycleIdentity{CommandID: "rewind-command", OperationID: "rewind-operation", Cycle: 1})
	current, err := second.AssembleModelContext(context.Background(), "continue", agentcontext.ModelContextInput{
	...
	recovered := NewContextWindowController(second, agentrun.AgentKindIDE).(*runContextWindowController)
	...
	if err := second.AppendAssistantWithMetadata("final after restart", "", session.MessageMetadata{AgentKind: agentrun.AgentKindIDE}); err != nil {
		t.Fatal(err)
	}
	if err := second.CommitAgentCycleStage(context.Background(), agentrun.DomainCommitOutput, agentrun.Outcome{Status: agentrun.OutcomeCompleted}); err != nil {
		t.Fatal(err)
	}

	snapshot, err := reloaded.SnapshotContext(agentrun.AgentKindIDE)
	if err != nil {
		t.Fatal(err)
	}
