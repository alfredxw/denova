...
var ErrStructuralRestoreUnavailable = errors.New("agent execution structural restore dependency is unavailable")
...
func (e *durableEngine) restoreStructuralOperation(
	ctx context.Context,
	...
	if err := e.pinAccepted(ref); err == nil {
		return nil
	} else if !errors.Is(err, ErrCycleSpecNotFound) {
		return err
	}

	profile, err := e.profiles.profile(snapshot.Binding.Profile)
	if err != nil {
		return fmt.Errorf("%w: %v", ErrStructuralRestoreUnavailable, err)
	}
	structural, ok := profile.(StructuralProfile)
	if !ok {
		return fmt.Errorf("%w: profile %q has no structural capability", ErrStructuralRestoreUnavailable, profile.ID())
	}
	slog.InfoContext(ctx, fmt.Sprintf(
		"[agent-execution] rebuilding cold structural operation binding=%+v command_id=%s operation_id=%s kind=%s spec_ref=%s",
		snapshot.Binding,
		snapshot.CommandID,
	...
	restored, err := structural.RestoreStructural(ctx, request)
	if err != nil {
		return fmt.Errorf("%w: %v", ErrStructuralRestoreUnavailable, err)
	}
	...
	registration, err := e.register(ref, command, cycleSpec{
	...
	registration.accept()
	slog.InfoContext(ctx, fmt.Sprintf(
		"[agent-execution] cold structural operation registered and pinned binding=%+v command_id=%s operation_id=%s action=%s",
		snapshot.Binding,
		snapshot.CommandID,
		snapshot.OperationID,
		action,
...
func contextStructuralBindingOptions(binding runstate.BindingRef, options agentrun.Options) (agentrun.Options, error) {
	...
	options.AgentKind = productBinding.AgentKind
	...
	options.AutomationTaskID = productBinding.TaskID
	options.Mode, err = structuralMode(ProfileID(binding.Profile), productBinding.Mode)
	if err != nil {
		return agentrun.Options{}, err
	}
...
func structuralMode(profile ProfileID, bindingMode string) (string, error) {
	switch profile {
	case ProfileWriting:
		return "ide", nil
	case ProfileAgentChat:
		return agentrun.ModeAgentChat, nil
	case ProfileGame, ProfileDirector:
		return "interactive", nil
	case ProfileAutomation, ProfileConfigManager, ProfileImage:
		return bindingMode, nil
	default:
		return "", fmt.Errorf("%w: unsupported structural profile %q", ErrProfileInvalid, profile)
	}
}
...
func (s *Runtime) ResumeRecoveredStructuralOperation(
	ctx context.Context,
	options agentrun.Options,
...
func (h *coordinator) resumeRecoveredContextStructuralOperation(
	...
	slog.InfoContext(ctx, fmt.Sprintf(
		"[agent-execution] explicitly resuming recovery-paused structural operation binding=%+v command_id=%s operation_id=%s action=%s",
		snapshot.Binding,
		snapshot.CommandID,
		snapshot.OperationID,
		action,
	...
	slog.InfoContext(ctx, fmt.Sprintf(
		"[agent-execution] recovered structural operation settled binding=%+v command_id=%s operation_id=%s action=%s",
		snapshot.Binding,
		snapshot.CommandID,
		snapshot.OperationID,
		action,
