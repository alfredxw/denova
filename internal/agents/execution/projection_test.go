...
func TestDefaultExecutionRuntimeUsesDurableMemory(t *testing.T) {
...
func TestForegroundWorkspaceClosePreservesAgentChatBindings(t *testing.T) {
	...
	writingRuntime, err := service.coordinator.runtime.Open(context.Background(), writingRef)
	if err != nil {
		t.Fatal(err)
	}
	agentChatRuntime, err := service.coordinator.runtime.Open(context.Background(), agentChatRef)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := writingRuntime.Status(context.Background()); !errors.Is(err, runstate.ErrHarnessClosed) {
	...
	if err != nil || stillAgentChat != agentChatRuntime {
		t.Fatalf("AgentChat binding changed during foreground close: same=%t err=%v", stillAgentChat == agentChatRuntime, err)
	}

	if err := service.CloseAgentChatSessionBindings(context.Background(), "/book", "agent-chat-session"); err != nil {
		t.Fatal(err)
	}
	if _, err := agentChatRuntime.Status(context.Background()); !errors.Is(err, runstate.ErrHarnessClosed) {
