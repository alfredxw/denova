...
func TestResumeRecoveredContextStructuralOperationColdRestoresExactPlanOnce(t *testing.T) {
	...
	restoreCalls := 0
	service, err := newRuntimeWithOptions(context.Background(), agentrun.DefaultLoopPolicy(), store, runtimeOptions{
		profiles: []Profile{&testExecutionProfile{id: ProfileWriting, structural: func(_ context.Context, request StructuralRestoreRequest) (agentstructural.Spec, error) {
			restoreCalls++
