...
func TestExecutionDomainCommitBarrierAbortBeforeOutputIntentKeepsUserOnly(t *testing.T) {
	conversation, sess := newDomainCommitBarrierSessionConversation(t)
	...
	options := domainCommitBarrierRunOptions(sess.ID)
	done := runDomainCommitBarrierExecution(t, service, conversation, options)

	waitEngineSignal(t, conversation.beforeOutputIntent, "pending output intent")
	harness := openDomainCommitBarrierBinding(t, service, options)
	operationID := activeDomainCommitBarrierOperation(t, harness)
...
func TestExecutionDomainCommitBarrierOutputIntentBeforeAbortCommitsAndWins(t *testing.T) {
	conversation, sess := newDomainCommitBarrierSessionConversation(t)
	...
	waitEngineSignal(t, conversation.outputAuthorized, "authorized output intent")
...
func TestExecutionCallerCancelWaitsWhenOutputCommitAlreadyFinalizing(t *testing.T) {
	conversation, sess := newDomainCommitBarrierSessionConversation(t)
	conversation.outputAuthorized = make(chan struct{})
	conversation.releaseOutputCommit = make(chan struct{})
	defer conversation.releaseCommit()
	...
	runOutcomeTestGoroutine(done, "domain commit barrier run", func() agentrun.Outcome {
		return runCycle(service, ctx, runner, conversation, nil, agentchat.ChatRequest{CommandID: "domain-commit-barrier", Message: "user input"}, options, nil)
	})
...
type domainCommitBarrierConversation struct {
...
func (c *domainCommitBarrierConversation) PendingAgentCycleCommit(stage agentrun.DomainCommitStage) (agentrun.DomainCommitIntent, bool, error) {
...
func (c *domainCommitBarrierConversation) CommitAgentCycleStage(ctx context.Context, stage agentrun.DomainCommitStage, outcome agentrun.Outcome) error {
...
func (c *domainCommitBarrierConversation) releaseIntent() {
	if c.releaseOutputIntent != nil {
		c.releaseIntentOnce.Do(func() { close(c.releaseOutputIntent) })
	}
}

func (c *domainCommitBarrierConversation) releaseCommit() {
	if c.releaseOutputCommit != nil {
		c.releaseCommitOnce.Do(func() { close(c.releaseOutputCommit) })
	}
}

func newDomainCommitBarrierSessionConversation(t *testing.T) (*domainCommitBarrierConversation, *session.Session) {
	t.Helper()
	...
	return &domainCommitBarrierConversation{SessionConversation: agentconversation.NewSessionConversation(sess)}, sess
}

func newDomainCommitBarrierRuntime(t *testing.T) *Runtime {
	t.Helper()
...
func domainCommitBarrierRunOptions(sessionID string) agentrun.Options {
...
func runDomainCommitBarrierExecution(t *testing.T, service *Runtime, conversation agentchat.Conversation, options agentrun.Options) <-chan agentrun.Outcome {
	t.Helper()
	done := make(chan agentrun.Outcome, 1)
	runner := newRunControlTestRunner(t, &runControlFixedModel{message: agent.AssistantMessage("canonical answer", nil)}, true)
...
func openDomainCommitBarrierBinding(t *testing.T, service *Runtime, options agentrun.Options) *runstate.Harness {
	t.Helper()
...
func activeDomainCommitBarrierOperation(t *testing.T, harness *runstate.Harness) runstate.OperationID {
	t.Helper()
	observation, err := harness.ObserveFromNow(context.Background())
	if err != nil {
		t.Fatal(err)
