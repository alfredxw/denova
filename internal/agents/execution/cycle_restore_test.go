...
func TestDurableExecutionRestoresAcceptedNextTurnAcrossReopen(t *testing.T) {
	dataDir := t.TempDir()
	first, err := NewDurableRuntime(context.Background(), dataDir)
	if err != nil {
		t.Fatal(err)
	}
	...
	startedOperation, err := first.Start(context.Background(), StartRequest{Cycle: Cycle{
		...
		Request:      agentchat.ChatRequest{CommandID: "restore-start", Message: "first"},
		Options:      options,
	}})
	if err != nil {
		t.Fatal(err)
	}
	started := startedOperation.Receipt()
	waitEngineSignal(t, blocking.started, "initial turn preparation")
	nextRequest := agentchat.ChatRequest{Message: "second", Locale: "zh-CN"}
	next, err := first.SubmitCommand(context.Background(), CommandRequest{
		...
		Request: nextRequest, Options: options,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	restoredRequest := make(chan CycleRestoreRequest, 1)
	restoredConversation := &runControlConversation{}
	second, err := NewDurableRuntime(
		context.Background(),
		dataDir,
		WithProfiles(&testExecutionProfile{id: ProfileWriting, prepare: func(_ context.Context, request CycleRestoreRequest) (Cycle, error) {
	...
	replaySpec := CommandRequest{
		Kind: CommandNextTurn, CommandID: "restore-next", AfterOperationID: started.OperationID,
		Request: nextRequest, Options: options.Normalize(""),
	}
	replayInput := runstate.UserInput{
		Text:        nextRequest.Message,
		ContextRefs: commandContextRefs(nextRequest),
		TurnSpecRef: commandCycleRef(
			binding,
			replaySpec.CommandID,
			cycleSemanticFingerprint(replaySpec),
		),
	}
	replayInput, err = withInputMaterializationDescriptor(replayInput, replaySpec)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDecodeExecutionTurnRestoreRequestSupportsTransientQueueCommands(t *testing.T) {
	options := agentrun.Options{
		AgentKind: agentrun.AgentKindIDE, RootAgentName: "restore-test",
		Workspace: "/book", SessionID: "restore-session", Mode: "writing",
	}.Normalize("")
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			input := runstate.UserInput{
				...
				ContextRefs: commandContextRefs(spec.Request),
				TurnSpecRef: commandCycleRef(binding, spec.CommandID, cycleSemanticFingerprint(spec)),
			}
			...
			request, err := decodeCycleRestoreRequest(bindingRef, queued)
			if err != nil {
				t.Fatal(err)
			}
...
func TestDecodeExecutionTurnRestoreRequestPreservesProjectIdentity(t *testing.T) {
	t.Parallel()
	...
	input := runstate.UserInput{
		Text: spec.Request.Message,
		TurnSpecRef: commandCycleRef(
			binding, spec.CommandID, cycleSemanticFingerprint(spec),
		),
	}
	input, err = withInputMaterializationDescriptor(input, spec)
	if err != nil {
	...
	request, err := decodeCycleRestoreRequest(bindingRef, runstate.QueuedInput{
		CommandID: runstate.CommandID(spec.CommandID), OperationID: runstate.OperationID(spec.OperationID),
