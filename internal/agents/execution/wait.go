...
func (h *coordinator) waitForOperation(
	...
	for terminal == nil || settlement == nil {
		...
		case event, ok := <-events:
			...
			case runstate.OperationInterruptedEvent:
				if payload.OperationID == receipt.OperationID {
					...
					failed := agentrun.NewOutcome(agentrun.OutcomeFailed, err, payload.Reason, "", "")
					terminal = &failed
		...
		case observationErr, ok := <-errorsCh:
			...
			if observationErr != nil {
				...
				return agentrun.NewOutcome(agentrun.OutcomeFailed, observationErr, observationErr.Error(), outputContent(terminal), outputThinking(terminal))
			}
		case <-callerDone:
			callerDone = nil
			...
			_, err := harness.Submit(h.lifecycle, runstate.Abort{
				ID:          runstate.CommandID(newOperationIdentity("command")),
			...
			if err != nil &&
				...
				return agentrun.NewOutcome(agentrun.OutcomeFailed, err, err.Error(), outputContent(terminal), outputThinking(terminal))
			}
		case <-h.lifecycle.Done():
			...
			return agentrun.NewOutcome(agentrun.OutcomeAborted, err, err.Error(), outputContent(terminal), outputThinking(terminal))
		}
		...
		if settlement != nil && terminal == nil {
			...
			// durableEngine publishes Outcome before returning EngineDone, so a
			...
			default:
				...
				failed := agentrun.NewOutcome(agentrun.OutcomeFailed, err, err.Error(), "", "")
				terminal = &failed
			}
		}
	}
...
func replayedTerminalFromSnapshot(
	...
	receipt runstate.Receipt,
) (*agentrun.Outcome, *runstate.OperationSettledEvent) {
...
func outcomeFromOperationStatus(status runstate.OperationStatus, reason, content, thinking string) agentrun.Outcome {
	switch status {
	case runstate.OperationSucceeded:
		return agentrun.NewOutcome(agentrun.OutcomeCompleted, nil, "", content, thinking)
	case runstate.OperationAborted:
		return agentrun.NewOutcome(agentrun.OutcomeAborted, nil, strings.TrimSpace(reason), content, thinking)
	case runstate.OperationFailed, runstate.OperationInterrupted:
		reason = strings.TrimSpace(reason)
		if reason == "" {
			reason = "agent operation failed"
		}
		return agentrun.NewOutcome(agentrun.OutcomeFailed, errors.New(reason), reason, content, thinking)
	default:
		err := fmt.Errorf("unsupported agent operation status %q", status)
		return agentrun.NewOutcome(agentrun.OutcomeFailed, err, err.Error(), content, thinking)
	}
}
...
func outcomeForSettlement(outcome agentrun.Outcome, status runstate.OperationStatus) agentrun.Outcome {
	switch status {
	case runstate.OperationSucceeded:
		outcome.Status = agentrun.OutcomeCompleted
		outcome.Error = nil
		outcome.Reason = ""
		return outcome
	case runstate.OperationAborted:
	...
	case runstate.OperationFailed, runstate.OperationInterrupted:
		...
		outcome.Status = agentrun.OutcomeFailed
		return outcome
	default:
		err := fmt.Errorf("unsupported agent operation status %q", status)
		return agentrun.NewOutcome(agentrun.OutcomeFailed, err, err.Error(), outcome.Content, outcome.Thinking)
	}
}
...
func outputContent(outcome *agentrun.Outcome) string {
	if outcome == nil {
		return ""
	}
...
func outputThinking(outcome *agentrun.Outcome) string {
	if outcome == nil {
		return ""
	}
