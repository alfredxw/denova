...
func (s *Runtime) OpenRecoveryObservation(
	ctx context.Context,
	options agentrun.Options,
) (*RecoveryObservation, error) {
	harness, binding, err := s.openRecoveryBinding(ctx, options)
	if err != nil {
		return nil, err
	}
...
func (s *Runtime) openRecoveryBinding(
	ctx context.Context,
	options agentrun.Options,
...
func recoveryDisplayRequest(binding runstate.BindingRef, input runstate.UserInput) (CycleRestoreRequest, bool) {
	if len(input.RestoreDescriptor) == 0 {
		return CycleRestoreRequest{}, false
	}
	var descriptor cycleRestoreDescriptor
	if err := json.Unmarshal(input.RestoreDescriptor, &descriptor); err != nil || descriptor.Version != cycleRestoreDescriptorVersion {
		return CycleRestoreRequest{}, false
	}
	...
	return CycleRestoreRequest{Request: request, Options: options}, true
}
