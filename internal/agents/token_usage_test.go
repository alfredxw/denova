...
func TestRunTokenUsageCollectorRecordsToolContext(t *testing.T) {
	...
	collector.AddMessage(&agent.Message{
		ToolCalls: []agent.ToolCall{{
			ID:       "call-1",
			Function: agent.FunctionCall{Name: "write_workspace_file"},
		}},
		ResponseMeta: &agent.ResponseMeta{
			FinishReason: "tool_calls",
			Usage: &agent.TokenUsage{
				PromptTokens:       100,
				PromptTokenDetails: agent.PromptTokenDetails{CachedTokens: 40},
				CompletionTokens:   20,
				TotalTokens:        120,
			},
		},
	...
	collector.AddMessage(&agent.Message{
		ResponseMeta: &agent.ResponseMeta{
			...
			Usage: &agent.TokenUsage{
				PromptTokens:       80,
				PromptTokenDetails: agent.PromptTokenDetails{CachedTokens: 100},
				CompletionTokens:   10,
				TotalTokens:        90,
			},
		},
