...
func buildIDEContextAnalysisForTest(t *testing.T, cfg *config.Config, state *book.State, teller IDEStoryTeller, bookService *book.Service, messages []*agent.Message, compaction *session.ContextCompaction, pending *session.Interruption, req ChatRequest) (ContextAnalysis, *SessionConversation) {
	t.Helper()
...
func TestIDEContextAnalysisSplitsStableAndDynamicWorkspaceState(t *testing.T) {
	...
	for _, part := range analysis.SystemPromptParts {
		if part.Source == book.LoreItemsRelativePath {
