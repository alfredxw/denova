...
func TestExplicitSkillsAreLoadedBeforeFirstModelCallFromAnywhereInMessage(t *testing.T) {
	...
	skillTool, err := producttools.NewSkill(context.Background(), backend, ContextBudgetForAgent(cfg, config.AgentKindIDE).MaxFragmentBytes)
	if err != nil {
		t.Fatal(err)
	}
	...
	outcome := service.RunWithOptions(
		...
		conversation,
		nil,
		ChatRequest{CommandID: "explicit-skills-anywhere", Message: message},
		RunOptions{
...
func TestResolveExplicitSkillInvocationsSupportsInteractiveStoryAndToolPolicy(t *testing.T) {
	...
	resolved, err := ResolveExplicitSkillInvocations(
...
func TestIDEContextAnalysisIncludesTheSameExplicitSkillBodiesAsRuntime(t *testing.T) {
	...
	analysis, _ := buildIDEContextAnalysisForTest(
		t, cfg, nil, IDEStoryTeller{}, nil, nil, nil, nil, ChatRequest{Message: message},
	)
...
func TestExplicitSkillFailsClosedWhenTheTurnContextCannotFitIt(t *testing.T) {
	...
	_, err = prepareTurnContext(context.Background(), turnContextPreparationInput{
		...
		Request:      ChatRequest{Message: "请使用 /alpha"},
		Environment:  newTurnRuntimeEnvironment(cfg.Workspace),
	})
	if err == nil || !strings.Contains(err.Error(), `显式 Skill "alpha" 未能完整进入模型上下文`) {
...
func (c *explicitSkillEventCollector) Append(event Event) {
	c.mu.Lock()
	c.events = append(c.events, event)
	c.mu.Unlock()
}

func (c *explicitSkillEventCollector) Snapshot() []Event {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (m *explicitSkillCaptureModel) Captured() ([]*agent.Message, []Event, int) {
	m.mu.Lock()
	defer m.mu.Unlock()
	return cloneAgentMessages(m.input), append([]Event(nil), m.eventsAtFirstCall...), m.calls
}
...
func visibleSkillEventSequence(events []Event) []string {
	var sequence []string
	for _, event := range events {
		...
		if !ok || eventDataString(data, "name") != "skill" {
			continue
		}
		name := ""
		if event.Type == "tool_call" {
			...
			_ = json.Unmarshal([]byte(eventDataString(data, "args")), &args)
			name = args.Name
		} else {
			content := eventDataString(data, "content")
...
func assertExplicitSkillToolArgs(t *testing.T, events []Event, want []string) {
	t.Helper()
	var got []string
	for _, event := range events {
		if event.Type != "tool_call" {
		...
		if err := json.Unmarshal([]byte(eventDataString(data, "args")), &args); err != nil {
