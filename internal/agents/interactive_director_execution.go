...
func (c *singleInstructionConversation) ContextLedgerParts() []ContextLedgerPart {
	...
	return c.ContextLedgerPartsForMessages([]*agent.Message{agent.UserMessage(stableMessage)})
}

func (c *singleInstructionConversation) ContextLedgerPartsForMessages(messages []*agent.Message) []ContextLedgerPart {
	if c == nil || strings.TrimSpace(c.stableContext) == "" {
		return nil
	}
	stableMessage := c.stableContextModelMessage()
