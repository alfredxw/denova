...
func TestToolExecutionGateSerializesWritesAcrossMiddlewareInstances(t *testing.T) {
	...
	firstMiddleware := &OrchestratorMiddleware{toolSettings: settings, enforceToolSettings: true, executionGate: gateA}
	secondMiddleware := &OrchestratorMiddleware{toolSettings: settings, enforceToolSettings: true, executionGate: gateB}
...
func mustWrapGateTestEndpoint(t *testing.T, middleware *OrchestratorMiddleware, name string, endpoint testTextToolEndpoint) testTextToolEndpoint {
	t.Helper()
