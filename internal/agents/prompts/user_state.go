...
// child Agent instruction. Root Agents receive the same content from their
// ContextSource so live edits do not change durable Definition identity.
func AppendUserStatePrompt(cfg *config.Config, composition SystemPromptComposition, content string) (SystemPromptComposition, error) {
	...
	fragments = append(fragments, SystemPromptFragment{
		...
		Purpose:  "apply user-managed Agent behavior and preferences",
		Content:  content,
		Prefix:   "\n\n---\n\n# User State Prompt\n\n",
