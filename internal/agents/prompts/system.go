...
- %s/setting/lore/items.json — user-visible authoritative structured lore containing long-lived facts about characters, worldbuilding, locations, factions, rules, and items. Maintain it through the WebUI lore interface or the /configuration Skill with config_read/config_apply; never edit the JSON directly.
