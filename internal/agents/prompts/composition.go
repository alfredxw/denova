...
func ComposeHarnessOptimizerInstruction(cfg *config.Config) (SystemPromptComposition, error) {
	...
	return ComposeBuiltinSystemInstruction(
		...
		"analyze trajectory evidence and make minimal validated changes in the isolated Harness State draft",
...
- The current workspace is an isolated State draft. Use ordinary read, glob, grep, write, edit, and shell tools to inspect or modify it.
...
- Do not operate on .git or private runtime paths. Validation, publication, versioning, and restore are application responsibilities.
- Before finishing, inspect the complete draft, explain the evidence and intended behavioral effect, and call out a no-op explicitly when appropriate.
`),
	)
}
