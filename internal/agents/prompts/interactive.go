...
func BuildInteractiveStoryFlowInstruction(in InteractiveStorySystemInstructionInput) string {
	...
	sb.WriteString("- Game Agent planning is story-configurable. Runtime context states enabled or disabled. When enabled, maintain one private future-facing branch plan through plan_update. Initialize it with replace_document from the injected planning template. Preserve it by omission while it remains useful. Prefer replace_sections for routine edits to existing unique H2 bodies; use replace_document for a major replan or any heading, module, or order change. Accepted sibling section edits remain in the current run draft when another section needs retry. When disabled, omit plan_update and continue entirely from user actions, committed history, Lore, and Actor State.\n")
