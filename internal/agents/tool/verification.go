...
func VerifyPostRunMutations(bookService *book.Service, mutations []Mutation) Verification {
	result := Verification{
		Status:    "skipped",
		Mutations: len(mutations),
	}
	if len(mutations) == 0 {
		result.Checks = append(result.Checks, VerificationCheck{Type: "mutation_scan", Status: "skipped", Message: "no workspace mutations observed"})
		return result
	}
	if bookService == nil || strings.TrimSpace(bookService.Workspace()) == "" {
		...
		result.Checks = append(result.Checks, VerificationCheck{Type: "workspace", Status: "warning", Message: "workspace unavailable"})
		return result
	}
	...
	if len(result.Checks) == 0 {
		result.Checks = append(result.Checks, VerificationCheck{Type: "mutation_scan", Status: "skipped", Message: "no workspace mutation requested post-check verification"})
		return result
	}
	result.Status = "ok"
...
func verifyMutation(bookService *book.Service, mutation Mutation) []VerificationCheck {
	...
	case deletion && statErr == nil:
		checks = append(checks, VerificationCheck{Type: "path_exists", Target: target, Status: "warning", Message: "delete-like tool returned but target still exists"})
	case statErr != nil:
		checks = append(checks, VerificationCheck{Type: "path_exists", Target: target, Status: "warning", Message: statErr.Error()})
	default:
		kind := "file"
		if info.IsDir() {
			kind = "directory"
		}
		checks = append(checks, VerificationCheck{Type: "path_exists", Target: target, Status: "ok", Message: kind + " exists"})
	}
	if strings.HasPrefix(relativeTarget, "chapters/") && !isChapterContentPath(relativeTarget) {
		checks = append(checks, VerificationCheck{Type: "chapter_path", Target: target, Status: "warning", Message: "chapter writes should use .md or .txt files under chapters/"})
	}
	if relativeTarget == "setting/character-states.md" || relativeTarget == "progress.md" {
		checks = append(checks, VerificationCheck{Type: "state_sync", Target: target, Status: "ok", Message: "tracked writing-state file"})
	}
	return checks
}
...
func verifyLoreMutation(workspace string, mutation Mutation) []VerificationCheck {
	store := lore.NewStore(workspace)
	items, err := store.List()
	if err != nil {
		return []VerificationCheck{{Type: "lore_store", Status: "warning", Message: err.Error()}}
	}
	...
	for _, id := range mutation.LoreItemIDs {
		item, ok := byID[id]
		if !ok {
			checks = append(checks, VerificationCheck{Type: "lore_item", Target: id, Status: "warning", Message: "changed lore item not found after write"})
			continue
		}
		if strings.TrimSpace(item.BriefDescription) == "" {
			checks = append(checks, VerificationCheck{Type: "lore_brief", Target: id, Status: "warning", Message: "lore item is missing brief_description"})
			continue
		}
		checks = append(checks, VerificationCheck{Type: "lore_brief", Target: id, Status: "ok", Message: "brief_description present"})
	}
	for _, id := range mutation.DeletedLoreItemIDs {
		if _, ok := byID[id]; ok {
			checks = append(checks, VerificationCheck{Type: "lore_delete", Target: id, Status: "warning", Message: "deleted lore item still exists"})
			continue
		}
		checks = append(checks, VerificationCheck{Type: "lore_delete", Target: id, Status: "ok", Message: "deleted lore item is absent"})
	}
	if len(checks) == 0 {
		checks = append(checks, VerificationCheck{Type: "lore_write", Status: "ok", Message: "lore write completed"})
	}
	return checks
}
