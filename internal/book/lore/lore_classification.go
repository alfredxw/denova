...
const (
	ClassificationModeHeuristic = "heuristic"
	ClassificationModeSemantic  = "semantic"
...
// ClassificationInput is the bounded semantic-classification payload.
...
type SemanticClassifier func([]ClassificationInput) ([]ClassificationSuggestion, error)
	...
	for _, signal := range loreTypeSignals {
		for _, prefix := range signal.Prefixes {
			if loreNameHasTypeSignal(name, prefix) {
				return ClassificationSuggestion{ID: input.ID, Type: signal.Type, Confidence: ClassificationConfidenceHigh, Reason: "名称包含明确类型信号：" + prefix}
			}
		}
	}
...
// NormalizeClassificationMode accepts the supported classification mode and
// falls back to deterministic local heuristics.
func NormalizeClassificationMode(value string) string {
	if strings.EqualFold(strings.TrimSpace(value), ClassificationModeSemantic) {
		return ClassificationModeSemantic
	}
