...
func TestServiceImportTavernCharacterCardCreatesLoreItems(t *testing.T) {
	...
	result, err := service.ImportTavernCard("liuyun.json", []byte(`{
		"spec": "chara_card_v2",
		"data": {
			"name": "柳云",
			"description": "负责整理情报",
...
func TestServiceImportTavernCharacterCardAddsNumericSuffixForDuplicateLoreNames(t *testing.T) {
	...
	if _, err := store.Create(lore.ItemInput{Type: "world", Name: "秘密", Importance: "important", Content: "既有资料"}); err != nil {
		t.Fatal(err)
	}
...
func TestServiceImportTavernCharacterCardImportsPNGCoverOpeningsAndUserPlaceholder(t *testing.T) {
	...
	result, err := service.ImportTavernCard("fengjiangyue.png", png, ImportOptions{UserCharacterName: "韩澈"})
	if err != nil {
	...
	allItems, err := lore.NewStore(workspace).ListAll()
	if err != nil {
...
func TestPreviewTavernCharacterCardReportsCompatibility(t *testing.T) {
	preview, err := PreviewTavernCard("card.json", []byte(`{
		"data": {
			"name": "谢眠",
			"first_mes": "开场",
...
func TestTavernWorldbookIsNormalizedWithoutLoadingEngine(t *testing.T) {
	workspace := t.TempDir()
	result, err := NewService(workspace).ImportTavernCard("card.json", []byte(`{
	...
	if location.Type != "location" || location.LoadMode != lore.LoadModeResident {
	...
	searchIndex, err := store.SearchIndexMarkdown(lore.IndexOptions{Keywords: []string{"旧港"}, Match: lore.IndexMatchAny})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCharacterCardImportSemanticallyClassifiesOnlyUncertainEntries(t *testing.T) {
	workspace := t.TempDir()
	called := 0
	result, err := NewService(workspace).ImportTavernCard("semantic.json", []byte(`{
		...
		ClassifyLore: func(inputs []lore.ClassificationInput) ([]lore.ClassificationSuggestion, error) {
			...
			return []lore.ClassificationSuggestion{{ID: inputs[0].ID, Type: "character", Confidence: lore.ClassificationConfidenceHigh, Reason: "正文描述人物职责"}}, nil
		},
	})
	if err != nil {
		t.Fatal(err)
	}
	if called != 1 || result.ClassificationMode != lore.ClassificationModeSemantic || result.UncertainTypeCount != 0 || result.ClassificationCounts["character"] != 1 || result.ClassificationCounts["location"] != 1 {
	...
	for _, item := range items {
		if item.Name == "沈凝" && (item.Type != "character" || item.TypeSource != lore.TypeSourceSemantic) {
			t.Fatalf("semantic type provenance not persisted: %#v", item)
		}
		if item.Name == "地点：旧港" && item.TypeSource != lore.TypeSourceHeuristic {
...
func TestCharacterCardImportDoesNotPersistLowConfidenceSemanticSuggestion(t *testing.T) {
	workspace := t.TempDir()
	result, err := NewService(workspace).ImportTavernCard("low-confidence.json", []byte(`{
		"data":{"name":"归舟","description":"旅者","character_book":{"entries":[
		...
		ClassificationMode: lore.ClassificationModeSemantic,
		ClassifyLore: func(inputs []lore.ClassificationInput) ([]lore.ClassificationSuggestion, error) {
			return []lore.ClassificationSuggestion{{ID: inputs[0].ID, Type: "character", Confidence: lore.ClassificationConfidenceLow, Reason: "证据不足"}}, nil
		},
	})
	if err != nil {
		t.Fatal(err)
...
func TestPNGPrefersCCV3AndWarnsOnConflict(t *testing.T) {
	...
	preview, err := PreviewTavernCard("conflict.png", png)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCharacterCardImportAboveRecommendationWarnsButSucceeds(t *testing.T) {
	...
	preview, err := PreviewTavernCard("budget.json", raw)
	if err != nil {
		t.Fatal(err)
	}
	...
	result, err := NewService(workspace).ImportTavernCard("budget.json", raw)
	if err != nil {
