...
var catalogZH = map[string]string{
	...
	"api.characterCard.residentLimitRequired": "常驻资料需要 {{required}} KB，当前上限为 {{current}} KB；请确认提升上限后再导入。",
	"api.characterCard.residentLimitTooLarge": "常驻资料需要 {{required}} KB，超过最大上限 {{maximum}} KB。",
...
var catalogEN = map[string]string{
	...
	"api.characterCard.residentLimitRequired": "Resident lore requires {{required}} KB; the current limit is {{current}} KB. Confirm the increase before importing.",
	"api.characterCard.residentLimitTooLarge": "Resident lore requires {{required}} KB, exceeding the {{maximum}} KB maximum.",
