...
var catalogZH = map[string]string{
	...
	"api.chat.contextHandoffTooLarge":         "上下文交接过大（{{bytes}} bytes），上限为 {{limit}} bytes",
...
var catalogEN = map[string]string{
	...
	"api.chat.contextHandoffTooLarge":         "Context handoff is too large ({{bytes}} bytes); the limit is {{limit}} bytes.",
