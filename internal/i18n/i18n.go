...
var catalogZH = map[string]string{
	...
	"api.access.localHostEffect":              "系统目录选择器只能从运行 Denova 的本机打开",
	...
	"api.conversationConfig.active":           "当前会话正在运行，结束后才能修改模型、思考强度或权限等级",
	...
	"api.projectFiles.listFailed":             "读取项目目录失败: {{detail}}",
...
var catalogEN = map[string]string{
	...
	"api.access.localHostEffect":              "The system folder picker can only be opened from the machine running Denova.",
	...
	"api.conversationConfig.active":           "This conversation is running. Change its model, thinking effort, or permission level after it finishes.",
	...
	"api.projectFiles.listFailed":             "Failed to read the project directory: {{detail}}",
