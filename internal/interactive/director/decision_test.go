...
func TestParsePlanDecisionJSONNormalizesStructuredOutput(t *testing.T) {
	decision, err := ParseDecisionJSON(`{"mode":"replan","triggers":["major_deviation"],"deviation":{"level":"major","invalidated_plan_refs":["beat-2"]},"reason":"关键前提失效"}`)
	if err != nil {
		t.Fatal(err)
	}
	if decision.Mode != DecisionReplan || decision.Deviation.Level != "major" || len(decision.Deviation.InvalidatedPlanRefs) != 1 {
...
func TestParsePlanDecisionJSONExtractsOneObjectAndRejectsNarrationOnly(t *testing.T) {
	decision, err := ParseDecisionJSON("I reviewed the plan.\n```json\n{\"mode\":\"replan\",\"reason\":\"opening plan required\"}\n```")
	if err != nil {
		t.Fatalf("parse fenced decision: %v", err)
	}
	if decision.Mode != DecisionReplan || decision.Reason != "opening plan required" {
		t.Fatalf("unexpected normalized decision: %#v", decision)
	}

	if _, err := ParseDecisionJSON("I updated the plan files."); err == nil || !strings.Contains(err.Error(), "PlanDecision") {
...
func TestParsePlanDecisionJSONRejectsMultipleValidObjects(t *testing.T) {
	...
	if _, err := ParseDecisionJSON(output); err == nil || !strings.Contains(err.Error(), "multiple") {
...
func TestParsePlanDecisionJSONDoesNotPromoteNestedObject(t *testing.T) {
	...
	if _, err := ParseDecisionJSON(output); err == nil {
...
func TestParsePlanDecisionJSONHandlesBracesInsideStrings(t *testing.T) {
	decision, err := ParseDecisionJSON(`prefix {"mode":"patch","reason":"preserve {this} text"} suffix`)
	if err != nil {
		t.Fatalf("parse decision containing braces in a string: %v", err)
	}
	if decision.Mode != DecisionPatch || decision.Reason != "preserve {this} text" {
