...
func NewDirectorPlanDomainCommitIntent(identity DirectorPlanDomainCommitIdentity, token DirectorPlanRunToken, sourceTurnID, summary string, docs DirectorPlanDocs) (DirectorPlanDomainCommitIntent, error) {
	...
	if identity.CommandID == "" || identity.OperationID == "" || identity.Cycle <= 0 {
		return DirectorPlanDomainCommitIntent{}, fmt.Errorf("%w: command_id, operation_id, and positive cycle are required", ErrDirectorPlanDomainCommitConflict)
	}
	...
	payload := struct {
		...
		Summary      string
		Docs         DirectorPlanDocs
	}{Token: token, SourceTurnID: sourceTurnID, Summary: summary, Docs: docs}
	...
	return DirectorPlanDomainCommitIntent{
		Identity: identity, Hash: fmt.Sprintf("sha256:%x", sum[:]), Token: token,
...
func (s *Store) CommitDirectorPlanRun(intent DirectorPlanDomainCommitIntent) (DirectorPlanDomainCommitReceipt, error) {
	canonical, err := NewDirectorPlanDomainCommitIntent(intent.Identity, intent.Token, intent.SourceTurnID, intent.Summary, intent.Docs)
	if err != nil {
	...
	pending := DirectorPlanDomainCommitReceipt{Identity: canonical.Identity, Hash: canonical.Hash}
	...
	receipt.Plan = plan
	if receipt.Identity != canonical.Identity || receipt.Hash != canonical.Hash || strings.TrimSpace(receipt.Revision) == "" {
...
// FindDirectorPlanDomainCommit reads the receipt stored atomically with the
// terminal Director metadata. It never infers success from run status alone.
func (s *Store) FindDirectorPlanDomainCommit(
	...
	if receipt.Identity != identity || strings.TrimSpace(receipt.Hash) != hash || strings.TrimSpace(receipt.Revision) == "" {
...
func matchDirectorPlanDomainCommit(existing *DirectorPlanDomainCommitReceipt, pending *DirectorPlanDomainCommitReceipt) (bool, error) {
	...
	if existing.Identity != pending.Identity || strings.TrimSpace(existing.Hash) != strings.TrimSpace(pending.Hash) || strings.TrimSpace(existing.Revision) == "" {
