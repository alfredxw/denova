...
func defaultActorStatePresetSpec() actorStatePresetSpec {
	return actorStatePresetSpec{
		...
		Name:                   "默认状态系统",
		Description:            "以故事、主角和世界实体三个初始 Actor 集中维护少量关键状态；角色以固定检定面板和动态状态区分长期数值与即时变化。",
		PanelDescription:       "记录会直接参与检定的有效面板：等级、六维、攻击 AC 与防御 DC；每项同时保留基础值、当前值和修正来源。",
		PanelUpdateInstruction: "只维护当前规则实际使用的检定项。每项使用基础值、当前值、修正说明；当前值是等级、装备和状态修正结算后的有效值，相关来源变化时同步更新。",
		PanelDefault:           defaultTRPGPanel(),
		StateDescription:       "集中维护动态资源、增益或异常效果，以及技能和物品冷却；不把当前处境或回合纪要塞入此处。",
		StateUpdateInstruction: "固定使用资源、效果、冷却三个分区。资源项写当前值与上限；效果键使用稳定 effect ID，写名称、类型、剩余、层数和影响；冷却键使用对应 ability/item ID，写名称、剩余与单位。结束的效果或冷却及时删除。",
		StateDefault:           defaultTRPGDynamicState(),
...
func storyContextStateTemplate(spec actorStatePresetSpec) ActorStateTemplate {
	fields := []ActorStateField{
		...
		objectStateFieldWithInstruction("scene.elements", "当前场景要素", "记录会影响下一步行动的现场实体。", "visible", "当前场景", "键使用稳定 element ID；每项只写名称、类型、当前状态和行动意义，最多 6 项。删除记录时 replace 整个 object。"),
		listStateField("rules.active", "当前生效规则", "合并记录当前场景或世界中已确认、且会实际约束行动的规则；每项注明适用范围。", "spoiler", "当前场景"),
		textStateField("world.setting", "世界背景", "合并记录世界名称、时代、文明或力量体系，以及当前故事所在的大范围环境。", "visible", "世界状态", "block"),
...
func actorPanelAndStateFields(spec actorStatePresetSpec) []ActorStateField {
	panel := presetStateField("mechanics.panel", "面板", "object", spec.PanelDescription, "visible", "面板", "")
	panel.Default = spec.PanelDefault
	panel.UpdateInstruction = spec.PanelUpdateInstruction
	state := presetStateField("current.dynamic_state", "状态", "object", spec.StateDescription, "visible", "状态", "")
	state.Default = spec.StateDefault
	state.UpdateInstruction = spec.StateUpdateInstruction
	return []ActorStateField{panel, state}
}

func defaultTRPGPanel() map[string]any {
	panel := map[string]any{
		"等级":   panelNumber(1, "1–4 初阶；5–10 中阶；11–16 高阶；17 及以上传奇。作品另有等级规则时以作品为准。"),
		"攻击AC": panelNumber(10, "1–5 极低；6–9 偏低；10–13 常规；14–17 高；18 及以上极高。"),
		"防御DC": panelNumber(10, "1–5 极低；6–9 偏低；10–13 常规；14–17 高；18 及以上极高。"),
	}
	for _, name := range []string{"力量", "敏捷", "体质", "智力", "感知", "魅力"} {
		panel[name] = panelNumber(10, "1–5 极弱；6–9 偏弱；10–11 常人；12–15 优秀；16–19 卓越；20 及以上超凡。")
	}
	return panel
}

func defaultTRPGDynamicState() map[string]any {
	return map[string]any{
		"资源": map[string]any{
			"生命": map[string]any{"当前值": float64(10), "上限": float64(10)},
			"法力": map[string]any{"当前值": float64(0), "上限": float64(0)},
		},
		"效果": map[string]any{},
		"冷却": map[string]any{},
	}
...
func panelNumber(value float64, scale ...string) map[string]any {
	entry := map[string]any{"基础值": value, "当前值": value, "修正说明": ""}
