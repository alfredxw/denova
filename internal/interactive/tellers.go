	...
	return normalizeRuleChecks([]RuleCheck{
		{
			...
			DifficultyGuidance:  "以普通目标为基准。角色有合适工具、明确计划、熟悉环境或相关属性优势时降一档；时间紧迫、信息不足、受伤、被压制或环境危险时升一档；多项不利同时存在时可升到困难或极难。",
			StateEffectGuidance: "默认不强制改状态。若失败代价已经在剧情中成立，可给体力、警戒度、位置暴露、临时劣势或资源消耗 1-2 点变化；代价成功时优先选择轻量消耗而不是阻断行动。",
		...
		},
		{
			...
			Dice:                "1d20",
			FailurePolicy:       "fail_forward",
			DifficultyGuidance:  "以普通目标为基准。武器克制、占据高地、队友牵制或敌人露出破绽时降一档；敌人护甲厚重、数量压制、主角受伤、视野差或被控制时升一档。面对精英敌人或致命处境时使用困难以上。",
			StateEffectGuidance: "根据伤害规模调整生命或护盾：擦伤约 -1，普通命中约 -2~-4，重击或大失败约 -5 以上；也可写入流血、缴械、失位、破甲等状态。成功可给敌方压制、破绽或位置优势。",
		...
		},
		{
			...
			DifficultyGuidance:  "以普通目标为基准。光线昏暗、守卫分心、工具合适、路线熟悉时降一档；锁具复杂、时间受限、附近有人、地面易响或角色负重受伤时升一档。魔法锁、警戒阵或多人巡逻可使用困难以上。",
			StateEffectGuidance: "失败优先造成时间、体力或警戒度变化：体力 -1~-2、警戒度 +1、工具耐久下降或陷阱倒计时推进；大失败才直接暴露或触发陷阱。成功可保留隐蔽、取得位置优势或降低后续难度。",
		...
		},
		{
			...
			Dice:                "1d20",
			FailurePolicy:       "fail_forward",
			DifficultyGuidance:  "以普通目标为基准。线索新鲜、现场完整、角色有专业知识或工具时降一档；线索被伪装、信息残缺、时间久远、干扰严重或存在误导时升一档。隐藏真相、复杂机关或跨领域推理可用困难以上。",
			StateEffectGuidance: "成功应推进线索进度、解锁事实或减少迷雾；部分成功可给线索进度 +1 但增加时间压力；失败也给方向，但可增加误导标记、危险接近、调查资源 -1 或错过一条次要线索。",
		...
		},
		{
			...
			DifficultyGuidance:  "以普通目标为基准。关系友好、筹码充分、诉求合理、抓住对方动机时降一档；立场敌对、利益冲突、证据不足、身份劣势或公开场合丢脸风险时升一档。说服强敌、逼迫重大让步或识破谎言可用困难以上。",
			StateEffectGuidance: "根据谈判结果调整关系、信任、声望或债务：小幅态度变化约 +/-1，明确让步或冒犯约 +/-2~3；代价成功可附加欠人情、提高价格、暴露意图或留下后续条件。",
		...
		},
		{
			...
			Dice:                "1d20",
			FailurePolicy:       "success_at_cost",
			DifficultyGuidance:  "以普通目标为基准。休息充分、目标强烈、同伴支援、装备防护或相关抗性时降一档；连续消耗、伤病、恐惧、饥饿、极端天气或精神压迫时升一档。突破身体极限或抵抗强力控制时用困难以上。",
			StateEffectGuidance: "优先调整体力、压力、疲劳、伤势或意志标记：轻度消耗 -1，持续压迫 -2~-3，大失败可增加负面状态或后续劣势。成功可移除轻度恐惧、稳定伤势或获得短暂抗性。",
