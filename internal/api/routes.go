...
func (s *Server) registerRoutes(h *hertzserver.Hertz) {
	...
	api := h.Group("/api")
	{
		...
		api.POST("/chat/ui", apiHandlers.HandleChatUI)
		...
		api.GET("/chat/ui/stream", apiHandlers.HandleChatUIStream)
		...
		api.GET("/session/messages/ui", apiHandlers.HandleSessionMessagesUI)
