...
func TestDeletedSessionHandleCannotAppendIntoRecreatedJournal(t *testing.T) {
	...
	if err := stale.Append(schema.UserMessage("old journal")); err != nil {
		t.Fatal(err)
	}
	...
	if err := recreated.Append(schema.UserMessage("new journal")); err != nil {
		t.Fatal(err)
	}

	appendErr := stale.Append(schema.UserMessage("must not cross incarnation"))
