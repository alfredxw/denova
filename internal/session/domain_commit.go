...
type ContextSnapshot struct {
	EffectiveMessages []*schema.Message
...
func NewDomainCommitIntent(identity DomainCommitIdentity, message *schema.Message, metadata MessageMetadata) (DomainCommitIntent, error) {
...
func (s *Session) FindDomainCommit(identity DomainCommitIdentity, role schema.RoleType, hash string) (_ DomainCommitReceipt, _ bool, resultErr error) {
	if s == nil {
...
func (s *Session) GetEffectiveMessagesWithCursor() ([]*schema.Message, ContextCursor) {
	if s == nil {
		return nil, ContextCursor{}
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	messages := make([]*schema.Message, len(s.messages)-s.clearAfterIndex)
...
func (s *Session) AppendContextMessageAt(expected ContextCursor, msg *schema.Message) error {
...
func domainMessageHash(message schema.Message, metadata MessageMetadata) (string, error) {
	metadata = sanitizeMessageMetadata(metadata)
	payload := struct {
...
func deterministicDomainMessageID(identity DomainCommitIdentity, role schema.RoleType) string {
...
func (s *Session) findDomainCommitLocked(identity DomainCommitIdentity, role schema.RoleType, hash string) (DomainCommitReceipt, bool, error) {
