// Package shellenv captures the exported environment of the user's login
// shell for GUI-launched Agent processes. It never evaluates that shell's
// configuration in Bash; only the resulting environment crosses the seam.
package shellenv
...
type Options struct {
	Mode  config.ShellEnvironmentMode
	Shell string
}
...
func Resolve(ctx context.Context, options Options) (Snapshot, error) {
	...
	if runtime.GOOS == "windows" || mode == config.ShellEnvironmentProcess {
		return Snapshot{Environment: normalizedEnvironment(os.Environ())}, nil
	}
	...
	if err != nil {
		return Snapshot{}, fmt.Errorf("resolve user home for shell environment: %w", err)
	}
	if strings.TrimSpace(home) == "" {
		return Snapshot{}, errors.New("resolve user home for shell environment: home directory is empty")
	}
...
func capture(ctx context.Context, shell, home string, base []string) (Snapshot, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	tokenBytes := make([]byte, 16)
	if _, err := rand.Read(tokenBytes); err != nil {
		return Snapshot{}, fmt.Errorf("create shell environment marker: %w", err)
	}
	...
	if err := command.Run(); err != nil {
		return Snapshot{}, fmt.Errorf("capture exported environment with %s: %w%s", shell, err, boundedStderr(stderr.String()))
	}
	...
	if err != nil {
		return Snapshot{}, fmt.Errorf("capture exported environment with %s: %w", shell, err)
	}
	return Snapshot{Environment: environment, Shell: shell}, nil
}
...
func normalizeMode(mode config.ShellEnvironmentMode) config.ShellEnvironmentMode {
	if mode == config.ShellEnvironmentProcess {
		return mode
	}
	return config.ShellEnvironmentAuto
}
