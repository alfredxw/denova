...
func TestLegacyStoryDirectorMaintenanceKeepsStateSchemaFixed(t *testing.T) {
	...
	generator := func(_ context.Context, _ *config.Config, _ *book.State, toolContext agentinteractive.InteractiveStoryToolContext, _ string) (string, error) {
	...
	conversation := NewConversation(store, "", workspace, story.ID, "main", turn.User, story.ReplyTargetChars, &config.Config{}).BindDirectorRuntime(NewDirectorTaskGroup(), generator)
	<-StartDirectorMaintenanceTask(&config.Config{}, book.NewState(workspace), conversation, turn, nil, true)

	if len(maintenanceTasks) != 1 || maintenanceTasks[0] != DirectorTaskPlanUpdate {
		t.Fatalf("legacy story Director tasks = %#v, want only %q", maintenanceTasks, DirectorTaskPlanUpdate)
	}
