...
func TestSubmitTurnResultValidatesFrozenStatePathsAndRetainsChoicesAcrossRetry(t *testing.T) {
	...
	invalidConversation := NewConversation(store, t.TempDir(), workspace, story.ID, "main", "休息", 800, &config.Config{})
	...
	conversation := NewConversation(store, t.TempDir(), workspace, story.ID, "main", "休息", 800, &config.Config{})
...
func TestSubmitTurnResultRequiresAndCommitsStoryContext(t *testing.T) {
	...
	missing := NewConversation(store, t.TempDir(), workspace, story.ID, "main", "进入酒馆", 800, &config.Config{})
	...
	withoutLocation := NewConversation(store, t.TempDir(), workspace, story.ID, "main", "进入酒馆", 800, &config.Config{})
	...
	conversation := NewConversation(store, t.TempDir(), workspace, story.ID, "main", "进入酒馆", 800, &config.Config{})
...
func TestDirectorContextBudgetFollowsModelWindowAndCapsEachSource(t *testing.T) {
	small, err := newDirectorContextBudget(&config.Config{OpenAIContextWindowTokens: 128000}, DirectorTaskPlanUpdate, DirectorStableContext{})
	if err != nil {
		t.Fatal(err)
	}
	large, err := newDirectorContextBudget(&config.Config{OpenAIContextWindowTokens: 400000}, DirectorTaskPlanUpdate, DirectorStableContext{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestInteractiveConversationDropsTransientIndexAndThinkingFromNextTurn(t *testing.T) {
	...
	conversation := NewConversation(store, t.TempDir(), workspace, story.ID, "main", "检查门", 800, &config.Config{})
	if err := conversation.AppendDisplayEvent(session.DisplayEvent{Role: "thinking", Content: "隐藏思考"}); err != nil {
		t.Fatal(err)
	}
	if err := conversation.AppendContextMessage(agents.AssistantMessage("", []agents.ToolCall{{
		ID:   "call-lore",
		Type: "function",
	...
	next := NewConversation(store, t.TempDir(), workspace, story.ID, "main", "继续观察", 800, &config.Config{})
	...
	for _, msg := range messages {
		if msg.Role == agents.RoleAssistant && len(msg.ToolCalls) == 1 && msg.ToolCalls[0].Function.Name == "list_lore_items" {
			t.Fatalf("transient Lore index call must not enter next-turn context: %#v", msg)
		}
		if msg.Role == agents.RoleTool && msg.ToolName == "list_lore_items" {
