...
// DirectorCommandID is fixed length but fully determined by the
// durable plan token and the exact maintenance operation it authorizes.
func DirectorCommandID(token interactive.DirectorPlanRunToken, sourceTurnID, maintenanceTask string) (string, error) {
...
func StartDirectorMaintenanceTask(cfg *config.Config, state *book.State, conversation *Conversation, turn interactive.TurnEvent, sessionStore *session.Store, runPlan bool) <-chan struct{} {
	tasks := directorTasksForConversation(conversation)
	done, started := tasks.GoKeyed(DerivedMaintenanceKey(conversation, turn.BranchID), func(ctx context.Context) {
		defer func() {
		...
		conversation.WithDirectorTask(DirectorTaskPlanUpdate)
		if _, err := runInteractiveDirectorMaintenance(ctx, cfg, state, conversation, turn, sessionStore, DirectorTaskPlanUpdate); err != nil {
...
func acknowledgeInteractiveDirectorDerivedTurn(conversation *Conversation, turn interactive.TurnEvent) {
...
func PrepareDirectorBeforeOpening(ctx context.Context, cfg *config.Config, state *book.State, conversation *Conversation, openingMessage string, sessionStore *session.Store) (bool, error) {
	if conversation == nil || conversation.store == nil || cfg == nil {
	...
	if SnapshotTurnCount(storyCtx.Snapshot) > 0 {
		return false, nil
	}
	...
	turn := interactive.TurnEvent{
		...
		ID:       DirectorOpeningSourceID,
		BranchID: storyCtx.Snapshot.BranchID,
		User:     openingContext,
	}
	conversation.WithDirectorTask(DirectorTaskOpeningPlan)
	if _, err := runInteractiveDirectorMaintenance(ctx, cfg, state, conversation, turn, sessionStore, DirectorTaskOpeningPlan); err != nil {
		return true, err
	}
	status, err = conversation.store.DirectorPlanStatus(conversation.storyID, storyCtx.Snapshot.BranchID)
	if err != nil {
...
func StartDirectorTask(cfg *config.Config, state *book.State, conversation *Conversation, turn interactive.TurnEvent, sessionStore *session.Store, prestartedTokens ...interactive.DirectorPlanRunToken) <-chan struct{} {
	tasks := directorTasksForConversation(conversation)
	done, started := tasks.GoKeyed(DerivedMaintenanceKey(conversation, turn.BranchID), func(ctx context.Context) {
		defer func() {
			if recovered := recover(); recovered != nil {
...
func interactiveBranchMaintenanceKey(conversation *Conversation, branchID, lane string) string {
	storyID := ""
	if conversation != nil {
...
func DerivedMaintenanceKey(conversation *Conversation, branchID string) string {
	return interactiveBranchMaintenanceKey(conversation, branchID, "derived")
}

func directorTasksForConversation(conversation *Conversation) *DirectorTaskGroup {
...
func runInteractiveDirectorPlan(ctx context.Context, cfg *config.Config, state *book.State, conversation *Conversation, turn interactive.TurnEvent, sessionStore *session.Store, prestartedTokens ...interactive.DirectorPlanRunToken) (interactive.DirectorPlan, error) {
	result, err := runInteractiveDirectorMaintenance(ctx, cfg, state, conversation, turn, sessionStore, DirectorTaskPlanUpdate, prestartedTokens...)
	return result.Plan, err
}

func runInteractiveDirectorMaintenance(ctx context.Context, cfg *config.Config, state *book.State, conversation *Conversation, turn interactive.TurnEvent, sessionStore *session.Store, task string, prestartedTokens ...interactive.DirectorPlanRunToken) (interactiveDirectorMaintenanceResult, error) {
	if conversation == nil || conversation.store == nil || cfg == nil {
		return interactiveDirectorMaintenanceResult{}, fmt.Errorf("互动导演运行上下文不完整")
	}
	task = strings.TrimSpace(task)
	if task == "" {
		task = DirectorTaskPlanUpdate
	}
	switch task {
	case DirectorTaskPlanUpdate, DirectorTaskOpeningPlan:
	default:
	...
	decision := DirectorRunDecision(director.Strategy)
	...
	commandID, err := DirectorCommandID(token, turn.ID, task)
	if err != nil {
	...
	if err != nil {
		if committedPlan, persistedOutput, committed := planCommit.committedResult(); committed {
			...
			persistAgentCall(sessionStore, config.AgentKindInteractiveDirector, instruction, persistedOutput)
...
func markInteractiveDirectorFailed(conversation *Conversation, turn interactive.TurnEvent, err error) {
...
func markInteractiveDirectorMaintenanceFailed(conversation *Conversation, turn interactive.TurnEvent, err error) {
	markInteractiveDirectorFailed(conversation, turn, err)
}

func DirectorRunDecision(strategy interactive.StoryDirectorStrategy) director.ScheduleDecision {
...
func persistAgentCall(store *session.Store, agentKind, instruction, response string) {
	if store == nil {
		slog.WarnContext(context.Background(), "[interactive-director-agent] skip call persistence: session store is unavailable", "agent_kind", agentKind)
		return
	}
	if err := session.PersistAgentCall(store, agentKind, instruction, response); err != nil {
		slog.ErrorContext(context.Background(), "[interactive-director-agent] persist call failed", "agent_kind", agentKind, "err", err)
	}
}
