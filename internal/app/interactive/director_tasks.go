...
// DirectorTaskGroup owns background Director work for exactly one
// workspace runtime. HTTP disconnects do not cancel it; replacing or closing
// the runtime does.
type DirectorTaskGroup struct {
	ctx    context.Context
...
func NewDirectorTaskGroup() *DirectorTaskGroup {
	ctx, cancel := context.WithCancel(context.Background())
	return &DirectorTaskGroup{ctx: ctx, cancel: cancel, tails: map[string]chan struct{}{}}
}

func (g *DirectorTaskGroup) Go(run func(context.Context)) (<-chan struct{}, bool) {
...
func (g *DirectorTaskGroup) GoKeyed(key string, run func(context.Context)) (<-chan struct{}, bool) {
...
func (g *DirectorTaskGroup) WaitKey(ctx context.Context, key string) error {
	if g == nil || key == "" {
		return nil
	}
	g.mu.Lock()
...
func (g *DirectorTaskGroup) HasKey(key string) bool {
	if g == nil || key == "" {
		return false
	}
	g.mu.Lock()
...
func (g *DirectorTaskGroup) Close() {
	if g == nil {
		return
	}
	g.mu.Lock()
