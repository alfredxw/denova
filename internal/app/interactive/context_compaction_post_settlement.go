...
func (c *Conversation) stagePreparedInteractiveCompaction(prepared preparedInteractiveContextCompaction) {
...
func (c *Conversation) PostSettlementContextStructuralSpec(
	ctx context.Context,
	...
	preparedHash, err := agentstructural.ValueHash(struct {
		Result          agentcompaction.Result
		SourceTurnCount int
	...
	commandID := agentstructural.CommandID(
	...
	ref := agentrun.ContextCompactionRef{
		...
		Resource: c.storyID + "/" + branchID, ExpectedRevision: agentstructural.StoryRevision(expectedParent),
	}
	binding := agentrun.RuntimeBinding{
		AgentKind: agentrun.AgentKindInteractiveStory, Workspace: options.Workspace,
