...
func validateAutomationReceipt(receipt agentruntime.Receipt, expectedCommandID string) error {
...
func automationRootReceipt(run automation.RunRecord) agentruntime.Receipt {
	...
	return agentruntime.Receipt{
		...
		Cursor: agentruntime.Cursor(cursor),
	}
}
...
func applyAutomationRootReceipt(run *automation.RunRecord, receipt agentruntime.Receipt) error {
	if run == nil {
...
func applyAutomationCurrentReceipt(run *automation.RunRecord, receipt agentruntime.Receipt, expectedCommandID string) error {
	if run == nil {
		return fmt.Errorf("automation run is required")
	}
...
func advanceAutomationCurrentReceipt(run *automation.RunRecord, receipt agentruntime.Receipt, expectedCommandID string) error {
	if run == nil {
		return fmt.Errorf("automation run is required")
	}
	if err := validateAutomationReceipt(receipt, expectedCommandID); err != nil {
...
func applyAutomationFollowUpReceipt(run *automation.RunRecord, receipt agentruntime.Receipt, expectedCommandID, fingerprint string) error {
	if run == nil {
		return fmt.Errorf("automation run is required")
	}
	...
	if receipt.Replayed && run.RuntimeCommandID == string(receipt.CommandID) && run.RuntimeOperationID == string(receipt.OperationID) {
		...
		if uint64(receipt.Cursor) < run.RuntimeReceiptCursor {
			receipt.Cursor = agentruntime.Cursor(run.RuntimeReceiptCursor)
		}
...
func advanceVerifiedAutomationCurrentReceipt(run *automation.RunRecord, receipt agentruntime.Receipt, expectedCommandID string) error {
	if run == nil {
		return fmt.Errorf("automation run is required")
	}
	if err := validateAutomationReceipt(receipt, expectedCommandID); err != nil {
