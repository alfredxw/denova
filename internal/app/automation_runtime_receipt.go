...
func validateAutomationReceipt(receipt runstate.Receipt, expectedCommandID string) error {
...
func automationRootReceipt(run automation.RunRecord) runstate.Receipt {
...
func applyAutomationRootReceipt(run *automation.RunRecord, receipt runstate.Receipt) error {
	if run == nil {
...
func applyAutomationCurrentReceipt(run *automation.RunRecord, receipt runstate.Receipt, expectedCommandID string) error {
	if run == nil {
		return fmt.Errorf("automation run is required")
	}
...
func advanceAutomationCurrentReceipt(run *automation.RunRecord, receipt runstate.Receipt, expectedCommandID string) error {
	if run == nil {
		return fmt.Errorf("automation run is required")
	}
	if err := validateAutomationReceipt(receipt, expectedCommandID); err != nil {
...
func applyAutomationFollowUpReceipt(run *automation.RunRecord, receipt runstate.Receipt, expectedCommandID, fingerprint string) error {
	if run == nil {
		return fmt.Errorf("automation run is required")
	}
	...
	if receipt.Replayed && run.RuntimeCommandID == string(receipt.CommandID) && run.RuntimeOperationID == string(receipt.OperationID) {
		...
		if uint64(receipt.Cursor) < run.RuntimeReceiptCursor {
			receipt.Cursor = runstate.Cursor(run.RuntimeReceiptCursor)
		}
...
func advanceVerifiedAutomationCurrentReceipt(run *automation.RunRecord, receipt runstate.Receipt, expectedCommandID string) error {
	if run == nil {
		return fmt.Errorf("automation run is required")
	}
	if err := validateAutomationReceipt(receipt, expectedCommandID); err != nil {
