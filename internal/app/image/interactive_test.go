...
func TestInteractiveImageRequiresBoundedCallerCommandIDBeforeWorkspaceAccess(t *testing.T) {
	service := &Service{}
	if _, err := service.GenerateInteractiveImage(context.Background(), "story", interactive.InteractiveImageGenerateRequest{}); !errors.Is(err, apptask.ErrCommandIDRequired) {
	...
	if _, err := service.GenerateInteractiveImage(context.Background(), "story", request); !errors.Is(err, agentrun.ErrInvalidCommand) {
...
func TestImageAgentMessageDeterministicallyLoadsRequestedSkill(t *testing.T) {
	req := AgentGenerateRequest{SkillName: "interactive-image", Purpose: "interactive_image"}
