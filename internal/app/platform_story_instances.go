...
func (h platformStoryHost) Instances(ctx context.Context) ([]platform.Instance, error) {
	projects, err := h.app.projectRegistry.List(false)
	if err != nil {
		return nil, err
	}
	items := []platform.Instance{}
	for _, project := range projects {
		store, err := h.store(ctx, project.ID)
		if err != nil {
			continue
		}
...
func platformStoryInstancesInStore(ctx context.Context, store *platformStoryStore) ([]platform.Instance, error) {
...
func (h platformStoryHost) Bind(ctx context.Context, instance platform.Instance, options platform.StoryBindingOptions) (platform.Instance, error) {
	operation, err := h.acquire(ctx, instance.ProjectID)
	if err != nil {
		return platform.Instance{}, err
	}
	defer operation.Release()
	if instance.StoryID == "" {
		story, err := h.app.CreateInteractiveStoryContext(operation.Context(), interactive.CreateStoryRequest{Title: instance.Title, Origin: options.Origin, ProfileID: options.ModelProfile, Preview: instance.Preview})
		if err != nil {
			return platform.Instance{}, err
		}
	...
	store, err := h.store(ctx, instance.ProjectID)
	if err != nil {
		return platform.Instance{}, err
	}
	defer store.Close()
...
func (h platformStoryHost) SaveBinding(ctx context.Context, instance platform.Instance) error {
	return h.writeBinding(ctx, platformStoryBinding{Instance: instance})
}

func (h platformStoryHost) RemoveBinding(ctx context.Context, instance platform.Instance) error {
...
func (h platformStoryHost) writeBinding(ctx context.Context, binding platformStoryBinding) error {
...
func (h platformStoryHost) Export(ctx context.Context, instance platform.Instance, writer io.Writer) error {
	store, err := h.store(ctx, instance.ProjectID)
	if err != nil {
		return err
	}
