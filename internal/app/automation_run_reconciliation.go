...
type automationRuntimeProjectionFunc func(context.Context, *automationWorkspaceSnapshot, automation.Task, automation.RunRecord) (agentrun.RuntimeStatus, error)

func (s *AutomationAppService) automationRuntimeProjection(ctx context.Context, snap *automationWorkspaceSnapshot, task automation.Task, run automation.RunRecord) (agentrun.RuntimeStatus, error) {
...
func (s *AutomationAppService) reconcileAutomationRunReceipt(ctx context.Context, snap *automationWorkspaceSnapshot, task automation.Task, candidate automation.RunRecord) (automation.RunRecord, bool, error) {
	...
	if match.active {
		...
		candidate.FinishedAt = time.Now().UTC()
		switch match.status {
		case agentrun.OperationSucceeded:
...
func automationTerminalStatusMatches(runStatus string, operationStatus agentrun.OperationStatus) bool {
...
type automationRuntimeReceiptMatch struct {
	...
	operationID   string
	fingerprint   string
	receiptCursor agentrun.Cursor
	status        agentrun.OperationStatus
	reason        string
	active        bool
}

func automationRuntimeReceipt(status agentrun.RuntimeStatus, candidate automation.RunRecord) automationRuntimeReceiptMatch {
	...
	matches := func(summary agentrun.OperationSummary) bool {
		if wanted != "" {
			return string(summary.OperationID) == wanted
		}
		return wantedCommand == "" || string(summary.CommandID) == wantedCommand
	}
	if status.ActiveOperation != "" && matches(agentrun.OperationSummary{OperationID: status.ActiveOperation, CommandID: status.ActiveCommandID}) {
...
func automationRuntimeMatchFromSummary(summary agentrun.OperationSummary) automationRuntimeReceiptMatch {
	return automationRuntimeReceiptMatch{
