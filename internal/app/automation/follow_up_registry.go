...
func newAutomationFollowUpIdentity(runID, commandID, message string) (automationFollowUpIdentity, error) {
	...
	if commandID == "" {
		return automationFollowUpIdentity{}, ErrCommandIDRequired
	}
...
func (r *automationFollowUpRegistry) replay(identity automationFollowUpIdentity) (*apptask.Task, automation.RunRecord, bool, error) {
	...
	if record.identity.runID != identity.runID || record.identity.fingerprint != identity.fingerprint || record.task == nil {
		if record.identity.runID == identity.runID && record.identity.fingerprint == identity.fingerprint && record.task == nil {
			r.order = apptask.TouchReplayKey(r.order, identity.commandID)
			r.pruneLocked()
			return nil, automation.RunRecord{}, false, nil
		}
		return nil, automation.RunRecord{}, false, fmt.Errorf("%w: command_id=%q", ErrCommandConflict, identity.commandID)
	}
	r.order = apptask.TouchReplayKey(r.order, identity.commandID)
	r.pruneLocked()
	...
	if existing, ok := r.records[identity.commandID]; ok {
		if existing.identity.runID != identity.runID || existing.identity.fingerprint != identity.fingerprint || existing.task != task {
			if existing.identity.runID != identity.runID || existing.identity.fingerprint != identity.fingerprint || existing.task != nil {
				return nil, fmt.Errorf("%w: command_id=%q", ErrCommandConflict, identity.commandID)
			}
		}
		existing.task = task
		...
		if r.registryChargeLocked() > apptask.EffectiveRegistryReplayByteLimit(r.replayByteLimit) {
			existing.task = nil
			r.records[identity.commandID] = existing
			return nil, fmt.Errorf("%w: automation follow-up command_id=%q", ErrReplayCapacity, identity.commandID)
		}
		return reservation, nil
	}
	for len(r.records) >= maxRememberedAutomationFollowUps {
		if !r.removeOldestSettledIdentityLocked() {
			return nil, fmt.Errorf("%w: automation follow-up records=%d", ErrReplayCapacity, len(r.records))
		}
	}
...
func (reservation *automationFollowUpReservation) bind(run automation.RunRecord) {
	...
	if !ok {
		...
		r.order = apptask.TouchReplayKey(r.order, reservation.identity.commandID)
	}
	record.run = run
