...
func (c *interactiveConversation) CommitPostSettlementToolResultCleanup(ctx context.Context, settledOperationID agentrun.OperationID) error {
	...
	hash, err := contextStructuralValueHash(prepared.event)
	if err != nil {
		return err
	}
	commandID := contextStructuralCommandID("auto-tool-result-cleanup", string(settledOperationID), c.storyID, storyCtx.Snapshot.BranchID, hash)
	event := prepared.event
	event.ID = contextStructuralRecordID("trc", commandID)
...
func (c *interactiveConversation) DiscardStagedToolResultCleanup() {
	if c == nil {
		return
	}
	c.mu.Lock()
