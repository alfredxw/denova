...
func TestTaskDisconnectsSlowSubscriberInsteadOfDroppingEvents(t *testing.T) {
	...
	task := New(func(_ context.Context, _ *Task, emit func(agentrun.Event)) {
		...
		<-emitAll
		for i := 0; i < 257; i++ {
			emit(agentrun.Event{Type: "chunk"})
		}
		close(emitted)
		<-finish
	})
	...
	if slow.EndReason() != SubscriptionLagged {
		t.Fatalf("slow subscription ended with %q, want %q", slow.EndReason(), SubscriptionLagged)
	}
	if received != 256 {
...
func TestTaskIgnoresEventsAfterFinish(t *testing.T) {
	...
	task.Emit(agentrun.Event{Type: "error"})
	task.Abort()
	...
	if status := task.Status(); status != Done {
		t.Fatalf("status after late emit and abort = %q, want %q", status, Done)
	}
	if _, ok := <-closed.Events(); ok {
		t.Fatal("finished task returned an open subscription")
	}
	if closed.EndReason() != SubscriptionTaskFinished {
...
func TestTaskDonePublishesCanceledLifecycle(t *testing.T) {
	...
	task := New(func(ctx context.Context, _ *Task, _ func(agentrun.Event)) {
		started <- ctx
	})
	ctx := <-started
	<-task.Done()
...
func TestTaskStartFailurePublishesCanceledLifecycle(t *testing.T) {
	...
	ctx := task.ctx
	task.RejectStart(errors.New("rejected"))
	<-task.Done()
	select {
	case <-ctx.Done():
	default:
...
func TestDetachedTaskRetainsRequestIDWithoutCallerCancellation(t *testing.T) {
	...
	task, err := NewDeferredWithContext(caller, nil)
	if err != nil {
		t.Fatal(err)
	}
	cancelCaller()
	...
	if err := task.Start(func(ctx context.Context, _ *Task, _ func(agentrun.Event)) {
		observed <- ctx
		<-release
	}); err != nil {
		t.Fatal(err)
...
func TestTaskAbortRequestDoesNotOverrideLaterDurableCompletion(t *testing.T) {
	...
	task := New(func(ctx context.Context, _ *Task, emit func(agentrun.Event)) {
		<-ctx.Done()
		<-settle
		emit(agentrun.Event{Type: "done"})
	})
	...
	if !snapshot.CancelRequested || snapshot.Finished || snapshot.Status != Running {
		t.Fatalf("abort request must not project a terminal outcome before settlement: %#v", snapshot)
	}
	close(settle)
	<-task.Done()
	snapshot = task.Snapshot()
	if !snapshot.CancelRequested || !snapshot.Finished || snapshot.Status != Done {
...
func TestTaskAcceptedAbortSettlesFromAbortedEvent(t *testing.T) {
	...
	<-task.Done()
	snapshot := task.Snapshot()
	if !snapshot.CancelRequested || !snapshot.Finished || snapshot.Status != Aborted {
...
func TestTaskPanicEmitsErrorAndSettlesAsTaskError(t *testing.T) {
	task := New(func(context.Context, *Task, func(agentrun.Event)) {
		panic("worker exploded")
	})
	<-task.Done()

	snapshot := task.Snapshot()
	if !snapshot.Finished || snapshot.Status != Failed {
		t.Fatalf("panic settlement = %#v, want finished Failed", snapshot)
	}
...
func TestTaskSubscribeAfterResumesAtExactDisplayCursor(t *testing.T) {
	...
	emitted := make(chan struct{})
	task := New(func(_ context.Context, _ *Task, emit func(agentrun.Event)) {
		emit(agentrun.Event{Type: "chunk", Data: "one"})
		emit(agentrun.Event{Type: "chunk", Data: "two"})
		close(emitted)
		<-release
	})
	...
	if _, _, err := task.SubscribeAfter(3); !errors.Is(err, ErrCursorAhead) {
		t.Fatalf("ahead cursor error = %v, want ErrCursorAhead", err)
	}
	close(release)
...
func TestTaskRetentionBoundsMemoryAndRejectsExpiredDisplayCursor(t *testing.T) {
	...
	for index := 1; index <= 5; index++ {
		task.Emit(agentrun.Event{Type: "chunk", Data: fmt.Sprintf("chunk-%d", index)})
	}
	...
	if _, _, err := task.SubscribeAfter(1); !errors.Is(err, ErrCursorExpired) {
...
func TestTaskRetentionDoesNotKeepOneEventLargerThanByteBudget(t *testing.T) {
	...
	task.Emit(agentrun.Event{Type: "chunk", Data: strings.Repeat("x", 128)})
	...
	if _, _, err := task.SubscribeAfter(0); !errors.Is(err, ErrCursorExpired) {
		t.Fatalf("oversized event cursor error = %v, want ErrCursorExpired", err)
	}
...
func TestTaskDisplayCheckpointRecoversActiveTaskAfterRawRetentionOverflow(t *testing.T) {
	...
	task.retainedEventLimit = 3
	task.retainedByteLimit = 1 << 20
	task.Emit(agentrun.Event{Type: "agent_cycle_started", Data: map[string]any{
	...
	for _, fragment := range []string{"一", "段", "完整", "思考", "。"} {
		task.Emit(agentrun.Event{Type: "thinking", Data: map[string]any{"content": fragment, "run_id": "run-1"}})
	}
	...
	task.Emit(agentrun.Event{Type: "chunk", Data: map[string]any{"content": "正文", "run_id": "run-1"}})
...
func TestTaskDisplayCheckpointReplaysFinishedTaskAndAssemblesToolArguments(t *testing.T) {
	...
	task.retainedByteLimit = 1 << 20
	if err := task.Start(func(_ context.Context, _ *Task, emit func(agentrun.Event)) {
		emit(agentrun.Event{Type: "agent_cycle_started", Data: map[string]any{
			"command_id": "command-1", "delivery": "start_turn", "message": "读取", "operation_id": "operation-1", "cycle": 1,
		}})
		emit(agentrun.Event{Type: "tool_call", Data: map[string]any{"id": "call-1", "name": "read", "args": "", "run_id": "run-1"}})
		for _, delta := range []string{`{"path"`, `:"chapter.md"`, `}`} {
			// Some providers omit the repeated tool name on delta frames.
			emit(agentrun.Event{Type: "tool_args_delta", Data: map[string]any{"id": "call-1", "delta": delta, "run_id": "run-1"}})
		}
...
func TestTaskDisplayCheckpointMarksWholeEventOmissionIncomplete(t *testing.T) {
	...
	task.Emit(agentrun.Event{Type: "thinking", Data: map[string]any{"content": "这段思考只能完整保留或完整省略"}})
...
func TestTaskDisplayCheckpointKeepsPersistenceBarrierAfterCycleAnchorEviction(t *testing.T) {
	...
	task.Emit(agentrun.Event{Type: "interactive_turn_persisted", Data: map[string]any{"turn_id": "turn-1"}})
	replay, subscription, err = task.SubscribeDisplayAfter(0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTaskDisplayCheckpointKeepsTerminalOutcomeAfterSettledReplayEviction(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "error",
			event:      agentrun.Event{Type: "error", Data: map[string]string{"message": "provider failed after acceptance"}},
			wantStatus: Failed,
			wantReason: "provider failed after acceptance",
		},
		{
			name:       "aborted",
			event:      agentrun.Event{Type: "aborted", Data: map[string]string{"reason": "user_requested"}},
			wantStatus: Aborted,
...
func TestNewRegisteredTaskPublishesBeforeRun(t *testing.T) {
	...
	task, err := NewRegistered(func(task *Task) error {
		registered = task
		return nil
	}, func(_ context.Context, task *Task, _ func(agentrun.Event)) {
		if registered != task {
