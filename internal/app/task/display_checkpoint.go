...
func (t *Task) displayCheckpointLocked() DisplayCheckpoint {
	events := make([]agentrun.Event, len(t.checkpointEvents))
...
func (t *Task) DisplayReplayCharge() int {
	if t == nil {
		return 0
	}
	t.mu.Lock()
...
func (t *Task) projectDisplayCheckpointLocked(item Event) {
	...
	if !t.mergeDisplayCheckpointEventLocked(event) {
		...
		size := taskEventSize(Event{Event: event})
...
func (t *Task) mergeDisplayCheckpointEventLocked(event agentrun.Event) bool {
	switch event.Type {
	case "chunk", "thinking":
...
func (t *Task) mergeDisplayCheckpointTextLocked(event agentrun.Event, field string) bool {
...
func (t *Task) mergeDisplayCheckpointToolArgsLocked(event agentrun.Event) bool {
...
func (t *Task) replaceDisplayCheckpointEventLocked(index int, event agentrun.Event) {
	oldSize := t.checkpointBytes[index]
	newSize := taskEventSize(Event{Event: event})
...
func (t *Task) boundDisplayCheckpointLocked() {
	...
	for len(t.checkpointEvents) > 0 && (len(t.checkpointEvents) > eventLimit || t.checkpointSize > byteLimit) {
		...
		t.checkpointEvents[len(t.checkpointEvents)-1] = agentrun.Event{}
...
func cloneTaskDisplayEvent(event agentrun.Event) agentrun.Event {
...
func taskDisplayReplacementKey(event agentrun.Event) string {
