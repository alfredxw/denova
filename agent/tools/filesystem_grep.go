...
func (workspace *LocalWorkspace) Grep(ctx context.Context, request GrepRequest) (SearchResult, error) {
	...
	fingerprint := grepCommandFingerprint(command)
	cursor, err := decodeGrepCursorFingerprint(request.Cursor, fingerprint)
	if err != nil {
		return SearchResult{}, err
	}
	...
	if truncated {
		...
		encoded, err := encodeGrepCursorFingerprint(next, fingerprint)
		if err != nil {
...
func (workspace *LocalWorkspace) grepExecutionFingerprint(request GrepRequest) (string, error) {
	command, err := workspace.compileGrepCommand(normalizeGrepRequest(request).Command)
	if err != nil {
		return "", err
	}
	return grepCommandFingerprint(command), nil
}
