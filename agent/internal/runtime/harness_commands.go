...
func (h *Harness) handleSubmit(state *harnessState, ctx context.Context, command Command) (Receipt, error) {
	...
	case NextTurn:
		...
		capabilities, err := h.prepareAdmission(ctx, state, commandID, operationID, 1, DeliveryNextTurn, false, command.Input)
		if err != nil {
			return Receipt{}, fmt.Errorf("prepare accepted NextTurn admission: %w", err)
		}
		...
		payloads = append(payloads, QueueEnqueuedEvent{Item: item})
...
func (h *Harness) beginOperation(
	...
	delivery := cycleDelivery(commandKind)
	capabilities, err := h.prepareAdmission(ctx, state, commandID, operationID, 1, delivery, false, input)
	if err != nil {
		return Receipt{}, err
	}
	payloads := []EventPayload{
	...
	payloads = append(payloads,
		...
		newCycleStartedEvent(operationID, 1, snapshotID, commandID, commandKind, false),
	)
	committed, err := h.commit(ctx, state, payloads)
	if err != nil {
		return Receipt{}, err
	...
	h.startEngine(state, state.turnSnapshot(snapshotID), false)
	return receipt, nil
}
...
func (h *Harness) prepareAdmission(
	...
	snapshot.Delivery, snapshot.Autonomous = delivery, autonomous
...
func (h *Harness) enqueueCurrent(
	...
	capabilities, err := h.prepareAdmission(
		ctx, state, commandID, state.activeOperation, state.activeCycle+1, delivery, false, input,
	)
	if err != nil {
		return Receipt{}, fmt.Errorf("prepare accepted %s admission: %w", delivery, err)
	}
