...
const harnessCheckpointVersion = 6
...
type harnessCheckpoint struct {
	...
	CycleDelivery   DeliveryKind `json:"cycle_delivery,omitempty"`
	CycleAutonomous bool         `json:"cycle_autonomous,omitempty"`
...
func (s *harnessState) checkpoint() (harnessCheckpoint, error) {
	...
	checkpoint := harnessCheckpoint{
		Version: harnessCheckpointVersion, Binding: s.binding.Clone(), Cursor: s.cursor,
		Phase: s.phase, OperationID: s.activeOperation, Cycle: s.activeCycle, SnapshotID: s.activeSnapshotID,
		CycleStartedAt: s.activeCycleStartedAt,
		CycleDelivery:  s.activeCycleDelivery, CycleAutonomous: s.activeCycleAutonomous,
...
func restoreHarnessCheckpoint(target *harnessState, checkpoint harnessCheckpoint) error {
	...
	if checkpoint.Version != harnessCheckpointVersion || !checkpoint.Binding.Equal(target.binding) || checkpoint.Cursor == 0 {
	...
	restored.activeCycleStartedAt = checkpoint.CycleStartedAt
	...
	restored.activeCycleDelivery = checkpoint.CycleDelivery
	restored.activeCycleAutonomous = checkpoint.CycleAutonomous
