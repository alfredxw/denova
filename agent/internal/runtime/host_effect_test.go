...
func TestHostEffectDoesNotTransferWhenExactOutputCommitFails(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), testBindingAt("/book", "host-effect-failed-output"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestToolFinishAndHostEffectShareOneDurableTransaction(t *testing.T) {
	...
	t.Cleanup(func() { _ = runtime.Close(context.Background()) })
	harness, err := runtime.Open(context.Background(), testBindingAt("/book", "host-effect-transaction"))
	if err != nil {
		t.Fatal(err)
	}
	ctx, cancel := context.WithTimeout(context.Background(), 500*time.Millisecond)
...
func TestColdOpenRetriesUnackedHostEffectIdempotentlyBeforeOperationRecovery(t *testing.T) {
	...
	ref := testBinding("host-effect-cold-retry")
	...
	t.Cleanup(func() { _ = runtime.Close(context.Background()) })
	binding := testBindingAt("/book", "host-effect-cold-retry")
...
func TestPendingHostEffectFencesExactReplayFreshCommandAndInputRecovery(t *testing.T) {
	ref := testBinding("host-effect-fence")
...
func TestOversizedHostEffectSettlesTypedIncomplete(t *testing.T) {
	...
	t.Cleanup(func() { _ = runtime.Close(context.Background()) })
	harness, err := runtime.Open(context.Background(), testBindingAt("/book", "host-effect-budget"))
	if err != nil {
		t.Fatal(err)
	}
	ctx, cancel := context.WithTimeout(context.Background(), 500*time.Millisecond)
