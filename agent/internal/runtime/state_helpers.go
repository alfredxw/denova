...
func cycleDelivery(delivery string) DeliveryKind {
	switch DeliveryKind(delivery) {
	case DeliveryStart, DeliverySteer, DeliveryFollowUp, DeliveryNextTurn:
		return DeliveryKind(delivery)
	case "start_turn":
...
func normalizedCycleDelivery(delivery string, cycle int) DeliveryKind {
	if normalized := cycleDelivery(delivery); normalized != "" {
		return normalized
	}
	// Journals written before Delivery became part of TurnSnapshot still have
	// unambiguous Start semantics for cycle one. Later legacy cycles cannot
	// distinguish Steer from FollowUp and are conservatively treated as Steer;
	// every newly written event carries the exact value.
	if delivery == "" {
		if cycle <= 1 {
			return DeliveryStart
		}
