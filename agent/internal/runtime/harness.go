...
func (h *Harness) run(state harnessState) {
	...
	for request := range h.requests {
		...
		case engineCheckpointRequest:
			capabilityDescriptors := make(map[string]PayloadDescriptor, len(state.capabilityStates))
			for name, value := range state.capabilityStates {
				capabilityDescriptors[name] = describePayload(value)
			}
			request.response <- EngineCheckpointSnapshot{
				Cursor: state.cursor, State: cloneRawMessage(state.engineState), StateDescriptor: describePayload(state.engineState),
				Capabilities: cloneCapabilityStates(state.capabilityStates), CapabilityDescriptors: capabilityDescriptors,
			}
		case idleEngineCheckpointRequest:
			if state.closing {
				request.response <- idleEngineCheckpointResponse{err: ErrHarnessClosed}
				continue
			}
			if state.phase != PhaseIdle || len(state.queue) != 0 || state.recoveryPaused ||
				state.inputRecovery != nil || state.pendingDomainCommit() != nil || len(state.interactions) != 0 {
				request.response <- idleEngineCheckpointResponse{err: ErrBusy}
				continue
			}
			capabilityDescriptors := make(map[string]PayloadDescriptor, len(state.capabilityStates))
			for name, value := range state.capabilityStates {
			...
			request.response <- idleEngineCheckpointResponse{snapshot: EngineCheckpointSnapshot{
				Cursor: state.cursor, State: cloneRawMessage(state.engineState), StateDescriptor: describePayload(state.engineState),
				Capabilities: cloneCapabilityStates(state.capabilityStates), CapabilityDescriptors: capabilityDescriptors,
			}}
		case replaceEngineCheckpointRequest:
			var err error
			if state.closing {
				err = ErrHarnessClosed
			} else if state.phase != PhaseIdle || len(state.queue) != 0 || state.recoveryPaused ||
				state.inputRecovery != nil || state.pendingDomainCommit() != nil || len(state.interactions) != 0 {
				err = ErrBusy
			} else if payloads, validationErr := state.replaceEngineCheckpointEvents(request.update); validationErr != nil {
				err = validationErr
			} else if len(payloads) == 0 {
				err = nil
			} else {
				_, err = h.commit(request.ctx, &state, payloads)
			}
			if terminal, fatal := terminalJournalAppendError(err); fatal {
				h.setFailure(terminal)
				request.response <- terminal
		...
		case setCapabilityStateRequest:
			...
			if state.closing {
				...
				err = ErrBusy
			} else if payloads, validationErr := state.capabilityStateEvents(request.changes); validationErr != nil {
				err = validationErr
			} else {
				_, err = h.commit(request.ctx, &state, payloads)
			}
