...
func (s *harnessState) reduce(event Event) error {
	...
	case OperationStartedEvent:
		...
		s.activeCycle = 0
		s.activeCycleDelivery = ""
		s.activeCycleAutonomous = false
		s.activeSnapshotID = ""
		...
		if payload.Structural != nil {
			...
			if structural.OperationID != payload.OperationID || structural.CommandID != s.operationCommands[payload.OperationID] || !structural.Binding.Equal(s.binding) || structural.Cycle <= 0 {
	...
	case CleanupCompletedEvent:
		if payload.OperationID != s.activeOperation || payload.Cycle != s.activeCycle || payload.ID == "" || payload.Transient {
			return fmt.Errorf("Cleanup completion does not match active operation")
		}
	...
	case CycleStartedEvent:
		...
		s.activeCycleDelivery = normalizedCycleDelivery(payload.Delivery, payload.Cycle)
		if s.activeCycleDelivery == "" {
			return fmt.Errorf("cycle has unsupported delivery %q", payload.Delivery)
		}
		s.activeCycleAutonomous = payload.Autonomous
		s.activeCycleStartedAt = payload.StartedAt
...
func (s *harnessState) clearActiveOperation() {
	...
	s.activeCycleDelivery = ""
	s.activeCycleAutonomous = false
	s.activeCycleStartedAt = time.Time{}
	s.activeSnapshotID = ""
	s.activeStructural = nil
	s.recoveryPaused = false
	s.inputRecovery = nil
