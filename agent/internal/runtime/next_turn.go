...
func (h *Harness) planNextTurn(ctx context.Context, state *harnessState, allowStart bool) nextTurnPlan {
	...
	plan.capabilities, plan.failure = h.prepareAdmission(ctx, state, item.CommandID, item.OperationID, 1, item.Delivery, item.Autonomous, item.Input)
	return plan
}
...
func (plan nextTurnPlan) appendStart(payloads []EventPayload) []EventPayload {
	...
	return append(payloads,
		...
		newCycleStartedEvent(
			plan.item.OperationID, 1, plan.snapshotID,
			plan.item.CommandID, string(DeliveryNextTurn), plan.item.Autonomous,
		),
...
func (h *Harness) resumeReplayedCommand(state *harnessState, command Command) error {
	...
	h.startEngine(state, state.turnSnapshot(state.activeSnapshotID), false)
	return nil
}
