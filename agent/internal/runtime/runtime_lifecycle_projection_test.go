...
func TestOpenLeaderCancellationDoesNotPoisonWaiters(t *testing.T) {
	...
	binding := testBindingAt("/book", "leader-cancel")
...
func TestCloseBindingCallerCancellationDoesNotReleaseFenceEarly(t *testing.T) {
	...
	binding := testBindingAt("/book", "cancelled-close")
...
func TestCloseBindingsEvictsOnlySelectedWorkspace(t *testing.T) {
	...
	aBinding := testBindingAt("/book-a", "session")
	bBinding := testBindingAt("/book-b", "session")
	...
	if err := runtime.CloseBindings(context.Background(), runstate.BindingSelector{Labels: map[string]string{"workspace": "/book-a"}}); err != nil {
		t.Fatal(err)
	}
...
func TestProjectDoesNotCreateEngineOrRetainBindingLease(t *testing.T) {
	...
	binding := testBindingAt("/book", "projection")
	harness, err := runtime.Open(context.Background(), binding)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRuntimeOwnerPanicCompletesOpenAndProjectWaiters(t *testing.T) {
	...
	binding := testBindingAt("/book", "panic-owner")
...
func TestHarnessContainsJournalClosePanicAtActorBoundary(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), testBinding("close-panic"))
	if err != nil {
		t.Fatal(err)
	}
