...
func (s *harnessState) turnSnapshot(id SnapshotID) TurnSnapshot {
	...
	snapshot := TurnSnapshot{
		...
		Cycle:     s.activeCycle, StartedAt: s.activeCycleStartedAt,
		Delivery: s.activeCycleDelivery, Autonomous: s.activeCycleAutonomous,
		Input:         cloneUserInput(s.activeInput),
	...
	if commit := s.domainCommit(DomainCommitInput); commit != nil {
		snapshot.InputCommit = cloneDomainCommitState(commit)
	}
	return snapshot
}
