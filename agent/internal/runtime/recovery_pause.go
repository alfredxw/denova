...
func (h *Harness) resumeRecoveryPausedCommand(state *harnessState, command Command) error {
	...
	if recovery := state.inputRecovery; recovery != nil && recovery.CommandID == command.commandID() {
		return h.resumePendingInputMaterialization(h.lifecycle, state, true)
	}
...
func (h *Harness) resumeRecoveryPausedInput(ctx context.Context, state *harnessState, commandID CommandID, delivery DeliveryKind) error {
	...
	capabilities, err := h.prepareAdmission(ctx, state, item.CommandID, state.activeOperation, nextCycle, item.Delivery, item.Autonomous, item.Input)
	if err != nil {
...
func (h *Harness) resumeAutonomousContinuation(ctx context.Context, state *harnessState, item QueuedInput) error {
	...
	nextCycle := state.activeCycle + 1
	snapshotID := SnapshotID(newID("snapshot"))
	capabilities, err := h.prepareAdmission(ctx, state, item.CommandID, state.activeOperation, nextCycle, item.Delivery, true, item.Input)
	if err != nil {
	...
	payloads = append(payloads,
		...
		newCycleStartedEvent(
			state.activeOperation, nextCycle, snapshotID,
			item.CommandID, string(item.Delivery), true,
		),
		InputMaterializationRecoveryPendingEvent{
			OperationID: state.activeOperation, Cycle: nextCycle,
...
func (h *Harness) resumePendingInputMaterialization(ctx context.Context, state *harnessState, recovered bool) error {
	...
	h.startEngine(state, state.turnSnapshot(state.activeSnapshotID), recovered)
	return nil
}
