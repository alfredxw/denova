...
func TestJournalCheckpointAdapterRoundTripsAllPendingObligations(t *testing.T) {
	...
	startedAt := time.Date(2026, 8, 13, 8, 30, 0, 0, time.FixedZone("UTC+08:00", 8*60*60))
	payloads := []EventPayload{
		...
		CycleStartedEvent{OperationID: "operation", Cycle: 1, SnapshotID: "snapshot", StartedAt: startedAt},
	...
	if !restored.turnSnapshot("").StartedAt.Equal(startedAt) {
		t.Fatalf("durable cycle start = %s, want %s", restored.turnSnapshot("").StartedAt, startedAt)
	}
