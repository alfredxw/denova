...
func TestActorDisplayRetentionIsBoundedAndColdCommandReplayStaysIdempotent(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), testBindingAt("/book", "retention"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestProjectConservativelyReportsUnfinishedJournalWithoutCreatingActor(t *testing.T) {
	...
	ref := testBinding("unfinished-projection")
	...
	status, err := runtime.Project(context.Background(), testBindingAt("/book", "unfinished-projection"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestCanonicalFileSessionLogStoresToolPayloadDescriptorsInsteadOfRawContent(t *testing.T) {
	root := t.TempDir()
	store, err := newCanonicalFileJournalStore(root)
	if err != nil {
		t.Fatal(err)
	}
	...
	harness, err := runtime.Open(context.Background(), testBindingAt("/book", "payload-descriptor"))
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(manifest, `"version":1`) || !strings.Contains(manifest, `"namespace":"test"`) ||
		!strings.Contains(manifest, `"id":"payload-descriptor"`) || !strings.Contains(manifest, `"agent.profile":"default"`) {
		t.Fatalf("versioned Session manifest missing: %s", manifest)
	}
}
