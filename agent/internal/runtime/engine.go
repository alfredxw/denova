...
func (h *Harness) startEngine(state *harnessState, snapshot TurnSnapshot, recovery bool) {
	controls := make(chan EngineControl, state.memoryLimits.normalized().MaxPendingInteractions+4)
	state.engineControls = controls
	request := EngineRequest{Binding: h.binding.Clone(), Snapshot: snapshot, Controls: controls, Recovery: recovery}
	go func() {
		returned := false
		defer func() {
...
func (h *Harness) handleEngineEvent(state *harnessState, request engineEventRequest) error {
	...
	switch event := request.event.(type) {
	case EngineContextNormalized:
		if event.RepairCount <= 0 || event.MessagesBefore < 0 || event.MessagesAfter < 0 ||
			event.RepairCount > max(1, event.MessagesBefore+event.MessagesAfter) {
			return fmt.Errorf("invalid context normalization metrics")
		}
		state.publish(Event{Cursor: state.cursor, Durability: EventEphemeral, Payload: ContextNormalizedEvent{
			OperationID: state.activeOperation, Cycle: state.activeCycle,
			RepairCount: event.RepairCount, MessagesBefore: event.MessagesBefore, MessagesAfter: event.MessagesAfter,
		}})
		return nil
	case EngineCleanupStarted:
		state.publish(Event{Cursor: state.cursor, Durability: EventEphemeral, Payload: CleanupStartedEvent{
			OperationID: state.activeOperation, Cycle: state.activeCycle, ID: event.ID,
			Reason: event.Reason, Automatic: event.Automatic, Transient: event.Transient, Metrics: event.Metrics,
		}})
		return nil
	case EngineCleanupCompleted:
		state.publish(Event{Cursor: state.cursor, Durability: EventEphemeral, Payload: CleanupCompletedEvent{
			OperationID: state.activeOperation, Cycle: state.activeCycle, ID: event.ID,
			Reason: event.Reason, Automatic: event.Automatic, Transient: event.Transient, Metrics: event.Metrics,
		}})
		return nil
	case EngineCleanupFailed:
		state.publish(Event{Cursor: state.cursor, Durability: EventEphemeral, Payload: CleanupFailedEvent{
			OperationID: state.activeOperation, Cycle: state.activeCycle, ID: event.ID,
			Reason: event.Reason, Automatic: event.Automatic, Metrics: event.Metrics,
		}})
		return nil
	case EngineCleanupSkipped:
		state.publish(Event{Cursor: state.cursor, Durability: EventEphemeral, Payload: CleanupSkippedEvent{
			OperationID: state.activeOperation, Cycle: state.activeCycle, ID: event.ID,
			Reason: event.Reason, Automatic: event.Automatic, Metrics: event.Metrics,
		}})
		return nil
	case EngineCompactionStarted:
		state.publish(Event{Cursor: state.cursor, Durability: EventEphemeral, Payload: CompactionStartedEvent{
			OperationID: state.activeOperation, Cycle: state.activeCycle,
			ID: event.ID, Automatic: event.Automatic, Metrics: event.Metrics,
		}})
		return nil
	case EngineCompactionFailed:
		state.publish(Event{Cursor: state.cursor, Durability: EventEphemeral, Payload: CompactionFailedEvent{
			OperationID: state.activeOperation, Cycle: state.activeCycle,
			ID: event.ID, Reason: event.Reason, Automatic: event.Automatic,
			ConsecutiveFailures: event.ConsecutiveFailures, FailureFuseOpen: event.FailureFuseOpen,
			Metrics: event.Metrics,
		}})
		return nil
	case EngineCompactionSkipped:
		state.publish(Event{Cursor: state.cursor, Durability: EventEphemeral, Payload: CompactionSkippedEvent{
			OperationID: state.activeOperation, Cycle: state.activeCycle,
			ID: event.ID, Reason: event.Reason, Automatic: event.Automatic,
			ConsecutiveFailures: event.ConsecutiveFailures, FailureFuseOpen: event.FailureFuseOpen,
			Metrics: event.Metrics,
	...
	case EngineThinkingDelta:
		...
		return nil
	case EngineNestedEvent:
		if err := validateEventSource(event.Source); err != nil {
			return err
		}
		if strings.TrimSpace(event.SessionID) == "" || strings.TrimSpace(event.ChildRunID) == "" ||
			strings.TrimSpace(event.PayloadType) == "" || len(event.Payload) == 0 || !json.Valid(event.Payload) {
			return fmt.Errorf("nested Agent event identity or payload is invalid")
		}
		if event.ChildDurability != EventDurable && event.ChildDurability != EventEphemeral {
			return fmt.Errorf("nested Agent event has invalid child durability %q", event.ChildDurability)
		}
		if int64(len(event.Payload)) > state.memoryLimits.normalized().MaxActiveOutputBytes {
			return &ByteBudgetError{
				Scope: ByteBudgetActiveOutput, Incoming: int64(len(event.Payload)),
				Limit: state.memoryLimits.normalized().MaxActiveOutputBytes,
			}
		}
		state.publish(Event{Cursor: state.cursor, Durability: EventEphemeral, Payload: NestedEventEvent{
			OperationID: state.activeOperation, Source: cloneEventSource(event.Source),
			SessionID: event.SessionID, ChildCursor: event.ChildCursor,
			ChildDurability: event.ChildDurability, ChildRunID: event.ChildRunID,
			PayloadType: event.PayloadType, Payload: cloneRawMessage(event.Payload),
		}})
		return nil
	case EngineToolInputStarted:
		...
		if err := validateLiveToolMetadata(event.Metadata); err != nil {
			return err
		}
		state.publish(Event{
			Cursor: state.cursor, Durability: EventEphemeral,
			Payload: ToolInputStartedEvent{
				...
				Index: event.Index, Metadata: cloneRawMessage(event.Metadata),
	...
	case EngineAssistantFinal:
		...
		seenCapabilities := make(map[string]struct{}, len(event.CapabilityUpdates))
		for _, update := range event.CapabilityUpdates {
			if _, duplicate := seenCapabilities[update.Capability]; duplicate {
				return fmt.Errorf("assistant final contains duplicate capability update %q", update.Capability)
			}
			seenCapabilities[update.Capability] = struct{}{}
			payload := CapabilityStateCommittedEvent{
				Capability: update.Capability, Expected: update.Expected,
				State: cloneRawMessage(update.State), Deleted: update.Delete,
				OperationID: request.operation, Cycle: request.cycle,
			}
			if err := state.validateCapabilityStateEvent(payload); err != nil {
				return err
		...
		if event.CleanupCompleted != nil {
			completed := *event.CleanupCompleted
			if completed.Transient {
				return errors.New("assistant final Cleanup completion cannot be transient")
			}
			if _, updated := seenCapabilities["agent.cleanup"]; !updated {
				return errors.New("assistant final Cleanup completion requires an atomic Cleanup capability update")
			}
			payloads = append(payloads, CleanupCompletedEvent{
				OperationID: request.operation, Cycle: request.cycle, ID: completed.ID,
				Reason: completed.Reason, Automatic: completed.Automatic, Metrics: completed.Metrics,
			})
		}
	...
	case EngineToolStarted:
			...
			if existing.OperationID == state.activeOperation && existing.Cycle == state.activeCycle && existing.Name == event.Name &&
				...
				if event.ExecutionAuthorized {
					state.publish(Event{Cursor: state.cursor, Durability: EventEphemeral, Payload: ToolStartedEvent{
						OperationID: state.activeOperation, Cycle: state.activeCycle,
						CallID: event.CallID, ProviderCallID: event.ProviderCallID, Name: event.Name, Index: event.Index,
						Arguments: cloneRawMessage(event.Arguments), Metadata: cloneRawMessage(event.Metadata),
						Source: cloneEventSource(event.Source),
					}})
				}
				return nil
		...
		if err == nil && event.ExecutionAuthorized {
			state.publish(Event{Cursor: state.cursor, Durability: EventEphemeral, Payload: ToolStartedEvent{
				OperationID: state.activeOperation, Cycle: state.activeCycle,
				CallID: event.CallID, ProviderCallID: event.ProviderCallID, Name: event.Name, Index: event.Index,
				Arguments: cloneRawMessage(event.Arguments), Metadata: cloneRawMessage(event.Metadata),
	...
	case EngineToolProgress:
		...
		state.publish(Event{
			...
			Payload: ToolProgressEvent{
				CallID: event.CallID, ProviderCallID: event.ProviderCallID, Name: event.Name, Index: event.Index,
				Delta: event.Delta, Metadata: cloneRawMessage(event.Metadata), Source: cloneEventSource(event.Source),
			},
		})
		return nil
	...
	case EngineToolFinished:
		...
		if err == nil {
			state.publish(Event{Cursor: state.cursor, Durability: EventEphemeral, Payload: ToolOutputEvent{
				OperationID: state.activeOperation, Cycle: state.activeCycle,
				CallID: event.CallID, ProviderCallID: event.ProviderCallID, Name: name, Index: event.Index,
				Result: event.Result, IsError: event.IsError, Metadata: cloneRawMessage(event.Metadata),
...
func validateLiveToolMetadata(metadata json.RawMessage) error {
	if len(metadata) == 0 {
		return nil
	}
	if len(metadata) > 64<<10 || !json.Valid(metadata) {
		return fmt.Errorf("live tool metadata must contain valid JSON of at most %d bytes", 64<<10)
	}
	return nil
}
...
func (h *Harness) handleEngineDone(state *harnessState, request engineDoneRequest) {
	...
	if pending := state.pendingDomainCommit(); pending != nil {
		cause := ""
		if request.err != nil {
			cause = ": " + request.err.Error()
		}
		h.failActiveOperation(state, engineDoneRequest{
			operation: request.operation, cycle: request.cycle,
			err: fmt.Errorf("authorized %s domain commit ended without a durable receipt%s", pending.Identity.Stage, cause),
		})
		return
	}
...
func (h *Harness) startQueuedCycle(state *harnessState, item QueuedInput) {
	...
	capabilities, err := h.prepareAdmission(context.Background(), state, item.CommandID, state.activeOperation, nextCycle, item.Delivery, item.Autonomous, item.Input)
	if err != nil {
		h.failActiveOperation(state, engineDoneRequest{
	...
	payloads = append(payloads,
		...
		newCycleStartedEvent(
			state.activeOperation, nextCycle, snapshotID,
			item.CommandID, string(item.Delivery), item.Autonomous,
		),
	...
	if err := h.resumePendingInputMaterialization(h.lifecycle, state, false); err != nil {
