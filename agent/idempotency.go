...
// ValidateIdempotencyKey applies the durable Agent command-identity boundary.
...
func ValidateIdempotencyKey(key string) error {
	...
	if key == "" {
		return fmt.Errorf("%w: idempotency key is required for durable retry", ErrInvalidInput)
	}
