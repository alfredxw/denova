...
func TestPendingInputAggregateBudgetRejectsBeforeDurableAcceptance(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), testBindingAt("/book", "pending-byte-budget"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestActiveStreamOverflowSettlesTypedIncomplete(t *testing.T) {
	...
	t.Cleanup(func() { _ = runtime.Close(context.Background()) })
	harness, err := runtime.Open(context.Background(), testBindingAt("/book", "active-byte-budget"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestRetainedPayloadBytesStayWithinPerBindingBudget(t *testing.T) {
	...
	t.Cleanup(func() { _ = runtime.Close(context.Background()) })
	harness, err := runtime.Open(context.Background(), testBindingAt("/book", "retained-byte-budget"))
	if err != nil {
		t.Fatal(err)
	}
