...
func TestCompactIfNeededIsACompactingDurableOperation(t *testing.T) {
	...
	harness, err := runtime.Open(context.Background(), testBindingAt("/book", "session"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestContextCompactionRestoreDescriptorIsBoundedAndValidJSON(t *testing.T) {
	for name, descriptor := range map[string]json.RawMessage{
		...
		t.Run(name, func(t *testing.T) {
			...
			defer runtime.Close(context.Background())
			harness, err := runtime.Open(context.Background(), testBindingAt("/book", "descriptor-"+name))
			if err != nil {
				t.Fatal(err)
			}
...
func TestContextCompactionRestoreDescriptorIsDeepClonedAndFingerprinted(t *testing.T) {
	...
	defer runtime.Close(context.Background())
	harness, err := runtime.Open(context.Background(), testBindingAt("/book", "descriptor-clone"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestRemoveCompactionAbortSettlesWithoutCanonicalCommit(t *testing.T) {
	...
	defer runtime.Close(context.Background())
	harness, err := runtime.Open(context.Background(), testGameBinding("/book", "story", "main"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestCompactionCrashRecoverySettlesAcknowledgedCanonicalWrite(t *testing.T) {
	store := runstate.NewMemoryJournalStore()
	ref := testBinding("compaction-crash")
	...
	defer runtime.Close(context.Background())
	harness, err := runtime.Open(context.Background(), testBindingAt("/book", "compaction-crash"))
	if err != nil {
		t.Fatal(err)
	}
	status, err := harness.Status(context.Background())
