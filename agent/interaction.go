...
// LocalizedText keeps host-generated permission copy bilingual without tying
...
// PermissionOption remains host-localized because permission choices are
// generated by policy rather than supplied by the model-facing Ask schema.
...
// askResolutionContract is the durable part of an Ask request. Presentation
// copy is deliberately excluded so an already-emitted Ask can still be
// answered after its model-facing text schema changes.
type askResolutionContract struct {
	ID         string
	Questions  []askResolutionQuestion
	AllowOther bool
}

type askResolutionQuestion struct {
	ID            string
	OptionValues  []string
	Multiple      bool
	AllowFreeText bool
...
func validatePermissionOptions(options []PermissionOption, canRemember bool) error {
...
func validateInteractionQuestion(question InteractionQuestion) error {
	...
	if len(question.Prompt) > maxInteractionQuestionTextBytes {
	...
	for _, option := range question.Options {
		...
		if strings.TrimSpace(option.Label) == "" {
			return fmt.Errorf("option %q label is required", option.Value)
		}
		if option.Description != "" && strings.TrimSpace(option.Description) == "" {
			return fmt.Errorf("option %q description cannot be blank", option.Value)
		}
		if len(option.Label)+len(option.Description) > maxInteractionOptionTextBytes {
...
func askResolutionContractFromRequest(request InteractionRequest) askResolutionContract {
	contract := askResolutionContract{
		ID: request.ID, AllowOther: request.AllowOther,
		Questions: make([]askResolutionQuestion, 0, len(request.Questions)),
	}
	for _, question := range request.Questions {
		...
		contract.Questions = append(contract.Questions, askResolutionQuestion{
			ID: question.ID, OptionValues: values,
			Multiple: question.Multiple, AllowFreeText: question.AllowFreeText,
		})
	}
	return contract
}

func decodePersistedAskResolutionContract(encoded json.RawMessage) (askResolutionContract, error) {
	if len(encoded) > maxInteractionRequestBytes+(4<<10) {
		return askResolutionContract{}, fmt.Errorf("persisted ask Interaction exceeds %d bytes", maxInteractionRequestBytes+(4<<10))
	}
	var stored struct {
		ID         string          `json:"id"`
		Kind       InteractionKind `json:"kind"`
	...
	if err := json.Unmarshal(encoded, &stored); err != nil {
		return askResolutionContract{}, fmt.Errorf("decode persisted Ask resolution contract: %w", err)
	}
	if stored.Kind != InteractionAsk {
		return askResolutionContract{}, fmt.Errorf("persisted Interaction kind %q is not Ask", stored.Kind)
	}
	if len(stored.Permission) != 0 && string(stored.Permission) != "null" {
		return askResolutionContract{}, errors.New("persisted Ask Interaction contains a Permission presentation")
	}
	contract := askResolutionContract{
		ID: stored.ID, AllowOther: stored.AllowOther,
		Questions: make([]askResolutionQuestion, 0, len(stored.Questions)),
	}
	...
	if err := validateAskResolutionContract(contract); err != nil {
		return askResolutionContract{}, err
	}
	return contract, nil
}

func validateAskResolutionContract(contract askResolutionContract) error {
	if err := validateInteractionStableID("Interaction Request ID", contract.ID); err != nil {
		return err
	}
	if len(contract.Questions) < 1 || len(contract.Questions) > 3 {
		return errors.New("persisted Ask Interaction requires one to three questions")
	}
	seenQuestions := make(map[string]struct{}, len(contract.Questions))
	for index, question := range contract.Questions {
		if err := validateInteractionStableID("ID", question.ID); err != nil {
			return fmt.Errorf("persisted Ask question %d: %w", index, err)
		}
		if _, duplicate := seenQuestions[question.ID]; duplicate {
			return fmt.Errorf("persisted Ask question %q is duplicated", question.ID)
		}
		seenQuestions[question.ID] = struct{}{}
		if len(question.OptionValues) == 0 {
			if !question.AllowFreeText || question.Multiple {
				return fmt.Errorf("persisted free-text question %q must allow text and cannot be multi-select", question.ID)
			}
			continue
		}
		...
		for _, value := range question.OptionValues {
			if err := validateInteractionStableID("option value", value); err != nil {
				return fmt.Errorf("persisted Ask question %q: %w", question.ID, err)
			}
			if strings.EqualFold(value, reservedInteractionOtherValue) {
				return fmt.Errorf("persisted Ask option value %q is reserved", value)
			}
			if _, duplicate := seenOptions[value]; duplicate {
				return fmt.Errorf("persisted Ask question %q duplicates option %q", question.ID, value)
			}
...
func resolvePersistedStandardAsk(encoded json.RawMessage, response InteractionResponse) (InteractionResolution, error) {
	contract, err := decodePersistedAskResolutionContract(encoded)
	if err != nil {
		return InteractionResolution{}, err
	}
	if response.Cancelled {
...
func resolveAskResolution(contract askResolutionContract, response InteractionResponse) (InteractionResolution, error) {
	...
	for _, answer := range response.Answers {
		...
		allowed := make(map[string]struct{}, len(question.OptionValues))
		for _, value := range question.OptionValues {
	...
	if len(answers) != len(contract.Questions) {
		return InteractionResolution{}, errors.New("every Interaction question requires an answer")
	}
	ordered := make([]InteractionAnswer, 0, len(contract.Questions))
	for _, question := range contract.Questions {
...
func isStandardInteractionPolicy(policy InteractionPolicy) bool {
	_, ok := policy.(standardInteractionPolicy)
	return ok
}

// StandardInteraction returns the built-in validation and normalization
// policy for Ask and permission interactions.
