...
- 当前 Beta 的兼容基线仅为最近一个已发布版本的用户数据；在简单可控范围内保护或迁移这些数据。内部接口、代码契约及最近一次 Release 后从未发布的中间格式不保留兼容，也不作为不兼容变更记录。
...
- 已落地功能以代码、测试和 `AGENTS.md` 中的长期不变量为事实源；`docs/` 只保留仍有独立阅读价值的文档，实施完成且内容已被代码覆盖的设计稿应及时删除，避免形成会过期的第二份实现说明。
- 设计 agent tool 输入参数或者任何受限于 agent 输出的 schema 时，需要充分考虑到 agent 可能犯错的情况，尽可能去包容可修正的错误，以及对于list的情况下的部分成功与部分失败，避免大块输入整体失败导致昂贵的重试。
- 所有面向用户的交互都要分别提供中文和英文本地化，并根据用户当前语言设置只展示对应语言；禁止在同一条文案或提示中拼接中英文。
- 所有注入模型的提示词、上下文片段、工具描述、schema 描述和模型可见工具反馈统一使用英文，不得中英双语重复；面向用户的本地化交互使用独立中英文资源并按当前语言单语展示。
...
- 写作、游戏与共通能力使用同一层级导航；点击一级菜单只进入对应目的地，不维护或隐式改变额外的全局模式。任何时候一级菜单只有一个高亮，且所有菜单行为一致。
...
- 注释、日志、内部错误处理、terminal 固定输出等统一用英文。面向用户的错误必须通过本地化 message / error key 按当前语言展示，不得直接拼接中英文。
...
- 共通能力要同时检查写作和游戏；涉及布局、导航、会话、设置、版本管理时尤其如此。
- 写作、游戏与共通能力均为并列一级菜单，不提供独立模式切换器；任意时刻只能有一个一级菜单高亮。
- 用户可见文案、按钮、错误提示和空状态必须同时提供中文和英文翻译，并随用户当前语言设置只展示一种语言；禁止拼接中英文，也不要只改中文或只改英文。
