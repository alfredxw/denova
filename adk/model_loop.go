...
func (agent *Agent) callModelWithRetry(
	...
	if streaming {
		streamOutput = &modelStreamOutput{agent: agent, events: events}
	...
	for attempt := 0; ; attempt++ {
		message, err, deliveredOnStream := agent.callModel(ctx, model, currentMessages, currentOptions, streaming, events, cancel, streamOutput)
...
func (output *modelStreamOutput) expose() {
	...
	output.events.Send(output.agent.messageEvent(nil, stream, Assistant, ""))
}
...
func (agent *Agent) callModel(
	...
	if !streaming {
		...
		events.Send(agent.messageEvent(message.Clone(), nil, Assistant, ""))
