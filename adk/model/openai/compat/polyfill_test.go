...
type fakeChatModel struct {
	fixedMsg *adk.Message
	stream   *adk.StreamReader[*adk.Message]
}

func (f *fakeChatModel) Generate(_ context.Context, _ []*adk.Message, _ ...adk.ModelOption) (*adk.Message, error) {
	return f.fixedMsg, nil
}
func (f *fakeChatModel) Stream(_ context.Context, _ []*adk.Message, _ ...adk.ModelOption) (*adk.StreamReader[*adk.Message], error) {
	if f.stream != nil {
		return f.stream, nil
	}
	if f.fixedMsg == nil {
		return adk.StreamReaderFromArray([]*adk.Message{}), nil
	}
	return adk.StreamReaderFromArray([]*adk.Message{f.fixedMsg}), nil
}
func (f *fakeChatModel) WithTools(_ []*adk.ToolInfo) (adk.ToolCallingChatModel, error) {
	return f, nil
}
...
var nonStandardProviderCfg = openai.Config{
...
func TestThinkingExtraFields_ProviderSupport(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			cfg:  openai.Config{BaseURL: "https://api.deepseek.com/v1", Model: "deepseek-reasoner"},
			in:   &enabled,
			want: map[string]any{"enable_thinking": true},
		},
		{
			name: "explicit false still sent for supported providers",
			cfg:  openai.Config{BaseURL: "https://api.deepseek.com/v1", Model: "deepseek-chat"},
			in:   &disabled,
			want: map[string]any{"enable_thinking": false},
		},
		{
			name: "gemini endpoint skips unsupported field",
			cfg:  openai.Config{BaseURL: "https://generativelanguage.googleapis.com/v1beta/openai/", Model: "gemini-3.5-flash"},
			in:   &enabled,
			want: nil,
		},
		{
			name: "gemini model behind proxy skips unsupported field",
			cfg:  openai.Config{BaseURL: "https://proxy.example/v1", Model: "gemini-2.5-flash"},
			in:   &enabled,
			want: nil,
		},
		{
...
func TestWrap_NonStandardProvider_RepairsToolCallAndThink(t *testing.T) {
	...
	content := "<think>Let me load the skill.</think>\n\n" +
		...
		"</tool_call>"
	inner := &fakeChatModel{fixedMsg: &adk.Message{Role: adk.Assistant, Content: content}}
...
func TestTextToolCallIDsRemainUniqueAcrossResponses(t *testing.T) {
	...
	for range 4 {
		message := &adk.Message{Role: adk.Assistant, Content: content}
...
func TestWrap_NonStandardProvider_PreservesNativeToolCalls(t *testing.T) {
	idx := 0
	inner := &fakeChatModel{fixedMsg: &adk.Message{
		Role:    adk.Assistant,
		Content: "正文",
...
func TestWrap_OpenAIProvider_PassThrough(t *testing.T) {
	inner := &fakeChatModel{fixedMsg: &adk.Message{Role: adk.Assistant, Content: "raw <think>oops</think> done"}}
	cfg := openai.Config{BaseURL: "https://api.openai.com/v1", Model: "gpt-4o"}
...
func TestWrap_NonStandardProvider_StreamReturnsBeforeUpstreamEOF(t *testing.T) {
	upstream, writer := adk.Pipe[*adk.Message](1)
	inner := &fakeChatModel{stream: upstream}
	wrapped := Wrap(inner, nonStandardProviderCfg)

	writer.Send(&adk.Message{Role: adk.Assistant, Content: "第一段"}, nil)

	result := make(chan *adk.StreamReader[*adk.Message], 1)
	...
	var stream *adk.StreamReader[*adk.Message]
	select {
	case err := <-errs:
	...
	writer.Send(&adk.Message{Role: adk.Assistant, Content: "第二段"}, nil)
	writer.Close()
	got, err = stream.Recv()
	if err != nil {
...
func TestWrap_NonStandardProvider_StreamParsesTextToolCallAfterStreamingPrelude(t *testing.T) {
	upstream := adk.StreamReaderFromArray([]*adk.Message{
		{Role: adk.Assistant, Content: "先说明"},
		{Role: adk.Assistant, Content: "<tool_"},
		{Role: adk.Assistant, Content: "call><invoke name=\"read_file\"><path>chapters/ch01.md</path></invoke></tool_call>"},
	})
...
func TestWrap_NonStandardProvider_StreamStripsInlineThinkWithoutWaitingForEOF(t *testing.T) {
	upstream, writer := adk.Pipe[*adk.Message](1)
	wrapped := Wrap(&fakeChatModel{stream: upstream}, nonStandardProviderCfg)
	writer.Send(&adk.Message{Role: adk.Assistant, Content: "<think>先想"}, nil)
	...
	writer.Send(&adk.Message{Role: adk.Assistant, Content: "</think>\n正文"}, nil)
	writer.Close()
