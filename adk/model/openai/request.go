...
func (model *ChatModel) request(input []*adk.Message, stream bool, opts ...adk.ModelOption) (sdk.ChatCompletionNewParams, []option.RequestOption, error) {
	messages, err := requestMessages(input)
	if err != nil {
		return sdk.ChatCompletionNewParams{}, nil, err
	}
	common := adk.GetCommonOptions(model.options, opts...)
	tools := common.Tools
...
func requestMessages(messages []*adk.Message) ([]sdk.ChatCompletionMessageParamUnion, error) {
...
func requestMessage(message *adk.Message) (sdk.ChatCompletionMessageParamUnion, error) {
	switch message.Role {
	case adk.System:
...
func requestTools(tools []*adk.ToolInfo) ([]sdk.ChatCompletionToolUnionParam, error) {
	if tools == nil {
		return nil, nil
	}
...
func filterTools(tools []*adk.ToolInfo, names []string) []*adk.ToolInfo {
...
func requestToolChoice(choice *adk.ToolChoice, toolCount int) (sdk.ChatCompletionToolChoiceOptionUnionParam, error) {
	if choice == nil {
		return sdk.ChatCompletionToolChoiceOptionUnionParam{}, nil
	}
	result := sdk.ChatCompletionToolChoiceOptionUnionParam{}
	switch *choice {
	case adk.ToolChoiceForbidden:
		result.OfAuto = sdk.String("none")
	case adk.ToolChoiceAllowed:
		result.OfAuto = sdk.String("auto")
	case adk.ToolChoiceForced:
		if toolCount == 0 {
