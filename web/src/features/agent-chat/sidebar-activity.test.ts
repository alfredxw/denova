...
const project: AgentChatProject = {
  ...
  total: 3,
  sessions: [
    { id: 'open', title: 'Open chat', created_at: '', updated_at: '', message_count: 2, running: true, active: false },
    { id: 'detached', title: 'Detached task', created_at: '', updated_at: '', message_count: 3, running: true, active: false },
    { id: 'history', title: 'History only', created_at: '', updated_at: '', message_count: 4, running: false, active: false },
  ],
}

const state: AgentChatProjectTabState = {
  tabs: [
    { kind: 'agent', id: 'agent-tab', workspace: project.path, sessionId: 'open', group: 'primary' },
    { kind: 'page', id: 'reader-tab', workspace: project.path, pageId: 'reader', group: 'primary' },
    { kind: 'terminal', id: 'terminal-tab', workspace: project.path, profileId: 'shell', title: '', group: 'secondary' },
    { kind: 'review', id: 'review-tab', workspace: project.path, threadID: 'review', group: 'secondary' },
  ],
...
describe('AgentChat sidebar activity projection', () => {
  it('filters tab order to conversations and terminals, then appends detached running work', () => {
    ...
    expect(activities[0]).toMatchObject({ status: 'running', paneVisible: true, focused: false })
    expect(activities[1]).toMatchObject({ status: 'ready', paneVisible: true, focused: true })
    expect(activities[2]).toMatchObject({ status: 'running', paneVisible: false })
  ...
  it('summarizes running and error states without classifying a normal exit as failure', () => {
    ...
    expect(summarizeSidebarActivities(activities)).toEqual({ total: 3, running: 2, attention: 1 })
  })
})
