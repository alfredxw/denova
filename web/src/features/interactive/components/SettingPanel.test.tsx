...
describe('SettingPanel', () => {
  ...
  it('shows every preset type with a fixed availability label', async () => {
    ...
    expect(screen.getByText('方案预设目录')).toBeInTheDocument()
    expect(screen.getByText('按创作能力分类管理')).toBeInTheDocument()
    ...
    expect(sectionToggle('故事导演')).toHaveClass('h-8', 'text-left', 'whitespace-nowrap')
    expect(sectionToggle('故事导演')).not.toHaveClass('h-auto', 'min-h-10')
    expect(sectionToggle('叙事风格')).toHaveAttribute('aria-expanded', 'true')
    expect(sectionToggle('故事导演')).toHaveAttribute('aria-expanded', 'false')
    ...
    expect(screen.getByRole('button', { name: '新建故事导演' })).toHaveClass('top-2.5')
    ...
    const search = screen.getByRole('textbox', { name: '搜索方案名称或描述' })
    await user.type(search, '不存在的方案')
