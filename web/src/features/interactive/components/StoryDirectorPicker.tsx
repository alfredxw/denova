...
export function StoryDirectorPicker({ story, storyDirectors, onChange, layout = 'inline' }: StoryDirectorPickerProps) {
    ...
    <Popover open={open} onOpenChange={(nextOpen) => setOpen(Boolean(story) && nextOpen)}>
      <PopoverTrigger asChild>
        <Button
          ...
          size="sm"
          disabled={!story}
          className={`nova-field ${sidebar ? 'w-full' : 'w-[180px]'} justify-between px-3 py-0.5 text-xs font-normal text-[var(--nova-text)] focus:ring-0`}
          aria-label={t('storyPicker.chooseStoryDirector')}
          aria-expanded={open}
        >
          <span className="min-w-0 flex-1 truncate text-left">{selectedDirector?.name || t('storyPicker.chooseStoryDirector')}</span>
      ...
      <PopoverContent
        align="start"
        sideOffset={6}
        className={`${sidebar ? 'w-[min(calc(100vw-2rem),22rem)]' : 'w-[210px]'} max-h-[min(70dvh,28rem)] overflow-y-auto rounded-[var(--nova-radius)] border border-[var(--nova-border)] bg-[var(--nova-surface-2)] p-1 text-[var(--nova-text)] shadow-[var(--nova-shadow)]`}
      >
        <div role="listbox" aria-label={t('storyPicker.chooseStoryDirector')} className="space-y-1">
          {normalizedDirectors.map((director) => {
            const selected = director.id === selectedDirector?.id
            return (
              <button
                key={director.id}
                type="button"
              ...
              >
                <span className="min-w-0 flex-1 truncate">{director.name || director.id}</span>
  ...
  if (sidebar) {
    return (
      <div className="flex min-w-0 flex-col gap-1.5">
        <span className="shrink-0 text-[11px] font-medium text-[var(--nova-text-faint)]">{t('storyPicker.storyDirector')}</span>
        {selector}
      </div>
    )
  }
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span className="shrink-0 text-[11px] font-medium text-[var(--nova-text-faint)]">{t('storyPicker.directorLabel')}</span>
      {selector}
    </div>
  )
}
