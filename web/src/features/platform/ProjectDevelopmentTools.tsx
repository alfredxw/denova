...
import { management, platformError, type Candidate, type CatalogEntry, type DevelopmentSource, type Instance, type RuntimeSnapshot } from './api'
...
function SourceDevelopmentActions({ source, visible, refreshSignal, beforeAction, onOpenFile }: ProjectDevelopmentToolsProps & { source: DevelopmentSource }) {
  ...
  const tests = (previews.data ?? []).filter(instance => instance.preview && instance.projectId === source.projectId && instance.gameId === source.manifest?.id)
  ...
  return <>
    <Button size="sm" variant="ghost" disabled={busy} onClick={() => void run('Open source manifest', async () => { if (await saveSource()) setConfigurationOpen(true) })}>{t('platform.editManifest')}</Button>
    <Button size="sm" variant="outline" disabled={busy || !!player} onClick={() => void run('Build', async () => {
      ...
      else { setDiagnostic(t('platform.noBuild')); recordFeedback('This source declares no build command. Proceed to package validation.') }
    })}>{t('platform.build')}</Button>
    <Button size="sm" disabled={busy || !!player} onClick={() => void run('Package validation', async () => {
      if (!await saveSource()) return
    ...
    })}>{t('platform.checkAndPackage')}</Button>
    {tests.length > 0 && <Button size="sm" variant="ghost" onClick={() => setTestsOpen(true)}>{t('platform.testSaves', { count: tests.length })}</Button>}
    {installed && <Button size="sm" variant="ghost" onClick={() => openInstalledExtension(installed.kind, installed.id)}>{t('platform.viewInstalled')}</Button>}
    ...
    <Dialog open={visible && configurationOpen} onOpenChange={open => { if (!open) void run('Save source manifest', async () => { if (await saveSource()) setConfigurationOpen(false) }) }}>
      <DialogContent className="max-h-[85dvh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader><DialogTitle>{t('platform.editManifest')}</DialogTitle><DialogDescription>{t('platform.configurationHelp')}</DialogDescription></DialogHeader>
        <SourceManifestEditor source={source} refreshSignal={refreshSignal} onSaved={refresh} onFlushChange={setManifestFlush} onOpenFile={path => {
          void run('Open source file', async () => { if (await saveSource()) { setConfigurationOpen(false); onOpenFile(path) } })
        }} />
      </DialogContent>
    </Dialog>
    <InstallDialog key={candidate?.candidateId ?? 'development'} projectId={source.projectId} open={visible && !!candidate} candidate={candidate} onCandidate={setCandidate} onOpenChange={() => {}}
      onInstalled={refresh} onFeedback={recordFeedback} onPlay={runtime => { setPlayer(runtime); refresh() }} />
