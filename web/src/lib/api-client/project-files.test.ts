...
describe('project files API', () => {
  ...
  it('keeps project identity and paths encoded at the transport boundary', async () => {
    ...
    apiMock.use(
      apiRoute.post('/api/projects/project%20one/files/resolve', async ({ request }) => {
      ...
      apiRoute.get('/api/projects/project%20one/files/file', ({ request }) => {
        const url = new URL(request.url)
      ...
      apiRoute.put('/api/projects/project%20one/files/file', async ({ request }) => {
        requests.push({ method: request.method, path: new URL(request.url).pathname, body: await request.json() })
        return jsonResponse({ project_id: 'project one', path: 'src/main file.ts', revision: 'r2', changed: true })
      }),
    )
  ...
  it('preserves per-item partial operation results', async () => {
    apiMock.use(
      apiRoute.post('/api/projects/p1/files/operations', async ({ request }) => {
  ...
  it('reveals only a project-relative path through the scoped endpoint', async () => {
    apiMock.use(
      apiRoute.post('/api/projects/p1/files/reveal', async ({ request }) => {
        expect(await request.json()).toEqual({ path: 'chapters/one.md' })
        return jsonResponse({ project_id: 'p1', path: 'chapters/one.md' })
      }),
    )
  ...
  it('treats an optional missing file as data and shares concurrent reads', async () => {
    let requests = 0
    apiMock.use(
      apiRoute.get('/api/projects/project-one/files/file', ({ request }) => {
        const url = new URL(request.url)
        if (url.searchParams.get('optional') !== 'true') return new Response(null, { status: 500 })
        requests += 1
        return jsonResponse({
          project_id: 'project-one',
  ...
  it('rejects responses from a different Project scope', async () => {
    apiMock.use(
      apiRoute.get('/api/projects/project-one/files/file', () => jsonResponse({
