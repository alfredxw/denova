...
export function ChangesList({ changes, onOpenDiff }: ChangesListProps) {
  ...
  return (
    <div className="min-w-0 space-y-0.5 overflow-hidden">
      {changes.map(change => (
        <button key={`${change.status}:${change.path}`} type="button" className="group flex w-full min-w-0 items-center gap-2 overflow-hidden rounded px-1.5 py-1 text-left hover:bg-[var(--nova-hover)]" onClick={() => onOpenDiff(change.path)}>
