...
/** Shared top-bar trigger for switching the active content context without changing modes. */
  ...
  return (
    <button
      ...
      className={cn(
        ...
        compact ? 'h-8 max-w-[34vw] px-2 text-[11px]' : 'h-7 max-w-[min(16rem,28vw)] px-2.5 text-[11px]',
        className,
      )}
      {...props}
    >
      <Icon aria-hidden="true" className={cn('h-3.5 w-3.5 shrink-0', compact && 'hidden')} />
      <span className="min-w-0 truncate font-medium">{label}</span>
      <ChevronDown aria-hidden="true" className="h-3.5 w-3.5 shrink-0 text-[var(--nova-text-faint)]" />
    </button>
  )
}
