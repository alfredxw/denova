...
- 新增用户级实验开关“开发者模式”；开启后，写作与游戏模式的一级菜单都会显示共享的“轨迹 / Trajectory”页面，且进入页面不会隐式切换模式。后续 Run 会在权限受限的本地 Trace 中记录完整模型可见 messages、System Prompt、tools schema、assistant Markdown、reasoning、工具入参与结果；若最后一次工具结果没有进入后续模型请求，还会在开发者模式下单独记录带截断状态的工具响应。内容可能包含敏感项目数据，并沿用 Trace 保留策略，已有纯元数据 Trace 不追溯补写。主视图按 Request → Output / Input → 消息或工具组 → 单次 Tool Call 分层，Readable 会将调用与结果按 call ID 配对，Debug 保留每次请求的精确原始消息；工具组默认展开，单次调用可原位查看 Arguments、Response、Schema、Timing 与 Source，System Prompt、Markdown Preview、Raw 和工具定义同样在当前层级展开，不再依赖挤占宽度的右侧检查器。原有多层调用树和事件账本作为次级分析保留。三泳道时间总览支持 TTFT / generation 分段、P50/P95、吞吐、实际时间/压缩空闲/等宽步骤、精确悬停、区间聚焦、滚轮缩放、右键平移和重置，并继续提供 request/tool 行内耗时、token、缓存命中、完整 JSONL 导出、自适应窄屏、深浅主题及中英文。新设置仅为“开发者模式”；关闭时完整内容不会进入 Trace。
- Added the user-level experimental Developer Mode switch. When enabled, both Writing and Game primary navigation expose the shared Trajectory page without implicitly changing the selected content mode. Future runs store complete model-visible messages, System Prompts, tool schemas, assistant Markdown, reasoning, tool payloads, and results in permission-restricted local traces. If a terminal tool result never reaches another model request, Developer Mode records a separate tool response with its truncation state. Content may contain sensitive project data and follows trace retention, while existing metadata-only traces are not backfilled. The primary view is hierarchical: Request → Output/Input → message or tool group → individual Tool Call. Readable mode pairs calls and results by call ID, while Debug preserves each request's exact source messages. Tool groups start expanded; each call opens inline Arguments, Response, Schema, Timing, and Source, while System Prompt, Markdown Preview, Raw, and tool definitions expand at their own level instead of relying on a width-consuming side inspector. The nested call tree and event ledger remain available as secondary analysis. The three-lane overview retains segmented TTFT/generation, P50/P95, throughput, actual/idle-compressed/equal-step scales, exact hover, interval focus, wheel zoom, right-drag pan, and reset, alongside inline request/tool latency, token/cache metrics, complete JSONL export, adaptive compact layouts, light/dark themes, and Chinese/English. Developer Mode is the only new setting; complete content is not persisted while it is disabled.
