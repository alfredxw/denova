...
- 写作页作品目录栏（大纲 tab）针对长章节列表优化导航：书籍设定与最新细纲入口默认固定在独立头部，可用 Pin 按钮切换并记忆偏好，不再随滚动轻微位移；最新细纲直接按名称单击打开，下方只保留历史细纲。最新章、已打开章节与回到顶部各有明确的浮动定位入口；章节切换来自目录面板外部（编辑器 tab、搜索跳转、切回大纲 tab 等）时仍会自动把已打开章节滚动到视野中央并展开所在卷，面板内点击不会抢夺滚动位置。`ChapterOutline` 相关组件同步从 `ModeRouter.tsx` 抽离至 `components/workbench/outline/`。
- The Writing page outline sidebar now handles long chapter lists with a separately pinned Book Settings and latest-outline header. A remembered Pin control can return that header to normal scrolling without sticky-position drift; the latest outline opens directly by name and only older outlines remain below. Clear floaters locate the latest chapter, the open chapter, or the top, while chapter changes originating outside the outline panel still center the open chapter and expand its volume without panel clicks stealing the scroll position. The `ChapterOutline` components were extracted from `ModeRouter.tsx` into `components/workbench/outline/`.
