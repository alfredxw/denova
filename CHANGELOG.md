...
- Agent 提示词与上下文生命周期新增显式 stable-prefix / session-state / turn / checkpoint / audit 契约；可变 Context State 现在以持久化 snapshot 驱动首次注入、无变化零注入、增量更新、显式删除及压缩后重建，不再反复改写稳定前缀。写作模式的 workspace runtime snapshot 与游戏模式的 resident lore 已接入同一协议；自动/手动压缩、只读 inspection、重试和恢复共享重建规则。General、Harness Optimizer、Background Director 与 context-compaction 的长静态提示词迁入嵌入式 Markdown，仍由 `SystemPromptComposition` 统一准入和生成 manifest/hash。开发模型输入日志新增有序 system section、message、Context State 与 tool schema 指纹、相邻请求首个分叉点，以及按 `call_id` 关联的 provider cache-read token 与命中比例；未提供 cache-write 指标时明确标记 unknown。无需新增配置，也未引入 Tool Schema 延迟发现。Beta 不兼容：内部 Agent transcript 升级到 v5，不再读取旧版内部 checkpoint；工作区文件与产品 canonical conversation 数据不受影响。
- Agent prompt and context lifecycles now use explicit stable-prefix, session-state, turn, checkpoint, and audit contracts. Mutable Context State uses a durable snapshot for initial injection, zero output when unchanged, append-only updates, explicit removals, and post-compaction rehydration instead of rewriting the stable prefix. Writing workspace runtime snapshots and Game resident lore share the protocol, and automatic/manual compaction, read-only inspection, retries, and recovery use the same rebuild rules. Long static prompts for General, Harness Optimizer, Background Director, and context compaction now live in embedded Markdown while `SystemPromptComposition` remains the sole admission and manifest/hash authority. Developer model-input logs add ordered system-section, message, Context State, and tool-schema fingerprints, the first divergence between adjacent requests, and provider cache-read tokens and hit ratios correlated by `call_id`; unavailable cache-write metrics are explicitly marked unknown. No new setting or deferred Tool Schema mechanism was added. Beta breaking change: the internal Agent transcript is now v5 and old internal checkpoints are no longer read; workspace files and product-owned canonical conversation data are unaffected.
...
- 修复单个 Agent 回合产生超过 200 条思考、工具与进展展示事务后，冷启动只恢复末尾 assistant canonical message、遗漏本轮 user message，进而触发 `Agent transcript source revision conflict` 并阻止写作或游戏新回合的问题。会话恢复现在分别限制展示事务与 canonical message，通过索引精确补读被展示窗口跨过的消息事务，不重放中间的大量展示数据。写作初始请求的非 2xx JSON 错误现在会显示错误提示，并连同游戏流式错误一起展示服务端 error 与日志 ID / Log ID。无需新增配置。
- Fixed cold startup restoring only the final canonical assistant message after one Agent turn produced more than 200 reasoning, tool, and progress display transactions. The missing user message caused `Agent transcript source revision conflict` and blocked the next Writing or Game turn. Session recovery now bounds display transactions and canonical messages independently, using the index to read message transactions skipped by the display window without replaying the large display prefix. Non-2xx JSON failures from an initial Writing request now raise a visible error toast, and both Writing and streamed Game errors show the server error plus its Log ID. No new setting is required.
...
- 修复内部 Agent transcript 升级后，产品 canonical conversation 已存在但 runtime checkpoint 仍为旧版本时，写作或游戏新回合直接报 `unsupported Agent transcript version` 的问题。Canonical 同步现在不解析旧内部格式，而是原子重建当前 transcript generation，并清理旧 generation 的 Todo、Cleanup 与 Compaction 派生状态；工作区、会话和故事 canonical 数据保持不变，无需新增配置。
- Fixed Writing or Game turns failing with `unsupported Agent transcript version` after an internal transcript upgrade when canonical product conversation data existed but the runtime checkpoint still used the retired version. Canonical synchronization now atomically rebuilds the current transcript generation without parsing the obsolete internal format and removes Todo, Cleanup, and Compaction state derived from that old generation. Workspace, Session, and Story canonical data remain unchanged, with no new setting required.
