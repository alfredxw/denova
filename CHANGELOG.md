...
- 新增用户级实验开关“开发者模式”；开启后，写作与游戏模式的一级菜单都会显示共享的“轨迹 / Trajectory”页面，且进入页面不会隐式切换模式。后续 Run 会在权限受限的本地 Trace 中记录完整模型可见 messages、System Prompt、tools schema、assistant Markdown、reasoning、工具入参与结果；内容可能包含敏感项目数据，并沿用 Trace 保留策略，已有纯元数据 Trace 不追溯补写。页面默认按 Request 展示 SYSTEM / USER / CONTEXT / ASSISTANT / TOOL 语义账本，类型化检查器提供 Summary、Markdown Preview、Raw blocks、Source、System Prompt、Tools、Payload、Result、Schema 与 Timing；原有多层调用树和事件账本作为次级分析保留。三泳道时间总览支持 TTFT / generation 分段、P50/P95、吞吐、实际时间/压缩空闲/等宽步骤、精确悬停、区间聚焦、滚轮缩放、右键平移和重置，并继续提供 token、缓存命中、完整 JSONL 导出、窄屏覆盖层、深浅主题及中英文。新设置仅为“开发者模式”；关闭时完整内容不会进入 Trace。
- Added the user-level experimental Developer Mode switch. When enabled, both Writing and Game primary navigation expose the shared Trajectory page without implicitly changing the selected content mode. Future runs store complete model-visible messages, System Prompts, tool schemas, assistant Markdown, reasoning, tool payloads, and results in permission-restricted local traces; this may contain sensitive project data and follows trace retention, while existing metadata-only traces are not backfilled. The default view is a per-request SYSTEM / USER / CONTEXT / ASSISTANT / TOOL semantic ledger with type-aware Summary, Markdown Preview, Raw blocks, Source, System Prompt, Tools, Payload, Result, Schema, and Timing inspectors; the nested call tree and event ledger remain available as secondary analysis. The three-lane overview adds segmented TTFT/generation, P50/P95, throughput, actual/idle-compressed/equal-step scales, exact hover, interval focus, wheel zoom, right-drag pan, and reset, alongside token/cache metrics, complete JSONL export, compact overlays, light/dark themes, and Chinese/English. Developer Mode is the only new setting; complete content is not persisted while it is disabled.
