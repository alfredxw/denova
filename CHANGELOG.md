...
- 游戏模式：后台导演不再循环读取并编辑规划文件后另行输出 JSON。`director.md` 与 `lore-context.md` 以有界完整快照注入，Director 通过一次终止型 `submit_director_plan_update` 提交决策；`keep` 不带文档，`patch / replan` 同时提交并由后端整体校验两份完整文档。提交接受后运行立即完成，不再触发摘要模型调用或残留 `running` 状态；Director 默认工具面也移除无效的通用文件工具。前台下一回合只等待关键的状态结构初始化，不再被 Story Memory 或导演规划耗时阻塞。
- Game Mode: The background Director no longer loops through file reads/edits and then emits separate JSON. Bounded complete snapshots of `director.md` and `lore-context.md` are injected, and one terminal `submit_director_plan_update` call carries the decision: `keep` has no documents, while `patch / replan` submits both complete documents for backend validation as one unit. An accepted submission now completes the run immediately without an extra summary model call or a lingering `running` state, and the Director's default tool surface no longer exposes ineffective general filesystem tools. The next foreground turn now waits only for critical state-schema initialization, not slow Story Memory or Director planning.
...
- 游戏模式：修复被拒绝的隐藏结果重试可能让后续正文 B 覆盖已展示正文 A、`prepare_interactive_turn` 允许模型重复声明结果状态变化，以及互动跨回合上下文继续携带旧提交工具噪音的问题。首个正文候选现为一次运行内不可替换；检定结果状态只由确定性 State Binding 生成；下一轮只保留资料与故事记忆的语义读取回执。状态审查的字段级 `actor_ops set` 还可在同一 item 内分别绑定不同来源，整体 Actor 替换仍要求单一来源。
- Game Mode: Fixed rejected hidden-result retries allowing later prose B to replace already displayed prose A, `prepare_interactive_turn` accepting duplicate model-authored outcome state changes, and old submission-tool noise leaking into later Game turns. The first prose candidate is now immutable within a run; check outcome state comes only from deterministic State Bindings; later turns keep only semantic Lore and Story Memory read receipts. Field-level schema-review `actor_ops set` operations may also bind different exact sources within one item, while whole-Actor replacement still requires one source.
...
- Agent：跨回合工具上下文改为按调用 ID 成对筛选。临时目录结果不再进入下一轮，资料和故事记忆正文只保留有界来源回执，游戏文风 `read_file` 预览不再重复注入；孤立、重号或缺少配对的调用/结果会被丢弃，工具错误不会被伪装成成功回执。展示历史仍保留完整工具卡片，不受模型上下文筛选影响。
- Agents: Cross-turn tool context is now filtered as call/result pairs by call ID. Transient indexes no longer enter the next model turn, Lore and Story Memory bodies become bounded source receipts, and Game style-reference `read_file` previews are not re-injected. Orphaned, duplicate, or unpaired protocol messages are dropped, and tool errors are never converted into successful receipts. Display history still keeps its full tool cards independently of model context.
