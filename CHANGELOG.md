...
- Agent 工具执行新增用户级 Ask / Write / Yolo 安全模式，首次进入必须选择且推荐 Write；输入区与设置可随时切换，新运行会固定使用启动时的模式。Ask 自动允许结构化工作区编辑与常用只读 Shell，Write 额外允许常用开发、构建、依赖和网络读取，Yolo 只保留极高危黑名单。网络、外部写入、远端修改及未知命令按模式进入无超时、可恢复的宿主审批卡，卡片展示具体命令或结构化操作详情，且只能「仅允许本次」或「拒绝」；工作区配置不能降低用户安全级别。Unix Agent 默认使用本机 Bash，并通过用户登录 Shell 捕获 export 环境以支持 GUI Release 下的 Homebrew/nvm/npm 等 PATH 配置；Windows 保持原生 PowerShell。该能力是执行前护栏而非 OS 沙箱，已放行的命令和项目脚本仍拥有 Denova 进程的本机权限。
- Agent tool execution now has user-scoped Ask, Write, and Yolo safety modes with a mandatory first choice and Write recommended. The composer and Settings can switch modes, while each new run snapshots its starting mode. Ask auto-allows structured workspace edits and common read-only Shell commands; Write also allows common development, build, dependency, and network-read commands; Yolo retains only a critical blocklist. Depending on the mode, network access, external writes, remote mutations, and unknown commands enter a durable, timeout-free host approval card that shows the concrete command or structured action details and offers only Allow once or Deny; workspace settings cannot weaken the user's posture. Unix Agents use local Bash with exported environment captured from the user's login shell so GUI releases inherit Homebrew/nvm/npm PATH setup; Windows keeps native PowerShell. This is a pre-execution guardrail rather than an OS sandbox; allowed commands and project scripts retain the local permissions of the Denova process.
