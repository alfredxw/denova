...
- Session context sidecar 现在为每个 Agent 持久化精确 checkpoint / rewind locator，以及 active checkpoint / latest rewind 所需的有界 effective 与 canonical boundary；即使活跃日志超过 200 个事务并触发 resident trim，实时运行和冷重启仍能恢复精确边界。最新 locator 或 boundary 损坏会在模型调用前 fail closed，不会回退到 session root。
- The Session context sidecar now persists exact checkpoint and rewind locators per Agent together with the bounded effective and canonical boundaries needed by the active checkpoint and latest rewind. Live runs and cold restarts recover the precise boundary even after more than 200 transactions trigger resident trimming. Corrupt latest locators or boundaries fail closed before any model call instead of rewinding to the Session root.
