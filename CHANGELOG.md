...
- Agent 工具执行新增用户级 Ask / Write / Full access 安全模式；新用户直接使用 Write 默认值，不再经过首次选择弹窗。写作输入框下方的当前模式仍可直接切换；游戏输入框默认使用当前模式，只在「输入动作」选项中提供切换，避免干扰回合输入。设置页仍提供全局配置，每次新运行会固定使用启动时的模式。Ask 自动允许结构化工作区编辑与常用只读 Shell，Write 额外允许常用开发、构建、依赖和网络读取，Full access 只保留极高危黑名单。网络、外部写入、远端修改及未知命令按模式进入无超时、可恢复的宿主审批卡，卡片展示具体命令或结构化操作详情，且只能「仅允许本次」或「拒绝」；工作区配置不能降低用户安全级别。Unix Agent 默认使用本机 Bash，并通过用户登录 Shell 捕获 export 环境以支持 GUI Release 下的 Homebrew/nvm/npm 等 PATH 配置；Windows 保持原生 PowerShell。该能力是执行前护栏而非 OS 沙箱，已放行的命令和项目脚本仍拥有 Denova 进程的本机权限。
- Agent tool execution now has user-scoped Ask, Write, and Full access safety modes. New users start directly in Write without a mandatory first-choice dialog. Writing keeps the current mode as a direct composer switcher; Game uses the current mode by default and exposes switching only under Input actions so it does not distract from turn input. Settings remains a secondary global configuration surface, and each new run snapshots its starting mode. Ask auto-allows structured workspace edits and common read-only Shell commands; Write also allows common development, build, dependency, and network-read commands; Full access retains only a critical blocklist. Depending on the mode, network access, external writes, remote mutations, and unknown commands enter a durable, timeout-free host approval card that shows the concrete command or structured action details and offers only Allow once or Deny; workspace settings cannot weaken the user's posture. Unix Agents use local Bash with exported environment captured from the user's login shell so GUI releases inherit Homebrew/nvm/npm PATH setup; Windows keeps native PowerShell. This is a pre-execution guardrail rather than an OS sandbox; allowed commands and project scripts retain the local permissions of the Denova process.
...
- Added the standalone `github.com/alfredxw/denova/agent/model/openai` adapter module, containing the OpenAI protocol, compatibility behavior, and SDK types at the provider seam. The public `agent` module graph contains no model-provider SDK, and product Agents no longer depend on Eino messages, tools, streams, or runtime abstractions.
...
- 本地文件持久化原语收敛到内聚的 `internal/localfs`：目录同步与文件租约不再分散为平行小 package；Agent File Journal 将租约保持为 runtime 私有实现。两边统一复用成熟的跨平台文件锁依赖，在保留可取消等待、稳定路径身份和幂等释放语义的同时补齐 Windows 跨进程保护。
- Local-file persistence primitives now converge in the cohesive `internal/localfs` package instead of separate directory-sync and file-lease packages, while the Agent File Journal keeps its lease as a private runtime implementation. Both reuse a mature cross-platform file-lock dependency, preserving cancellable waits, stable path identity, and idempotent release while adding real Windows inter-process protection.
...
- 工具结果清理只接受经当前 run artifact store 校验的 complete-output receipt；空 purpose 的旧路径/哈希形状不再获得恢复权限。写作 Session 与游戏 Journal 的清理记录规范化、幂等比较及压缩健康状态机现复用 `internal/contextmaintenance` 纯领域实现，各自仍独立负责 CAS、时间戳和持久化格式。
- Tool-result cleanup now accepts only complete-output receipts verified by the current run artifact store; legacy empty-purpose path/hash shapes no longer gain recovery authority. Writing Sessions and Game Journals now share the storage-neutral `internal/contextmaintenance` rules for cleanup normalization, idempotency, and compaction health transitions while retaining their own CAS, timestamps, and persistence formats.
...
- CI and GitHub Release builds now validate the root module, public `agent`, and OpenAI adapter as three independent Go modules with separate dependency-drift checks, tests, and static analysis. CI reachable-vulnerability scans also cover every module so root commands cannot hide nested-module regressions.
