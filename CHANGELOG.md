...
- 游戏模式：修复故事舞台“世界状态”长期为空的问题。Game Agent 的系统提示、每轮提示和 TurnResult 工具协议现在都明确要求每回合提交 `actor_id="story"` 的 `story_context`，至少维护“当前事件”，并在首次初始化或场景变化时同步“当前详细地点”；后端会拒绝缺失更新的提交并让 Agent 定向重试。前端同时过滤旧存档中的空字符串、空数组和空对象，不再把 `Characters`、`Events`、`Scene` 等空容器显示成状态卡片。兼容性说明：既有历史不会被改写，空的故事上下文会从下一次成功生成的回合开始填充。
- Game Mode: Fixed World State remaining empty on the story stage. The Game Agent system prompt, per-turn prompt, and TurnResult tool contract now explicitly require an `actor_id="story"` `story_context` patch on every turn, including Current Event and Current Detailed Location during initialization or scene changes; the backend rejects missing updates and asks the Agent to retry with targeted diagnostics. The frontend also filters empty strings, arrays, and objects from legacy snapshots instead of rendering empty `Characters`, `Events`, or `Scene` cards. Compatibility note: existing history is not rewritten, and an empty story context starts filling from the next successfully generated turn.
