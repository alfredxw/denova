...
- Files Tab 文件树交互对齐 vscode.dev Explorer：移除可见的「项目文件」标题与生成目录开关，生成目录始终参与按需加载；新建文件和目录改为在当前目录树内直接命名，右键与键盘操作统一支持剪切、复制、粘贴、路径复制、F2 重命名、Enter 打开和 Delete 删除，同时保留多选及拖拽移动。
- The Files tab explorer now follows vscode.dev: the visible “Project files” heading and generated-folder toggle are removed, generated folders always participate in lazy loading, and files/folders are named inline at the current tree location. Context-menu and keyboard behavior now consistently supports cut, copy, paste, path copying, F2 rename, Enter open, and Delete, while preserving multi-selection and drag-to-move.
