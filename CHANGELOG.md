...
- 游戏模式：重新设计正文后的「当前状态」面板，统一使用 shadcn/Radix 的 Collapsible、Tabs、Progress、Button、Dropdown Menu 与 Empty 组合。展开和折叠复用同一条 44px Header，折叠后只隐藏内容，不再切换成高度不同的自定义摘要条；角色与世界状态改为标准分段 Tab 和真实 TabPanel。有明确上下限的角色数值字段整合为最多三列并排的紧凑状态条，显示当前值/上限；无范围数值与其他状态仍以双列信息表展示，并在窄屏自动收敛。本回合状态变化不再单独展示时间线，而是直接在对应字段内以小字注明；数值增减使用 `+` / `-`，其他更新显示简短状态与原因。紧凑模式分别优先保留发生变化的数值和详情字段，空状态复用 shadcn Empty，并补齐相关中英文文案、无障碍数值标签与明暗主题语义色。
- Game Mode: Redesigned the Current State panel after the prose with shadcn/Radix Collapsible, Tabs, Progress, Button, Dropdown Menu, and Empty primitives. Expanded and collapsed states share one 44px header, with collapse hiding only the content instead of swapping in a differently sized custom strip. Actor and World State use standard segmented tabs with real tab panels. Bounded Actor numeric fields are consolidated into a dense grid of up to three parallel progress bars showing current value and upper limit; unbounded numbers and other state remain in a two-column information grid that contracts responsively. Per-turn changes no longer render as a separate timeline and instead appear as small annotations inside the affected field; numeric deltas use `+` / `-`, while other updates show a concise status and reason. Compact mode prioritizes changed metrics and details independently, empty states reuse shadcn Empty, and the related bilingual copy, accessible value labels, and semantic light/dark theme colors are included.
