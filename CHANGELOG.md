...
- 语言模型接入改为「服务商预设 + API 协议适配器」两层：内置 OpenAI Chat Completions、OpenAI Responses、Anthropic Messages 与 Google Generative AI 四种协议，并补充 DeepSeek、Anthropic、Gemini、OpenRouter、Groq、Mistral、xAI、Moonshot、硅基流动、火山方舟、Ollama、LM Studio、LiteLLM、vLLM 等常用服务商预设。预设只提供默认地址和协议，不再充当白名单；任意服务商 ID 均可自由组合已安装协议、Base URL、自定义 Header 与协议兼容选项。DeepSeek 现完整支持 Chat Completions 的 thinking 开关、思考强度与工具调用 reasoning 回传，同时支持其 Responses API 和 Anthropic API 路由；各协议均通过同一 Agent 消息、工具、结构化输出、流式响应、用量与续传边界运行。Beta 不兼容：尚未发布的 `model_profiles` 中 `openai_api_key` / `openai_base_url` / `openai_model` 已直接替换为通用 `api_key` / `base_url` / `model`，不保留旧字段兼容。
- Language-model integration now has two explicit layers: provider presets and API protocol adapters. Denova ships OpenAI Chat Completions, OpenAI Responses, Anthropic Messages, and Google Generative AI adapters, plus presets for DeepSeek, Anthropic, Gemini, OpenRouter, Groq, Mistral, xAI, Moonshot, SiliconFlow, Volcengine Ark, Ollama, LM Studio, LiteLLM, vLLM, and other common endpoints. Presets only supply default routes and protocols and are no longer an allowlist; any provider ID can freely combine an installed protocol, Base URL, custom headers, and protocol compatibility options. DeepSeek now supports the Chat Completions thinking toggle, effort, and tool-call reasoning replay, together with its Responses API and Anthropic API routes. Every protocol shares the same Agent message, tool, structured-output, streaming, usage, and continuation boundary. Beta breaking: the unreleased `model_profiles` fields `openai_api_key` / `openai_base_url` / `openai_model` are directly replaced by generic `api_key` / `base_url` / `model` fields with no legacy-field compatibility.
- 设置中的每个语言模型 Profile 新增「测试连接」，会使用当前未保存草稿和已保存密钥继承，通过真实 Agent 协议适配器发送一次最小生成请求，同时校验路由、鉴权、模型可用性、请求序列化与响应解析并展示实际协议与耗时；编辑或移除 Profile 会取消并清除过期结果，请求不附加 LLM 超时。
- Every language-model profile in Settings now has Test Connection. It combines the current unsaved draft with inherited saved secrets and sends a minimal real generation through the Agent protocol adapter, validating routing, authentication, model availability, request serialization, and response parsing while reporting the effective protocol and latency. Editing or removing a profile cancels and clears stale results, and the probe adds no LLM timeout.
