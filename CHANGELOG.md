...
- Beta 不兼容：书籍目录、统计、章节确认、资料条目 CRUD 与文本审阅的前端/HTTP 契约统一改为 Project ID 资源；旧 `/api/workspace/{tree,summary,chapter-status,document-review,document-comments...}` 与 `/api/lore/items...` 基础 CRUD 路由已移除。独立的资料分类、图片生成与前台工作区文件工作流保持各自现有契约。
- Beta breaking: Book trees, summaries, chapter confirmation, Lore CRUD, and document review now use only Project-ID frontend and HTTP contracts. The former `/api/workspace/{tree,summary,chapter-status,document-review,document-comments...}` routes and base `/api/lore/items...` CRUD routes have been removed. Independent Lore classification, image-generation, and foreground workspace-file workflows retain their existing contracts.
- 文本审阅账本从书籍内容目录下的隐藏状态迁至用户级 Project 状态目录；旧账本采用仅复制迁移且保留原文件作为回滚源，已完成旧版 Project 状态迁移的项目会按新版迁移收据补齐，不会双写。
- Document-review ledgers move from hidden state beneath Book content into the user-level Project state directory. Existing ledgers use a copy-only migration that retains the source as a rollback path, and Projects with an older completed state migration receive the new receipt upgrade without dual writes.
