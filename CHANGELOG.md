...
- 写作页作品目录栏（大纲 tab）针对长章节列表重新组织导航：书籍设定与最新细纲共用一份等高顶部容器，Pin / 取消 Pin 以不同图标表达且记忆偏好，不再通过高亮或切换时的 8px 位移表达状态；最新章与回顶移到 Pin 左侧，原有底部浮标全部移除，折叠分卷时不会再压住卷标题。最新细纲直接按名称打开，下方只保留历史细纲；最新章、目录定位、保存和编辑器设置统一收敛为带悬浮提示的纯图标操作，其中目录定位可打开作品目录、展开所在卷并定位当前章节。章节、细纲和已 Pin 的书籍设定文件现在都支持右键或悬停更多菜单，可引用到 Chat、在项目文件中显示、重命名文件和确认删除。Chat 输入框中的文件引用只显示文件名，完整 workspace 路径仍保留用于发送、移除和悬停提示。`ChapterOutline` 相关组件同步从 `ModeRouter.tsx` 抽离至 `components/workbench/outline/`。
- The Writing outline sidebar now reorganizes navigation for long chapter lists. Book Settings and the latest outline share one equal-height header in pinned and scrolling states; distinct Pin/Unpin icons replace active highlighting and remove the former 8px layout shift. Latest Chapter and Back to Top now sit beside Pin, all bottom floaters are removed, and collapsed volumes no longer have controls covering their titles. The latest outline opens directly by name while Latest Chapter, Reveal, Save, and Editor Settings are consistently presented as icon-only actions with hover tooltips; Reveal opens the outline, expands the volume, and locates the current chapter. Chapters, outlines, and pinned Book Settings files now expose both right-click and hover action menus for Chat references, Project Files reveal, file rename, and confirmed delete. File-reference tokens show only the basename in the Chat composer while retaining the canonical workspace path for sending, removal, and hover disclosure. The `ChapterOutline` components were extracted from `ModeRouter.tsx` into `components/workbench/outline/`.
...
- 写作及固定 Agent 会话新增真实存储分页、稳定 compaction cursor 和流式完整历史导出；游戏快照与 UI 历史改为有界最近窗口及向上游标分页。两种模式的热追加、最近历史和模型上下文均不再物化完整 JSONL，索引只保存当前领域投影、最近定位和稀疏锚点，不保存历史正文、thinking 或工具结果。
- Writing and fixed Agent sessions now provide storage-backed paging, stable compaction cursors, and explicit streaming full-history export; Game snapshots and UI history use a bounded recent window with upward cursor paging. Hot appends, recent history, and model context in both modes no longer materialize the complete JSONL, while indexes retain only current domain projections, recent locators, and sparse anchors—never historical prose, reasoning, or tool results.
...
- Agent 新增 `web_fetch`：仅抓取公开 HTTP(S) 页面，先使用带常见 Chrome 导航头的 HTTP 快路径，再默认尝试固定的 Jina Reader，最后用隔离的本机 Chrome/Rod 渲染 JavaScript 页面；Jina Markdown 直接规范化，HTML 才经过 Readability 与 `html-to-markdown`。该工具支持 Unicode 字符分页、响应/正文配置上限、重定向逐跳校验、私网与云元数据地址阻断，并把外部正文明确标记为不可信来源。设置页新增用户级网页访问配置，可填写 SearXNG 实例、单个搜索服务超时及搜索/抓取上限。
- Agents now include `web_fetch`, which retrieves only public HTTP(S) pages through a Chrome-header HTTP fast path, the default fixed Jina Reader service, then an isolated installed Chrome/Rod renderer for JavaScript pages. Jina Markdown is normalized directly, while HTML alone passes through Readability and `html-to-markdown`. The tool supports Unicode-character pagination, configurable response/content limits, redirect-by-redirect validation, private/cloud-metadata address blocking, and explicit untrusted-source labeling. Settings now has user-level Web Access configuration for a SearXNG instance, per-provider search timeout, and search/fetch limits.
...
- `agent/context` 新增显式片段装配器，分别记录 system、turn context、state memory、工具结果和用户消息的来源、用途、位置、hash 与高上限预算；`agent/tools.Definition` 将实现、schema 和执行/恢复/结果投影策略绑定为单一注册单元，产品工具的定义与实现同位置维护，未知工具默认采用保守恢复策略。
- `agent/context` adds an explicit fragment assembler that records source, purpose, placement, hash, and high bounded budgets for system, turn-context, state-memory, tool-result, and user-message fragments. `agent/tools.Definition` binds implementation, schema, and execution/recovery/result-projection policy into one registration unit; product tool definitions now live beside their implementations, and unknown tools recover conservatively.
...
- `web_search` 重构为可替换 provider 的网页访问模块：配置 SearXNG 时优先使用并在失败/空结果后回退；默认并发查询 DuckDuckGo 与 Bing，等待两边后交错去重合并，单边失败/超时仍返回另一边结果及诊断。Bing 优先读取完整 HTML、按查询语言选择市场，RSS 仅作降级且会拒绝未覆盖完整查询的明显无关结果。Beta 不兼容：移除原有 Baidu/Google 聚合，暂不支持 Brave；原 `web_search` 权限开关现在同时控制 `web_search` 与 `web_fetch`。
