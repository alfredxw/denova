...
- 新增用户级实验开关“开发者模式”；开启后，写作与游戏模式的一级菜单都会显示共享的“轨迹 / Trajectory”页面，且进入页面不会隐式切换模式。页面按 Run → 模型 → 工具/上下文构建多层调用树，并提供 100 条运行历史、实际时间/压缩空闲/等宽步骤三种时间尺度、模型/工具/运行时三轨时间总览、区间聚焦、滚轮缩放与右键平移、搜索、分类与错误筛选、事件账本、展开折叠、原始有界元数据检查和完整 JSONL 导出；顶部汇总总耗时、模型与工具耗时、空闲、TTFT、token 与缓存命中率。窄屏改为横向 Run 列表与按需详情覆盖层，深浅主题及中英文共用同一实现。新模型调用会额外持久化已有的 `ttft_ms` 观测值；旧 Trace 缺失时明确显示不可用，不推测历史数据。完整 prompt、输出、thinking 与工具结果仍不会进入 Trace。
- Added the user-level experimental Developer Mode switch. When enabled, both Writing and Game primary navigation expose the shared Trajectory page without implicitly changing the selected content mode. The page builds a nested Run → model → tool/context call tree and provides 100-run history, actual-time/idle-compressed/equal-step scales, a three-lane model/tool/runtime overview, interval focus, wheel zoom and right-drag pan, search, category and error filters, an auxiliary event ledger, expand/collapse controls, bounded raw-metadata inspection, and complete JSONL export. Its summary covers total, model, tool, idle, TTFT, token, and cache-hit metrics. Compact layouts use a horizontal run strip with on-demand detail overlays, while light/dark themes and Chinese/English share the same implementation. New model-call spans now persist the existing `ttft_ms` observation; older traces show it as unavailable instead of inventing historical latency. Full prompts, outputs, thinking, and tool results remain outside traces.
