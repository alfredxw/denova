...
- The Ask tool no longer requires the model to generate duplicate Chinese and English copies of questions, options, and descriptions; visible copy now follows the user's current input language. Public interaction validation, event projection, and answer projection share the single-string contract across Writing, Game, AgentChat, and management Agents. Beta breaking: the former `prompt/label/description.zh|en` call shape is removed. No new configuration is required.
- 待回答的标准 Ask 现在从已持久化的稳定问题 ID、选项 ID 和选择约束校验答案，不再因展示文案、工具 schema 或动态上下文的 materialized Definition 变化而失效；Definition/behavior identity、权限确认与自定义交互仍保持严格隔离。Ask API 错误统一返回稳定英文，界面失败提示继续按用户语言设置本地化。无需新增配置。
- Pending standard Ask interactions now validate answers from their persisted stable question IDs, option IDs, and selection constraints, so presentation-copy, tool-schema, or dynamic-context changes no longer invalidate them through a materialized Definition mismatch. Definition and behavior identities, permissions, and custom interactions remain strictly fenced. Ask API errors now return stable English while the UI continues to localize failure messages from the user's language setting. No new configuration is required.
- 已回答或取消的 Ask 现在会把原问题、选项与最终答案保存在仅展示的会话日志中，刷新和重新打开会话后仍可追溯当时的选择；历史 Ask 卡片默认折叠，点击整行标题即可展开，待回答卡片继续保持展开。该展示数据不进入模型上下文，无需新增配置。
- Answered or cancelled Ask interactions now retain their original questions, options, and final answers in the display-only Session journal, so the chosen response remains inspectable after refresh or reopening a conversation. Historical Ask cards start collapsed and expand from the full header row, while pending cards remain open. This display data never enters model context, and no new configuration is required.
- Agent Chat 的思考滚动窗口最大高度从约 10 行增加到约 20 行，减少阅读较长思考内容时的滚动频率；Writing、Game、AgentChat 与管理型 Agent 共用，无需新增配置。
- The Agent Chat reasoning viewport now grows from about 10 lines to about 20 lines before scrolling, reducing scrolling while reading longer reasoning. Writing, Game, AgentChat, and management Agents share the behavior with no new configuration.
...
- Go 构建基线升级到 1.26.6，修复主程序与公共 Agent module 可达的标准库安全问题；CI 也会显式安装项目声明的 ripgrep 开发依赖，确保真实 grep 集成测试始终运行。无需新增配置。
- The Go build baseline is upgraded to 1.26.6, resolving reachable standard-library security issues in the main application and public Agent module. CI also installs the declared ripgrep development dependency explicitly so real grep integration tests always run. No new configuration is required.
...
- Agent Cleanup now uses public `agent/cleanup.Standard` as its sole pressure authority. Denova only maps product policy, the concrete model window, and capacity reserves, so Writing, Game, and delegated child Agents share grouping protection, cache gates, recovery placeholders, and metric semantics. Child Cleanup is sized from its own model window, while disabled Compaction policy and the exact output limit of each model request are handled by the shared policy; automatic checkpoint-fork instructions are bounded by their declared static reserve to preserve prefix-cache safety. Beta breaking: the duplicate internal pressure/Cleanup planner and the obsolete Cleanup DTO, merge, and target paths were removed. No new configuration is required.
- Agent 中止事件统一使用 canonical `reason` 字段，所有生产端通过同一构造入口发布，AI SDK stream 会保留真实中止原因，不再错误回落为 `user cancelled`。无需新增配置。
- Agent abort events now use the canonical `reason` field, with every production source publishing through one constructor. The AI SDK stream preserves the real abort reason instead of incorrectly falling back to `user cancelled`. No new configuration is required.
- Harness Optimizer 的 `trajectory://index` 现在按时间对全部 Project 的 Session 与 Run 合计执行全局条数上限；默认值由 50 调整为 100，模型请求不能越过用户配置，显式配置最大值仍为 500，设置页也不会再把未配置层的零值误作继承值。单个 Project 的 Session 或 Run 读取失败会写入 `issues`，不再隐藏其他 Project 的健康证据；现有配置继续生效。
- Harness Optimizer's `trajectory://index` now applies one time-ordered global limit across Sessions and Runs from every Project. The default increases from 50 to 100, model requests cannot exceed the user setting, the explicit configuration maximum remains 500, and the settings page no longer treats an unset layer's zero value as inherited configuration. Session or Run failures from one Project are reported through `issues` without hiding healthy evidence from other Projects; existing settings remain effective.
- 语言模型设置按依赖顺序调整为“服务商与协议 → Base URL 与 API Key → 模型与别名”，模型输入仍可完全自定义，并在打开候选列表时按当前未保存协议配置懒加载。Anthropic Messages 现在通过官方分页 `/v1/models` API 提供候选，OpenAI Chat Completions、Responses 与兼容端点继续复用既有 `/models` 能力；无效或重复条目会被局部忽略，查询失败或上游不支持不会阻断手动配置。无需新增配置，现有模型 Profile 与 API 保持兼容。
- Language-model settings now follow their dependency order: provider and protocol, Base URL and API key, then model and alias. The model field remains fully editable and lazily loads suggestions from the current unsaved protocol route when its list is opened. Anthropic Messages now uses the official paginated `/v1/models` API, while OpenAI Chat Completions, Responses, and compatible endpoints retain their existing `/models` discovery. Invalid or duplicate entries are skipped independently, and an unsupported or failed listing never blocks manual configuration. No new setting is required, and existing model profiles and APIs remain compatible.
- Agent Chat 的 Run 在运行中与完成后现在共用同一个「执行过程」结构：标题统一汇总进展段数、工具次数和耗时，运行中自动展开，完成后仅收起外层；展开后按原始顺序直接展示每段进展正文、思考与工具，不再增加进展二级折叠，游戏正文后的工具继续留在正文之后。思考入口同步简化为“思考 / Thinking”，并在同一行展示首个非空思考行，空间不足时自适应省略，完整内容仍可独立展开。Writing、Game、AgentChat 与管理型 Agent 共用，无需新增配置。
- Agent Chat Runs now keep the same Execution structure while active and completed: the heading summarizes progress updates, tool calls, and duration; active Runs open automatically, and completion only closes the outer disclosure. Expanding it shows each progress body, thinking block, and tool directly in original order without a second progress disclosure, while Game tools emitted after narrative stay after it. Thinking triggers are also shortened to “思考 / Thinking” and show the first non-empty reasoning line inline with responsive truncation while preserving independent access to the full content. Writing, Game, AgentChat, and management Agents share the behavior with no new configuration.
...
- Agent Chat reasoning now uses a scrollable streaming window capped at about 160px. A shadcn scroll fade and subtle border clarify the window boundary; short content keeps its natural height, while long content follows the latest output until the user scrolls upward and resumes following upon returning to the bottom. Decorative left rails and extra indentation are removed from execution and reasoning bodies while true parent/child tool trees retain their hierarchy lines. The execution disclosure arrow now follows the heading, and the whole row retains hover feedback. The single live “Executing” heading updates the full Run duration every second; a durable server summary freezes it after success, failure, abort, or interruption and keeps it shown only once across multiple progress segments and post-result tools. Legacy history without a summary is not estimated. Writing, Game, AgentChat, and management Agents share the behavior with no new configuration.
- 工具输入流统一为按调用 ID 维护的 append-only 原始文本：Writing 的 AI SDK 消费链与 Game 的 SSE 消费链都直接执行 `inputText += inputTextDelta`，所有工具在 `input-streaming` 阶段逐段原样展示，不解析、格式化、隐藏或等待 schema 字段；提供方把完整参数合并成一个超大 delta 时，统一展示层会把这段原始文本按有限渲染帧重新分块，避免 `edit` 等调用从空白直接跳到完成态，拼接结果和执行参数保持逐字不变。卡片标题仅保留两个非权威展示派生：`read` / `write` / `edit` 显示路径中的文件名，`grep` / `bash` 显示 `description`；解析失败不影响原始流。Beta 不兼容：删除 `hide_novel_chapter_body_in_live_output` 配置、write/Director 输入压缩 middleware 及隐藏字段元数据，实时工具卡改为展示完整原始参数。
- Tool input streaming now uses one append-only raw-text source per call ID. Both the Writing AI SDK consumer and Game SSE consumer directly apply `inputText += inputTextDelta`; every tool renders each received chunk verbatim during `input-streaming` without parsing, formatting, hiding, or waiting for schema fields. When a provider coalesces complete arguments into one oversized delta, the shared presentation layer re-frames that raw text across a bounded number of render frames so calls such as `edit` no longer jump directly from empty to complete; concatenated text and execution arguments remain byte-for-byte unchanged. Card headers retain only two non-authoritative display derivations: `read` / `write` / `edit` show the filename from the path, while `grep` / `bash` show `description`; extraction failure never affects the raw stream. Beta breaking: `hide_novel_chapter_body_in_live_output`, the write/Director input-compaction middleware, and hidden-field metadata are removed, and live tool cards now show the complete raw arguments.
...
- The Trajectory / Harness top tabs are now anchored at the upper-left of the page header, before the title and right-side actions, so title length, loading state, and export controls cannot shift the switcher. Writing and Game share the layout with no new configuration.
- Developer Mode 现在是 Trajectory 与 Harness 能力的唯一 Lab 开关：写作和游戏模式共享的 Trajectory 一级菜单新增并列的“轨迹 / Harness”顶部 Tab，两个视图都跨所有已注册项目工作且不会切换当前书籍。Trajectory 通过新的全局 Run 目录展示项目、Agent、状态、时间、调用与耗时，并隔离单项目读取失败；Harness 从 Agents 中移除，保留 State、Git 历史、定时优化和自适应 Optimizer，旧 Trajectories 子页替换为支持搜索、精确多选和独立查看的紧凑 Run 选择器。未选择证据时 Optimizer 自主读取近期全局 `trajectory://index`，有选择时严格限定于对应 Run URI；刷新会明确移除已消失的选择。Beta 不兼容：删除 `labs.continual_learning` 配置、旧 Harness trajectory API 与前端类型，不提供兼容或迁移逻辑；现有 State 与历史会保留，但 Developer Mode 关闭时不加载或生效。定时开关、间隔和 trajectory 上限配置继续保留，无新增配置。
- Developer Mode is now the sole Lab switch for Trajectory and Harness capabilities. The shared Trajectory primary menu in Writing and Game adds peer “Trajectory / Harness” top tabs; both are global across all registered projects and never switch the current Book. Trajectory uses a new global Run catalog with project, Agent, status, time, call, and duration metadata while isolating per-project read failures. Harness has moved out of Agents and retains State, Git history, scheduled optimization, and the adaptive Optimizer; its obsolete Trajectories subpage is replaced by a compact searchable Run picker with exact multi-selection and an independent View trajectory action. With no selection, Optimizer discovers recent global evidence through `trajectory://index`; with selections, analysis is strictly limited to those Run URIs. Refresh explicitly removes disappeared selections. Beta breaking: `labs.continual_learning`, the old Harness trajectory APIs, and their frontend types are removed without compatibility or migration logic. Existing State and history remain stored but are neither loaded nor effective while Developer Mode is off. Schedule, interval, and trajectory-cap settings remain; no new configuration is added.
- Agent 新增同步 JavaScript `script` 编排工具：可通过 `ctx.tools.call` / `ctx.tools.parallel` 组合当前 Run 中的普通工具、用户脚本工具与 `task` 子 Agent，每个子调用都重新进入完整权限、调度、结果与轨迹链。Harness State 支持以 `tools/<name>.js` 保存 User Script Tool，可在双语可视化编辑器中新建、校验、启停、删除与恢复，也可由用户明确授权根 Agent 或 Harness Optimizer 原子更新；运行时只认当前 Harness State，Agent 重新构建或恢复时直接读取当前内容，不保存历史版本 pin。Chat 与 Trajectory 使用同一 `parent_call_id` 展示递归工具树，模型历史仍只保留外层结果。General 与 Writing 默认开启 `script` / `harness_state`，Game 默认关闭但可配置；新增可选用户设置 `agent_script_timeout_seconds`（默认 0，不限时）。
- Agents now provide a synchronous JavaScript `script` orchestration tool. `ctx.tools.call` and `ctx.tools.parallel` can compose ordinary tools, saved user Script Tools, and `task` subagents from the current Run; every child call re-enters the complete permission, scheduling, result, and trajectory pipeline. Harness State now accepts saved User Script Tools at `tools/<name>.js`, with bilingual UI flows for creation, validation, enable/disable, deletion, history, and restore, plus atomic updates when the user explicitly authorizes a root Agent or Harness Optimizer. Runtime execution uses only the current Harness State: Agent rebuilds and recovery read the current content directly and never persist a historical version pin. Chat and Trajectory use the same `parent_call_id` to render recursive tool trees while provider history retains only the outer result. General and Writing enable `script` and `harness_state` by default; Game keeps them configurable and off by default. The only new user setting is the optional `agent_script_timeout_seconds` (default 0, unlimited).
...
- Developer Mode trajectory message summaries now use a wrapping inline metric strip: metrics stay side by side when space allows and automatically flow onto additional rows when needed, instead of reserving one full row per metric. Writing and Game modes share the layout with no new setting.
- `grep` 工具卡的中英文短名称收敛为“检索 / Search”，其可选 `description` Schema 直接要求使用与当前用户输入相同的语言。共享 System Prompt 不再根据界面语言固定思考语言，而是要求内部思考、流式思考与面向用户的输出跟随当前输入语言；固定输出协议、JSON 键、代码、路径和引用原文保持不变。Writing、Game 与子 Agent 共用该规则，无需新增配置。
- The `grep` tool card now uses the shorter bilingual label “检索 / Search,” and its optional `description` schema directly requires the same language as the user's current input. The shared System Prompt no longer fixes thinking language from the UI locale; internal reasoning, streamed thinking, and user-facing output now follow the current input language, while fixed output protocols, JSON keys, code, paths, and quoted source text remain unchanged. Writing, Game, and child Agents share the rule without a new setting.
- Agent Chat 的“执行过程”现在使用两层独立折叠：外层展开后按原始顺序展示进展正文、工具调用与每段思考标题，历史思考默认收起，当前流式思考自动展开；每段思考可单独开关，用户手动选择不会被该段完成事件覆盖，收起外层后统一重置。Writing、Game、AgentChat 与管理型 Agent 共用该行为，无需新增配置。
- Agent Chat execution details now use two independent disclosure levels. Expanding the outer process preserves the original order of progress prose, tool calls, and per-segment thinking headers; historical thinking starts collapsed while the current streaming segment opens automatically. Each thinking segment toggles independently, explicit user choices survive that segment completing, and collapsing the outer process resets all inner state. Writing, Game, AgentChat, and management Agents share the behavior with no new setting.
- 工具执行卡进一步收紧信息密度：`read` 完成后也会在折叠态保留目标文件名，内置文件工具简化为“读取 / 写入 / 编辑”，`bash` 简化为“命令”；移除独立的“详情”文字按钮，改为点击整行卡片头展开或折叠参数与结果，并同步缩小字号和间距。英文界面对应使用 “Read / Write / Edit / Command”，Writing、Game、AgentChat 与子 Agent 共用该行为。
- Tool execution cards now use a denser presentation: completed `read` calls keep the target filename visible when collapsed, built-in file tools use the shorter “Read / Write / Edit” labels, and `bash` uses “Command”. The separate Details text button is removed; the whole card header now toggles arguments and results with tighter typography and spacing. Writing, Game, AgentChat, and child Agents share the behavior.
- Agent 会话的 Diff 审阅卡现在只在所属 Run 停止后挂载，手动中断与其他终止结果共用同一边界，并以快速、支持 reduced-motion 的入场动画出现；运行中不再预留未完成审阅卡。`edit` 从首个可展示的 `new_string` 字符开始立即切换为流式预览，长 `old_string` 阶段继续保留正常、可展开的卡片。`read` / `write` / `edit` 等文件工具的折叠摘要只显示最终文件名，完整路径仍保留在展开参数中。Writing、Game、AgentChat 与子 Agent 共用对应行为，无需新增配置。
- Diff review cards in Agent conversations now mount only after their owning Run stops. Manual aborts and all other terminal outcomes share the same boundary, and the card enters with a fast reduced-motion-aware reveal; no incomplete review placeholder is reserved during execution. `edit` switches to its live preview as soon as the first displayable `new_string` character arrives, while the long `old_string` phase keeps the normal expandable card instead of showing an empty streaming state. Collapsed summaries for file tools such as `read`, `write`, and `edit` show only the final filename, while expanded arguments retain the complete path. Writing, Game, AgentChat, and child Agents share the applicable behavior without a new setting.
- 开发者模式的轨迹页改为高密度 Trace 工作区：对话、时间线与原始数据按页签切换，时间线不再常驻挤压模型记录；请求、System Prompt、用户/助手消息、工具定义与工具调用统一为紧凑单行层级，工具参数、响应、Schema、耗时和来源可直接查看。记录详情默认在当前行内展开，也可切换为可调整宽度的右侧面板或窄屏抽屉；点击态取消高亮描边，并完成深浅色、宽窄屏与无内容轨迹适配。写作和游戏模式共用该行为，无需新增配置。
- Developer Mode trajectories now use a high-density trace workspace. Conversation, Timeline, and Raw views are tabbed so timing no longer permanently reduces the model-record viewport; requests, system prompts, user/assistant messages, tool definitions, and tool calls share a compact one-line hierarchy with direct access to arguments, responses, schemas, timing, and source data. Record details expand inline by default and can switch to a resizable right inspector or narrow-screen drawer. Click states no longer use bright focus borders, and the workspace is verified across light/dark themes, wide/narrow layouts, and metadata-only runs. Writing and Game share the behavior without a new setting.
- `grep` 与平台 Shell（`bash` / `pwsh`）工具新增可选、限长的 `description` 调用字段，供 Agent 用用户当前语言简述命令目的；描述会随完整工具参数记录，并优先显示在运行中及成功的工具卡摘要，缺失时继续回退到原始命令。执行、Shell 审批和命令规则仍只以 `command` 为准，展示文案不参与安全决策。Writing、Game 与子 Agent 共用该行为，无需新增配置。
- The `grep` and platform shell (`bash` / `pwsh`) tools now accept an optional bounded `description` call field so Agents can summarize command intent in the user's language. The description is retained with the complete tool arguments and takes precedence in running and successful tool-card summaries, while calls without it continue to show the command. Execution, shell approval, and command rules remain authoritative on `command` only; display text never affects security decisions. Writing, Game, and child Agents share the behavior with no new setting.
- Agent `grep` 工具继续保留最小的 `command + cursor` 接口，并将模型说明收敛为原生 `rg` 用法；执行前会在不经过 Shell 的前提下，将常见 `grep` / `egrep` / `fgrep` / `ripgrep` 写法、误带的 `-E` / 递归短参数、工作区内绝对路径和位置 glob 归一为安全的 canonical `rg` 命令。无法确定映射的参数、非搜索管道、重定向、外部路径及危险 `rg` 能力仍明确拒绝。分页游标改为绑定归一化执行计划，因此近似写法与 canonical 命令可以连续翻页；Writing、Game 与子 Agent 共用该行为，无需新增配置。
- The Agent `grep` tool keeps its minimal `command + cursor` interface while its model guidance now consistently describes native `rg` usage. Before execution—and without invoking a shell—it normalizes common `grep`, `egrep`, `fgrep`, and `ripgrep` spellings, carried-over `-E` or recursive short flags, absolute paths inside the workspace, and positional globs into a safe canonical `rg` command. Ambiguous options, non-search pipelines, redirects, external paths, and dangerous `rg` capabilities remain explicitly rejected. Pagination cursors now bind to the normalized execution plan, so approximate and canonical spellings can continue the same result sequence. Writing, Game, and child Agents share this behavior with no new setting.
- Agent 静态组合改为单一错误边界：`builtin.Model`、`Workspace`、`Shell`、`Todo`、`Ask`、`Skills`、`Tasks`、`Combine`、标准 Cleanup 与 Compaction 现在返回可直接嵌入 `Definition` 的声明值；`agent.New` 统一解析模型 Identity、初始化能力，并按组件路径汇总配置错误。文件 Store 也可直接写入 `WithSessionStore(sessionfile.New(...))`。动态 Source 在每轮 `Prepare` 返回 Definition 后使用同一校验链路，模型调用、工具执行与 Session I/O 错误仍留在运行操作。Beta 不兼容：上述内置能力构造器不再返回 `(value, error)`。无新增配置。
- Static Agent composition now has one error boundary. `builtin.Model`, `Workspace`, `Shell`, `Todo`, `Ask`, `Skills`, `Tasks`, `Combine`, standard Cleanup, and standard Compaction return declarations that can be embedded directly in `Definition`; `agent.New` resolves model identity, initializes capabilities, and joins configuration failures with component paths. A file Store can likewise be passed as `WithSessionStore(sessionfile.New(...))`. Dynamic Sources use the same validation path after returning each cycle's Definition from `Prepare`, while model-call, tool-execution, and Session-I/O errors remain runtime operation results. Beta breaking change: these built-in capability constructors no longer return `(value, error)`. No new setting is required.
- Agent package 使用文档拆分为独立中文 `README.md` 与英文 `README.en.md`，并围绕当前轻量 `Agent → Session → Run` API 提供最小 Quickstart、完整能力组合和产品定制 Source 三个任务导向示例。
- Split the Agent package guide into a dedicated Chinese `README.md` and English `README.en.md`, with three task-oriented examples for the current lightweight `Agent → Session → Run` API: a minimal Quickstart, a complete capability composition, and a product-specific Source.
- Agent 运行时完成减法改造：保留统一的 `Agent → Session → Run` 体验与 Writing、Game、AgentChat、Automation、子 Agent 共用能力，但把执行协调收敛为进程内 Session 串行与 Run 状态机；Session Store 只持久化 provider-neutral transcript 和 Goal/Todo/Cleanup/Compaction 等能力快照，内置文件实现简化为 manifest、带校验和的 JSONL 事务与独占 lease。Canonical input/output 和工具 effects 直接调用产品侧幂等 Adapter，不再经过第二套 intent/receipt/outbox/reconcile 状态机。同一后端进程内的页面刷新仍可附着活动 Run，并可观察队列、流式输出、未完成工具与交互；后端重启期间未完成的回合现在明确显示为 interrupted，由用户作为新 Run 重试，不再精确恢复工具栈、交互 waiter、队列或命令重放。写作与游戏的已完成会话内容、工作区文件和 canonical 产品数据不受影响，无新增配置。
- Simplified the Agent runtime while preserving the shared `Agent → Session → Run` experience across Writing, Game, AgentChat, Automation, and child Agents. Execution coordination is now an in-process serial Session lane plus Run state; the Session Store persists only provider-neutral transcript and capability snapshots such as Goal, Todo, Cleanup, and Compaction. The built-in file Store is reduced to a manifest, checksummed JSONL transactions, and an exclusive lease. Canonical input/output and tool effects call idempotent product Adapters directly instead of passing through a second intent/receipt/outbox/reconcile state machine. Browser refreshes can still attach to a live Run in the same backend process and observe queues, streaming output, open tools, and interactions. If the backend restarts during a Run, that turn is now shown as interrupted and can be retried as a new Run; tool stacks, interaction waiters, queues, and commands are no longer reconstructed or replayed. Completed Writing/Game conversations, workspace files, and canonical product data are unaffected. No new setting is required.
- 修复已完成 SubAgent 详情首次打开时复用主会话底部锁、直接从长输出末尾展示的问题。详情面板现在从子会话开头打开，并保留“回到底部”操作；运行中的 SubAgent 仍持续跟随最新输出。写作、游戏与 AgentChat 共用该行为，无需新增配置。
- Fixed completed SubAgent details reusing the primary conversation's bottom lock and opening at the end of long output. The detail pane now opens at the beginning of the child session and retains the jump-to-bottom action, while active SubAgents continue following live output. Writing, Game, and AgentChat share the behavior without a new setting.
- 工具展示现在由每个 Tool Descriptor 声明稳定的语义展示契约，并通过 Agent 生命周期事件、写作会话历史和游戏回合历史持久化；前端按契约选择通用、文件、搜索、终端、网页、浏览器、图像、互动媒体、待办、交互与委派组件，不再依赖工具名分支。展示字段明确排除在模型 Tool Schema、稳定前缀与恢复身份之外，纯 UI 调整不会旋转模型缓存。现有 Agent 能力目录同时改为从完整 Descriptor 生成每个具体工具的契约，并在静态组装和最终动态 Toolset materialization 时校验工具名、能力授权、来源、执行、恢复、结果上限与展示语义；Writing、Game 与子 Agent 共用同一链路，无需新增配置。Beta 不兼容：内置 Goal 工具和目录统一使用真实名称 `goal`，不再暴露未实现的 `goal_finish` 名称；本开发周期内缺少展示契约的旧工具历史按通用工具显示，不再按旧工具名推断专用卡片。
- Tool presentation is now a stable semantic contract owned by each Tool Descriptor and persisted through Agent lifecycle events, Writing session history, and Game turn history. The frontend selects generic, file, search, terminal, web, browser, image, interactive-media, todo, interaction, and delegation renderers from that contract instead of branching on tool names. Presentation is explicitly excluded from model Tool Schemas, stable prefixes, and recovery identity, so display-only changes do not rotate model caches. The existing Agent capability catalog now derives a complete contract for every concrete tool and validates names, authorization, source, execution, recovery, result limits, and presentation during both static assembly and final dynamic Toolset materialization. Writing, Game, and child Agents share the path with no new setting. Beta breaking changes: the built-in Goal tool and catalog now use the real `goal` name instead of advertising the nonexistent `goal_finish` name; tool-history entries from this development cycle that lack a presentation contract render as generic tools instead of inferring specialized cards from legacy tool names.
- 新增用户级实验开关“开发者模式”；开启后，写作与游戏模式的一级菜单都会显示共享的“轨迹 / Trajectory”页面，且进入页面不会隐式切换模式。后续 Run 会在权限受限的本地 Trace 中记录完整模型可见 messages、System Prompt、tools schema、assistant Markdown、reasoning、工具入参与结果；若最后一次工具结果没有进入后续模型请求，还会在开发者模式下单独记录带截断状态的工具响应。内容可能包含敏感项目数据，并沿用 Trace 保留策略，已有纯元数据 Trace 不追溯补写。主视图按 Request → Output / Input → 消息或工具组 → 单次 Tool Call 分层，Readable 会将调用与结果按 call ID 配对，Debug 保留每次请求的精确原始消息；工具组默认展开，点击消息或单次调用会在可拖拽宽度的右侧检查器中查看 Summary、Arguments、Response、Schema、Timing、Source、System Prompt、Markdown Preview、Raw 与工具定义，窄屏自动切换为右侧抽屉。主列表使用无亮边的低对比选中背景并收紧行距，切换详情时不会撑开层级。原有多层调用树和事件账本作为次级分析保留。三泳道时间总览支持 TTFT / generation 分段、P50/P95、吞吐、实际时间/压缩空闲/等宽步骤、精确悬停、区间聚焦、滚轮缩放、右键平移和重置，并继续提供 request/tool 行内耗时、token、缓存命中、完整 JSONL 导出、自适应窄屏、深浅主题及中英文。新设置仅为“开发者模式”；关闭时完整内容不会进入 Trace。
- Added the user-level experimental Developer Mode switch. When enabled, both Writing and Game primary navigation expose the shared Trajectory page without implicitly changing the selected content mode. Future runs store complete model-visible messages, System Prompts, tool schemas, assistant Markdown, reasoning, tool payloads, and results in permission-restricted local traces. If a terminal tool result never reaches another model request, Developer Mode records a separate tool response with its truncation state. Content may contain sensitive project data and follows trace retention, while existing metadata-only traces are not backfilled. The primary view is hierarchical: Request → Output/Input → message or tool group → individual Tool Call. Readable mode pairs calls and results by call ID, while Debug preserves each request's exact source messages. Tool groups start expanded; selecting a message or individual call opens a resizable right-side inspector for Summary, Arguments, Response, Schema, Timing, Source, System Prompt, Markdown Preview, Raw, and tool definitions, with an automatic right drawer on compact screens. Dense rows use a low-contrast, borderless selection state and never expand the hierarchy merely to switch details. The nested call tree and event ledger remain available as secondary analysis. The three-lane overview retains segmented TTFT/generation, P50/P95, throughput, actual/idle-compressed/equal-step scales, exact hover, interval focus, wheel zoom, right-drag pan, and reset, alongside inline request/tool latency, token/cache metrics, complete JSONL export, adaptive compact layouts, light/dark themes, and Chinese/English. Developer Mode is the only new setting; complete content is not persisted while it is disabled.
- 所有启用 Skills 的 Agent 现在都会在稳定内建提示词之后、项目与回合上下文之前，注入独立且可审计的高优先级 Skills 目录：按名称稳定列出当前 Agent 实际可用的 `name + description`，并明确名称/描述匹配、按需加载、显式 `/skill-name` 去重、多个 Skill 协调、引用读取与失败回退规则。完整 `SKILL.md` 仍仅在命中后通过 `skill` 工具加载；动态目录已从 provider Tool Schema 移除，避免 Skill 增删改破坏工具前缀缓存。目录复用现有高上限上下文配置，对异常超长描述公平缩减并保持 UTF-8 与标记边界安全；写作、游戏、General、Config、Image、Harness Optimizer 及其可用子 Agent 共用同一装配路径，无需新增配置。
- Every Skills-enabled Agent now receives a separate, attributable, high-priority Skills catalog after the stable built-in instruction and before project or turn context. It deterministically lists the effective `name + description` entries for that Agent and defines name/description matching, progressive loading, explicit `/skill-name` deduplication, multi-Skill coordination, reference reads, and failure fallback. Complete `SKILL.md` bodies remain on demand through the `skill` tool, while the dynamic catalog no longer changes the provider Tool Schema or its cacheable prefix. The catalog reuses the existing generous context limits, fairly shortens pathological descriptions while preserving UTF-8 and framing boundaries, and shares one assembly path across Writing, Game, General, Config, Image, Harness Optimizer, and eligible child Agents. No new setting is required.
- Diff 审阅打开前现在会预热审阅线程与 Monaco，并将外层切换收敛为不触发布局重算的合成层动画；写作页保留底层编辑器状态，审阅聚焦时侧栏几何在一帧内完成，减少首次打开时的停顿。审阅工具栏同步收紧密度并展示总增删行数。右侧变更文件导航复用项目文件树的虚拟化视口、目录展开、选中与键盘语义，只投影本次有变更的文件，支持路径筛选，并用中英文状态图标区分新增、修改、删除、冲突与已接受；窄屏继续使用紧凑下拉导航，深浅主题与 Writing / AgentChat 审阅入口共享，无需新增配置。
- Diff review now warms its review thread and Monaco before opening, and its outer transition uses a compositor-only layer without repeated layout recalculation. Writing keeps the underlying editor state mounted, sidebar geometry settles in one frame while Review is focused, and the denser toolbar shows aggregate added/deleted lines. The changed-file navigator reuses the Project explorer's virtualized viewport, directory expansion, selection, and keyboard semantics; it projects changed files only, supports path filtering, and distinguishes added, modified, deleted, conflicted, and accepted files with localized status icons. Narrow layouts retain the compact dropdown, while light/dark themes and Writing/AgentChat review entry points share the behavior. No new setting is required.
- 工具调用卡片移除重复的“调用工具”前缀，直接以本地化工具名作为主标题，为参数与执行摘要释放更多横向空间；内置工具补齐中英文名称，自定义工具继续原样显示。写作、游戏与工作台 Agent 会话共享该调整，无需新增配置。
- Tool-call cards now use the localized tool name as the primary title without the repetitive “Calling Tool” prefix, leaving more horizontal space for arguments and execution summaries. Built-in tools have Chinese and English names, while custom tool identities remain unchanged. Writing, Game, and Workspace Agent conversations share the update, with no new setting required.
- Agent 提示词与上下文生命周期新增显式 stable-prefix / session-state / turn / checkpoint / audit 契约；可变 Context State 现在以持久化 snapshot 驱动首次注入、无变化零注入、增量更新、显式删除及压缩后重建，不再反复改写稳定前缀。写作模式的 workspace runtime snapshot 与游戏模式的 resident lore 已接入同一协议；自动/手动压缩、只读 inspection 与重试共享重建规则。General、Harness Optimizer、Background Director 与 context-compaction 的长静态提示词迁入嵌入式 Markdown，仍由 `SystemPromptComposition` 统一准入和生成 manifest/hash。开发模型输入日志新增有序 system section、message、Context State 与 tool schema 指纹、相邻请求首个分叉点，以及按 `call_id` 关联的 provider cache-read token 与命中比例；未提供 cache-write 指标时明确标记 unknown。无需新增配置，也未引入 Tool Schema 延迟发现。Beta 不兼容：轻量 Agent Store 使用新的 v1 transcript，不再读取旧版内部 runtime checkpoint；工作区文件与产品 canonical conversation 数据不受影响。
- Agent prompt and context lifecycles now use explicit stable-prefix, session-state, turn, checkpoint, and audit contracts. Mutable Context State uses a durable snapshot for initial injection, zero output when unchanged, append-only updates, explicit removals, and post-compaction rehydration instead of rewriting the stable prefix. Writing workspace runtime snapshots and Game resident lore share the protocol, while automatic/manual compaction, read-only inspection, and retries use the same rebuild rules. Long static prompts for General, Harness Optimizer, Background Director, and context compaction now live in embedded Markdown while `SystemPromptComposition` remains the sole admission and manifest/hash authority. Developer model-input logs add ordered system-section, message, Context State, and tool-schema fingerprints, the first divergence between adjacent requests, and provider cache-read tokens and hit ratios correlated by `call_id`; unavailable cache-write metrics are explicitly marked unknown. No new setting or deferred Tool Schema mechanism was added. Beta breaking change: the lightweight Agent Store uses a fresh v1 transcript and does not read old internal runtime checkpoints; workspace files and product-owned canonical conversation data are unaffected.
...
- Fixed Game stories containing released v0.3.3 `context_compaction` or `context_compaction_removed` events failing to open after upgrade. During the first legacy-index rebuild, Denova now fully backs up the original JSONL, atomically removes these retired non-content/non-state events, and reconnects event `parent_id` plus branch `head/from_event` references to the nearest surviving ancestor. The migration is idempotent, current-format stories are not scanned, and no new setting is required.
- 修复 Agent 会话历史按思考、工具调用和进展片段的数量分页，导致单个长回合刷新后被截断、错误显示“加载更早消息”的问题。分页大小现在只作为目标，并始终向前对齐到完整用户回合；写作、游戏与工作台 Agent 会话共享该修复，无需新增配置。
- Fixed Agent conversation history counting reasoning, tool-call, and progress fragments as independent page boundaries, which split a single long turn after refresh and incorrectly showed “Load earlier messages.” Page size is now a target that always aligns backward to a complete user turn. Writing, Game, and Workspace Agent conversations share the fix, with no new setting required.
- 修复写作续写在 DeepSeek 等 function-calling provider 上因 `todo` 等 union Tool Schema 缺少顶层 `type: object` 而在生成前被拒绝的问题。所有 union 工具现在都显式声明 object 根类型；已接收请求的运行失败会在对话中持久、完整换行展示，不再只留下用户消息。“续写下一段”同时明确以实际最新非空章节为目标，追加到同一文件，不受当前编辑器选中文件影响，也不会创建下一章。无需新增配置。
- Fixed Writing continuation being rejected before generation by function-calling providers such as DeepSeek because union tool schemas including `todo` omitted the root `type: object`. Every union tool now declares an object root; failures after request acceptance remain visible in the conversation as persistent, fully wrapping alerts instead of leaving only the user message. “Continue Next Paragraph” now explicitly appends to the actual latest non-empty chapter regardless of the selected editor file and never creates the next chapter. No new setting is required.
- 修复 Agent 对话在短会话中经历思考、工具调用与正文尾行替换后只能短暂锁底、随后内容继续增长但视口停止跟随，以及运行结束后短对话贴底导致上方留出大块空白的问题。流式控制器现在从稳定滚动容器解析真实尾行，响应跑道独占运行中的垂直定位；短对话始终从顶部排列，长历史首次打开仍定位到最新消息。写作、游戏、工作台与子 Agent 会话共用该修复，无需新增配置。
- Fixed Agent conversations briefly following the bottom and then stopping after thinking, tool-call, and prose tail-row replacements in short sessions, as well as large blank space above short conversations after a run settled. The streaming controller now resolves the rendered tail from the stable scroller and the response runway exclusively owns active vertical placement; short conversations remain top-aligned, while long restored history still opens at the latest message. Writing, Game, Workspace, and child-Agent conversations share the fix, with no new setting required.
...
- 移除本开发周期内未发布的 Agent actor/mailbox、runtime reducer、checkpoint 与 command index、精确 command replay、冷恢复 action、host-effect outbox、canonical reconcile 回调、工具 retry-safety journal、运行时全局 Limits，以及 file Store 的 `NewWithOptions` 调优面。旧 `agent-sessions` 目录不读取、不迁移；新 transcript 位于 `agent-transcripts`。这些删除只影响未发布的内部运行状态格式。
- Removed the unreleased Agent actor/mailbox, runtime reducer, checkpoints and command index, exact command replay, cold-recovery actions, host-effect outbox, canonical reconcile callbacks, tool retry-safety journal, runtime-wide Limits, and the file Store `NewWithOptions` tuning surface. The old `agent-sessions` directory is neither read nor migrated; new transcript state lives in `agent-transcripts`. These removals affect only unreleased internal runtime formats.
...
- AgentChat 与创作 Agent 新增会话级 `/goal`：通过 `/goal` 或输入动作菜单顶部的「目标」开关显式进入，未激活时不在输入框常驻；激活后在权限右侧显示可关闭的模式提示，并与同样样式的「计划」提示严格互斥。提交后可编辑、暂停、清除目标，活动目标会以紧凑卡片固定展示在输入框上方；游戏模式不提供 Goal。目标通过 Conversation Journal 持久化并以稳定的最终用户前缀注入模型上下文；运行成功后若目标仍活动，会创建 UI 隐藏但模型可见的持久化续跑指令，直至模型通过仅根 Agent 可用的 `goal` 工具标记完成或阻塞。Goal 能力沿用 Agent capability 配置，不增加用户配置项；所有状态、API、恢复链路和交互均支持中英文。
- AgentChat and the Writing Agent now support session-scoped `/goal`: Goal mode is entered explicitly through `/goal` or the Goal toggle at the top of Input Actions and never remains visible in an inactive composer. Once active, its removable mode indicator appears to the right of permission and is strictly exclusive with the identically styled Plan indicator. Submitted goals can be edited, paused, or cleared, while the active goal remains in a compact card directly above the composer; Game mode does not expose Goal. Goals persist through the Conversation Journal and are injected as a stable final-user prefix for model context. After a successful run, an active goal creates a durable continuation input that stays hidden from the UI but visible to the model until the root Agent calls the `goal` tool with completed or blocked status. Goal reuses the existing Agent capability configuration without adding a user setting, and all states, APIs, recovery paths, and interactions support Chinese and English.
