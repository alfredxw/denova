...
- 互动模式故事舞台支持像 IDE 模式一样通过 `#` 引用 `setting/styles/` 下的风格参考，本轮会随互动 Agent 请求注入。
- 互动模式支持复用场景化风格规则；讲述者页面新增“讲述者配置”，集中维护场景风格规则和互动单轮目标字数。
...
- 设置页不再展示场景化风格规则和互动单轮目标字数，这两项移动到讲述者页面的“讲述者配置”。
