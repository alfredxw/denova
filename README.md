...
- **章节编辑**：支持 Markdown / TXT 编辑、自动保存、手动保存、字数统计、文章内搜索、选区引用和多 Tab。
...
3. 在 `chapters/` 下写正文，章节文件使用 `chXX-章节名.md` 命名。
4. 每完成一段稳定内容就创建版本，后续重写和续写更容易回退。
...
- 资料库支持手动创建版本和恢复历史版本。
- 活动栏的上传图标可导入 SillyTavern 酒馆 v2 PNG / JSON 角色卡，并选择写入当前书籍或创建新书。
...
- 工作区级适合当前书籍的章节命名、最大打开 Tab、场景化风格规则、互动模式字数等。
