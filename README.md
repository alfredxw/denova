...
游戏模式内置故事导演能力：互动 Agent 负责理解玩家行动并生成本回合 TurnBrief，后端工具只执行字段校验、状态结算和固定 d20 检定等确定性工作；后台 Director Agent 在每个回合落盘后维护分支级单文档 `director.md`，用“正文Agent可读”和“后台导演私密”两个区域同时承载阶段钩子、资料库锚点、重要角色/势力、当前场景、线索密度、检定代价、危机反转和最近分支安排。互动正文只读取 `director.md` 的有界可见区，导演私密区只提供给后台导演；每个非终局 TurnResult 同时持久化 2–4 个行动建议供玩家舞台展示，不再运行独立快捷选项 Agent 或追加模型请求。创作者可以在故事导演中组合或关闭叙事风格、事件包、TRPG 检定、状态系统、Story Memory Structure 和图像方案，并可配置分支规划回合数、单份规划模板与 `off / sparse / balanced / frequent` 事件机会频率。事件包是显式选择的剧情事件库，不是随机队列：后端按已提交回合确定性产生评估机会，Director 可选择不采用，或审计事件的埋设、推进、回收、解决和放弃；活动事件按分支保存在 Director metadata，支持重试、分支和回退。新事件机会到期时 Director 只看到紧凑卡片索引，需要时再按需读取卡片正文。导演规划会优先参考资料库里的重要角色、势力、规则和地点，非必要不临时自创；每个可玩回合都要求推进有效信息、关系变化、压力升级、收益/代价或新悬念，保持互动小说式的高信息密度和阅读节奏。状态系统定义 Actor 字段 schema、默认值、边界、可见性、初始 Actor、通用词条库和模板抽取规则；默认情况下，新故事创建前会由初始化 Director 根据有界的故事设定对预设模板、字段和初始 Actor 做一次差异适配，后端校验 TRPG 引用后再冻结为故事专属 schema，也可在故事导演中关闭该步骤并直接使用原始预设。主角、重要角色、敌人与怪物都在创建时自动获得词条定义快照；TRPG 检定提供固定 d20 规则模板，并可通过 State Binding 读取状态字段计算修正；Story Memory Structure 定义长期记忆的结构和字段，具体记忆记录仍属于单个故事/分支。正文 Agent 只接收来源明确、按可见性过滤且不超过 64KB 的当前 Actor 状态摘要，完整词条库不会进入每轮上下文。
