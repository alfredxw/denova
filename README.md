...
如果通过 npm 安装发布版，只需要 Node.js 18+：

```bash
npm install -g nova-ai-novel
nova --workspace /path/to/your-novel
```

npm 版默认把 Nova 数据目录放在执行命令时的当前目录：
...
```bash
npx nova-ai-novel --workspace /path/to/your-novel
```
...
- `NOVA_DIR` 是 Nova 数据目录，新建书籍、用户级配置和最近书籍记录会放在这里；npm 版未显式配置时默认使用当前目录的 `./.nova`。
- `NOVA_WEB_DIR` 指定生产态 Web 静态资源目录；开发态通常不需要配置，npm 包会自动指向包内构建产物。
...
指定 registry 或 npm OTP：

```bash
scripts/npm-release.sh --publish --registry https://registry.npmjs.org/ --otp 123456
```

脚本默认会构建 `web/dist`，交叉编译 macOS/Linux/Windows 的 Nova 二进制，并把发布所需文件复制到 `npm/`；随后执行 `npm pack --dry-run --ignore-scripts` 预览包内容。只有显式传入 `--publish` 时才会执行 `npm publish`。npm 发布版仍是前后端分离架构：前端负责 UI，后端负责 Agent 和文件管理；部署形态上由 Go 服务托管已经构建好的 Web 静态资源，从而实现一个命令启动完整工作台。

### GitHub Release 发布

本地生成 GitHub Release 下载包：

```bash
scripts/build-github-release.sh v0.1.5
```

产物会输出到 `dist/github-release/`，包含：

```text
nova-v0.1.5-darwin-arm64.tar.gz
nova-v0.1.5-darwin-x64.tar.gz
nova-v0.1.5-linux-arm64.tar.gz
nova-v0.1.5-linux-x64.tar.gz
nova-v0.1.5-windows-x64.zip
checksums.txt
```

推送版本 tag 后，GitHub Actions 会自动创建或更新 GitHub Release，并上传这些下载包：
...
最新版本：v0.1.5（2026-06-02）。完整记录见 [CHANGELOG.md](./CHANGELOG.md)。
