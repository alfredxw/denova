...
- **默认保护创作过程**：Nova 使用 go-git 在作品根目录维护本地 `.git` 仓库，支持手动保存、历史查看、差异对比、恢复和自动保存；正文、设置和 `.nova` 内的 lore/session 等创作状态都会保存，仅排除 `.nova/versions/` 版本元数据，无需安装系统 Git 或手动初始化仓库。
...
- **版本管理**：手动保存版本、查看历史和差异、恢复旧版本，并支持定时与 Agent 大量输出自动保存；`.nova/lore`、`.nova/sessions` 等本地创作状态会进入版本，`.nova/versions/` 仅作为版本索引和内部元数据保留在版本外。
